    ...
    int (*stop)(double t, const double y[], double dydt[], void *params), double eps_abs, double eps_rel)
{
    ...
    // XLAL_CALLGSL(integrator->step = gsl_odeiv_step_alloc(gsl_odeiv_step_rk8pd,
    // dim));
                                         ...
                                         REAL8 deltat_or_h0, REAL8 min_deltat_or_h0, REAL8Array **t_and_y_out)
{
    ...
    /* note: for speed, this replaces the single CALLGSL wrapper applied before
     * each GSL call */
    XLAL_BEGINGSL;

    /* allocate the buffers!
     * note: REAL8Array has a field dimLength (UINT4Vector) with dimensions, and
     * a field data that points to a single memory block; dimLength itself has
     * fields length and data */
    dim = integrator->sys->dimension;
    bufferlength =
        (int)((tend - tinit) / deltat_or_h0) + 2; /* allow for the initial value and possibly a final semi-step */

    UINT dimn; /* Variable for different loop indices below */
    if (EOBversion == 2)
        dimn = dim + 1;
    else
        dimn = dim + 4; // v3opt: Include three derivatives
    ...
    t = tinit;
    h0 = deltat_or_h0;
    h0old = h0; /* initialized so that it will not trigger the check h0<h0old at
                   the first step */
    ...
    try_step:
        ...
        /* note: If the user-supplied functions defined in the system dydt return
         * a status other than GSL_SUCCESS, the step will be aborted. In this
         * case, the elements of y will be restored to their pre-step values, and
         * the error code from the user-supplied function will be returned. */
        ...
        if (status != GSL_SUCCESS)
        {
            if (retries--)
            {
                h0 = h0 / 10.0; /* if we have singularity retries left, reduce
                                   the timestep and try again */
                goto try_step;
            }
            else
            {
        ...
        {
            retries = integrator->retries; /* we stepped successfully, reset the
                                              singularity retries */
        }
         ...
         * other possible return codes are GSL_ODEIV_HADJ_INC if it was
         * increased, GSL_ODEIV_HADJ_NIL if it was unchanged since we introduced
         * a minimal step size we simply compare to the saved value of h0 */
        ...
        if (outputlength >= bufferlength)
        {
            ...
            /* sadly, we cannot use ResizeREAL8Array, because it would only work
             * if we extended the first array dimension, so we need to copy
             * everything over and switch the buffers. Oh well. */
            ...
            else
            {
                for (i = 0; i <= loop /* dim */; i++)
                    memcpy(&rebuffers->data[i * 2 * bufferlength], &buffers->data[i * bufferlength],
                           outputlength * sizeof(REAL8));
        ...
        {
            for (i = 1; i <= 3; i++)
                buffers->data[(dim + i) * bufferlength + outputlength] =
                    dydt_out[i - 1]; // OPTV3: Include 3 derivatives
        }
    } // End loop
    ...
    /* if we have completed at least one step, allocate the GSL interpolation
...
bail_out:
    ...
    // if (buffers)
    //     XLALDestroyREAL8Array(buffers); /* let's be careful, although all
                                                      ...
                                                      REAL8 min_deltat_or_h0, REAL8Array **t_and_y_and_dydt_out)
{
    ...
    dimn = dim + nderiv + 1 + nextra; // SEOBPrec: t,
                                      // (x,y,z,pTx,pTy,pTz,s1x,s1y,s1z,s2x,s2y,s2z,phiM,phi),
                                      // (vx,vy,vz), H, flux, prTDot
    // dimn = dim + 1;
    buffers = CreateREAL8Array(2, dimn, bufferlength);
                            ...
                            REAL8 deltat, REAL8Array **yout)
{
    ...
    try_step:
        ...
        /* did the error-checker reduce the stepsize?
         * note: other possible return codes are GSL_ODEIV_HADJ_INC if it was
         * increased, GSL_ODEIV_HADJ_NIL if it was unchanged */
        if (status == GSL_ODEIV_HADJ_DEC)
        {
            memcpy(y, y0, dim * sizeof(REAL8)); /* if so, undo the step, and try again */
            memcpy(dydt_in, dydt_in0, dim * sizeof(REAL8));
