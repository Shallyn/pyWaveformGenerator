...
unsigned long LOG_PrintLogTime(unsigned char *ucTime, unsigned long ulBufLen)
{
    ...
    snprintf((char *)ucTime, ulBufLen - 1, "%04d-%02d-%02d %02d:%02d:%02d %03dms", pstTmSec->tm_year + 1900,
             pstTmSec->tm_mon + 1, pstTmSec->tm_mday, pstTmSec->tm_hour, pstTmSec->tm_min, pstTmSec->tm_sec,
             stTmMsec.tv_usec / 1000);
...
unsigned long LOG_Init(char *ucLogFileName, unsigned long ulFileSize)
{
    ...
    {
        // fprintf(stderr, "g_ulPrintLogPlaceFlag = %ld g_ulLogInitFlag =
