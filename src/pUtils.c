...
{
    // Rodrigues rotation formula, see
...
REAL8
inner_product3d(const REAL8 values1[], const REAL8 values2[])
{
    REAL8 result = 0;
    INT i;
    for (i = 0; i < 3; i++)
...
REAL8 *cross_product3d(const REAL8 values1[], const REAL8 values2[], REAL8 result[])
{
...
REAL8
SEOBCalculateChiS(REAL8 chi1dotZ, REAL8 chi2dotZ)
{
    return 0.5 * (chi1dotZ + chi2dotZ);
}
REAL8
SEOBCalculateChiA(REAL8 chi1dotZ, REAL8 chi2dotZ)
{
    return 0.5 * (chi1dotZ - chi2dotZ);
}

INT EOBCalculateSigmaStar(REAL8Vector *sigmaStar, REAL8 mass1, REAL8 mass2, REAL8Vector *s1norm, REAL8Vector *s2norm)
...
INT EOBCalculateSigmaKerr(REAL8Vector *sigmaKerr, REAL8Vector *s1norm, REAL8Vector *s2norm)
{
    UINT i;
...
INT XLALSimIMRSpinEOBCalculateSigmaStar(REAL8Vector *sigmaStar, REAL8 mass1, REAL8 mass2, REAL8Vector *s1,
                                        REAL8Vector *s2)
...
INT XLALSimIMRSpinEOBCalculateSigmaKerr(REAL8Vector *sigmaKerr, REAL8 mass1, REAL8 mass2, REAL8Vector *s1,
                                        REAL8Vector *s2)
{
    UINT i;
    REAL8 mTotal = mass1 + mass2, mT2;
...
SpinEOBDynamics *CreateSpinEOBDynamics(UINT length)
{
    ...
    dy->t0 = 0.0;
    dy->length = length;
    dy->tVec = dy->xVec = dy->yVec = dy->zVec = dy->pxVec = dy->pyVec = dy->pzVec = NULL;
    dy->tVec = CreateREAL8Vector(length);
    if (!dy->tVec)
    {
        STRUCTFREE(dy, SpinEOBDynamics);
        return NULL;
    }
...
void DestroySpinEOBParams(SpinEOBParams *params)
{
    ...
    {
        MYFree(params->seobCoeffs);
        params->seobCoeffs = NULL;
    }
    if (params->seobCoeffConsts)
    {
        MYFree(params->seobCoeffConsts);
        params->seobCoeffConsts = NULL;
    }
    if (params->hParams)
    {
        MYFree(params->hParams);
        params->hParams = NULL;
    }
    if (params->prefixes)
    {
        MYFree(params->prefixes);
        params->prefixes = NULL;
    }
    if (params->hCoeffs)
    {
        MYFree(params->hCoeffs);
        params->hCoeffs = NULL;
    }
    if (params->nqcCoeffs)
    {
        MYFree(params->nqcCoeffs);
        params->nqcCoeffs = NULL;
    }
    if (params->eccCoeffs)
    {
        MYFree(params->eccCoeffs);
        params->eccCoeffs = NULL;
    }
    if (params->saCoeffs)
    {
        MYFree(params->saCoeffs);
        params->saCoeffs = NULL;
    }
    MYFree(params);
    return;
}
...
SEOBdynamics *CreateSEOBdynamics(INT length)
{
    ...
    if (!ret->array)
        return NULL;
    /* Set array pointers corresponding to the data vectors (successive vectors
     * of length retLen in the 1D array data) */
...
int SphHarmListEOBNonQCCoeffs_AddMode(SphHarmListEOBNonQCCoeffs **list_prepended, /* List structure to prepend to */
...
)
{
    ...
    { /* We don't allow for the case where the mode already exists in
                    the list*/
        PRINT_LOG_INFO(LOG_CRITICAL, "tried to add an already existing mode to a "
...
SphHarmListEOBNonQCCoeffs *SphHarmListEOBNonQCCoeffs_GetMode(
    SphHarmListEOBNonQCCoeffs *list, /* List structure to get a particular mode from */
    UINT l,                          /*< Mode number l */
    INT m                            /*< Mode number m */
)
{
...
int DestroySphHarmListEOBNonQCCoeffs(SphHarmListEOBNonQCCoeffs *list /* Pointer to list to be destroyed */
)
{
...
int CAmpPhaseSequence_Init(CAmpPhaseSequence **campphase, /* Double pointer to campphase sequence */
...
void DestroyCAmpPhaseSequence(CAmpPhaseSequence *campphase /* Pointer to structure to be destroyed */
)
{
...
int SphHarmListCAmpPhaseSequence_AddMode(
    SphHarmListCAmpPhaseSequence **list_prepended, /* List structure to prepend to */
...
int DestroySphHarmListCAmpPhaseSequence(SphHarmListCAmpPhaseSequence *list /* Pointer to list to be destroyed */
)
{
 ...
 */
void XLALSphHarmTimeSeriesSetTData(SphHarmTimeSeries *ts, /**< Linked list to be prepended */
 ...
 */
COMPLEX16TimeSeries *XLALSphHarmTimeSeriesGetMode(SphHarmTimeSeries *ts, /**< linked list to extract mode from */
 ...
 * \exp{i m' \gamma}\f] with the notation \f$ c,s = \cos, \sin (\beta/2)\f$,
 * \f$ k_{min} = \max(0,m-m'), k_{max} = \min(l+m, l-m')\f$:
 *
 ...
 */
REAL8
SEOBWignerDAmp(UINT l, INT m, INT mp, REAL8 beta)
{
    return WignerdMatrix(l, m, mp, beta);
}

REAL8
SEOBWignerDPhase(INT m, INT mp, REAL8 alpha, REAL8 gamma)
{
...
int XLALEOBFindRobustPeak(REAL8 *tPeakQuant, REAL8Vector *tVec, REAL8Vector *quantVec, UINT window_width)
{
    ...
    {
        lo = kk - window_width;
        hi = kk + window_width + 1; // Slice function does not return the value at the end
        sl = get_slice(quantVec, lo, hi);
        local_argmax = argmax(sl);
        if (sl->data[local_argmax] > sl->data[0] && sl->data[local_argmax] > sl->data[sl->length - 1])
        {
    ...
    *tPeakQuant = 0;
    ...
    if (idx_global == 0 ||
        ((quantVec->data[global_arg_max] - quantVec->data[idx_global]) / quantVec->data[idx_global] > 0.1) ||
