...
/* Spin EOB root */
typedef struct tagSEOBRootParams
{
    REAL8 values[12];      /**<< Dynamical variables, x, y, z, px, py, pz, S1x, S1y,
                              S1z, S2x, S2y and S2z */
...
static inline REAL8 CalculateCrossProduct(const INT4 i, const REAL8 a[], const REAL8 b[])
{
...
static inline REAL8 CalculateDotProduct(const REAL8 a[], const REAL8 b[])
{
    return a[0] * b[0] + a[1] * b[1] + a[2] * b[2];
}

static int CalculateRotationMatrix(gsl_matrix *rotMatrix,  /**< OUTPUT, rotation matrix */
...
)
{
     ...
     * 2. rotate about the former x-axis (now x') by an angle b, rotation matrix
     * Rx(b);
     * 3. rotate about the former z-axis (now z') by an algle g, rotation matrix
     * Rz(g);
     */
    /* populate the matrix */
...
static int ApplyRotationMatrix(gsl_matrix *rotMatrix, /**< rotation matrix */
...
static int XLALFindSphericalOrbit(const gsl_vector *x, /**<< Parameters requested by gsl root finder */
...
/**
 * Wrapper for calculating specific derivative of the Hamiltonian in spherical
 * co-ordinates, dH/dr, dH/dptheta and dH/dpphi. It only works for the specific
 * co-ord system we use here
 */
...
REAL8
XLALCalculateSphHamiltonianDeriv2(const int idx1,       /**<< Derivative w.r.t. index 1 */
...
)
{
    ...
    // result = GSLSpinHamiltonianDerivWrapper( values[idx1] + STEP_SIZE,
    // &dParams
    // )
    //         - GSLSpinHamiltonianDerivWrapper( values[idx1] - STEP_SIZE,
    //         &dParams );
    // printf( "%.16e - %.16e / 2h\n", GSLSpinHamiltonianDerivWrapper(
    // values[idx1] + STEP_SIZE, &dParams ), GSLSpinHamiltonianDerivWrapper(
    // values[idx1] - STEP_SIZE, &dParams ) );

    // result = result / ( 2.*STEP_SIZE );

    gslStatus = gsl_deriv_central(&F, values[idx1], STEP_SIZE, &result, &absErr);
...
INT EOBInitialConditions(REAL8Vector *initConds, const REAL8 mass1, const REAL8 mass2, const REAL8 fMin,
                         const REAL8 ecc, const REAL8 inc, const REAL8 spin1[], const REAL8 spin2[],
                         SpinEOBParams *params)
{
     ...
     * LNhat0 and N0 are initial normal to orbital plane and initial orbital
     * separation;
     */
    ...
    /* Set the radial direction - need to take care to avoid singularity if L is
     * along z axis */
    if (LnHat[2] > 0.9999)
    {
        rHat[0] = 1.;
    ...
    /* STEP 2) After rotation in STEP 1, in spherical coordinates, phi0 and
     * theta0 are given directly in Eq. (4.7), r0, pr0, ptheta0 and pphi0 are
     * obtained by solving Eqs. (4.8) and (4.9) (using gsl_multiroot_fsolver). At
     * this step, we find initial conditions for a spherical orbit without
     * radiation reaction.
     */
    REAL8 fMinE = fMin;
    rootParams.e0 = 0.0;
    ...
    //  finalValues, 0 ), gsl_vector_get( finalValues, 1 ), gsl_vector_get(
    //  finalValues, 2 ) );
    ...
    /* STEP 3) Rotate to L0 along z-axis and N0 along x-axis frame, where L0 is
     * the initial orbital angular momentum and L0 is calculated using initial
     * position and linear momentum obtained in STEP 2.
     */
    ...
    if (d2Hdr2 != 0.0 && ecc == 0.0)
    {
        ...
        // qCart[2]); print_debug( "pCartVec = (%g, %g, %g)\n", pCart[0],
        // pCart[1], pCart[2]); print_debug( "s1Vec = (%g, %g, %g)\n", tmpS1[0],
        // tmpS1[1], tmpS1[2]); print_debug( "s2Vec = (%g, %g, %g)\n", tmpS2[0],
        // tmpS2[1], tmpS2[2]);
        ...
        // print_debug( "sigStar = (%g, %g, %g)\n", sStarData[0], sStarData[1],
        // sStarData[2]); print_debug( "sigKerr = (%g, %g, %g)\n", sStarData[0],
        // sStarData[1], sStarData[2]);
        ...
        // params->eobParams->hCoeffs, mass1, mass2, eta, /*a*/0.0, chiS, chiA );
        // XLALSimIMRCalculateSpinEOBHCoeffs( params->seobCoeffs, eta, a );
        ham = EOBHamiltonian(eta, &qCartVec, &pCartVec, &s1VecNorm, &s2VecNorm, &sKerr, &sStar, params->tortoise,
                             params->seobCoeffs);
        ...
        flux = XLALInspiralSpinFactorizedFlux(&polarDynamics, &cartDynamics, nqcCoeffs, omega, 0, qSph[0] * omega,
                                              params, ham, lMax);
        // flux  = InspiralSpinFactorizedFlux_elip( &polarDynamics, polarData,
        // tmpdvalues, nqcCoeffs, omega, params, ham, lMax, eobVersion );
        flux = flux / eta;
        if (ecc != 0.0)
            rDot = 0.0;
        else
        ...
        /* We now need dHdpr - we take it that it is safely linear up to a pr
         * of 1.0e-3 */
        cartValues[3] = 1.0e-3;
    ...
    {
        /* Since d2Hdr2 has evaluated to zero, we cannot do the above. Just set
         * pr to zero */
    ...
    PRINT_LOG_INFO(LOG_DEBUG, "Sph initial condition : r = (%e,%e,%e), p = (%e,%e,%e)", qSph[0], qSph[1], qSph[2],
                   pSph[0], pSph[1], pSph[2]);
    /* STEP 5) Rotate back to the original inertial frame by inverting the
     * rotation of STEP 3 and then inverting the rotation of STEP 1.
     */
    ...
    /*printf( " %.16e %.16e %.16e\n%.16e %.16e %.16e\n%.16e %.16e %.16e\n%.16e
     ...
     initConds->data[3], initConds->data[4], initConds->data[5],
     initConds->data[6], initConds->data[7], initConds->data[8],
...
static int XLALRobustDerivative(const gsl_function *F, double x, double h, double *result, double *absErr)
{
    ...
    REAL8 frac = 0.01; // Adjust this to change how accurtate we demand the
                       // derivative to be
    if (fabs(*absErr) > frac * fabs(*result))
    {
        ...
        while (!deriv_ok && n <= 10)
        {
            PRINT_LOG_INFO(LOG_WARNING, "Warning: second derivative "
                                        "computation went wrong. Trying again");
        ...
        if (!deriv_ok)
        {
            PRINT_LOG_INFO(LOG_CRITICAL, "The computation of the second derivative "
                                         "of H in initial data has failed!");
            return CEV_FAILURE;
        }
    }
    return CEV_FAILURE;
...
static double GSLSpinHamiltonianDerivWrapperPrec(double x,    /**<< Derivative at x */
                                                 void *params /**<< Function parameters */
)
{
    // int		debugPK = 0;
    ...
    double Lvec[3] = {CalculateCrossProduct(0, rvec, pvec), CalculateCrossProduct(1, rvec, pvec),
                      CalculateCrossProduct(2, rvec, pvec)};
    double theta1 = acos(CalculateDotProduct(chi1vec, Lvec) / sqrt(CalculateDotProduct(chi1vec, chi1vec)) /
                         sqrt(CalculateDotProduct(Lvec, Lvec)));
    double theta2 = acos(CalculateDotProduct(chi2vec, Lvec) / sqrt(CalculateDotProduct(chi2vec, chi2vec)) /
                         sqrt(CalculateDotProduct(Lvec, Lvec)));
    // if (debugPK) {
    // 	XLAL_PRINT_INFO("In GSLSpinHamiltonianDerivWrapperPrec");
    // 	XLAL_PRINT_INFO("rvec = %.16e %.16e %.16e\n", rvec[0], rvec[1],
    // rvec[2]); 	XLAL_PRINT_INFO("pvec = %.16e %.16e %.16e\n", pvec[0], pvec[1],
    // pvec[2]); 	XLAL_PRINT_INFO("theta1 = %.16e\n", theta1);
    ...
    if (theta1 > 1.0e-6 && theta2 >= 1.0e-6)
    {
        ...
        case 5:
            ...
            break;
        default:
            PRINT_LOG_INFO(LOG_CRITICAL, "This option is not supported in the "
 ...
 * Function to calculate the second derivative of the Hamiltonian.
 * The derivatives are taken with respect to indices idx1, idx2    */
static REAL8 XLALCalculateSphHamiltonianDeriv2Prec(
...
)
{
    ...
    XLALRobustDerivative(&F, values[idx1], STEP_SIZE, &result, &absErr);
    /*
    if (gslStatus != GSL_SUCCESS) {
            XLALPrintError("XLAL Error %s - Failure in GSL function\n",
    __func__); XLAL_ERROR_REAL8(XLAL_EDOM);
    }
    */
...
static int XLALFindSphericalOrbitPrec(const gsl_vector *x, /**<< Parameters requested by gsl root finder */
                                      void *params,        /**<< Spin EOB parameters */
                                      gsl_vector *f        /**<< Function values for the given parameters*/
)
{
    ...
    REAL8 rvec[3] = {rootParams->values[0], rootParams->values[1], rootParams->values[2]};
    REAL8 pvec[3] = {rootParams->values[3], rootParams->values[4], rootParams->values[5]};
    REAL8 chi1vec[3] = {rootParams->values[6], rootParams->values[7], rootParams->values[8]};
    REAL8 chi2vec[3] = {rootParams->values[9], rootParams->values[10], rootParams->values[11]};
    REAL8 Lvec[3] = {CalculateCrossProduct(0, rvec, pvec), CalculateCrossProduct(1, rvec, pvec),
                     CalculateCrossProduct(2, rvec, pvec)};
    REAL8 theta1 = acos(CalculateDotProduct(chi1vec, Lvec) / sqrt(CalculateDotProduct(chi1vec, chi1vec)) /
                        sqrt(CalculateDotProduct(Lvec, Lvec)));
    REAL8 theta2 = acos(CalculateDotProduct(chi2vec, Lvec) / sqrt(CalculateDotProduct(chi2vec, chi2vec)) /
                        sqrt(CalculateDotProduct(Lvec, Lvec)));
    ...
    else
    {
        ...
        // rootParams->values[8] = sqrt(CalculateDotProductPrec(chi1vec,
        // chi1vec)); rootParams->values[9] = 0.; rootParams->values[10]= 0.;
        // rootParams->values[11]= sqrt(CalculateDotProductPrec(chi2vec,
        // chi2vec));
    ...
    dHdpphi = dHdpy / r;
    e0 = rootParams->e0;
    // print_debug("dHdr = %.16e dHdptheta = %.16e dHdpphi = %.16e\n",
...
static int XLALFindSphericalOrbitPrecEccAnomaly(const gsl_vector *x, /**<< Parameters requested by gsl root finder */
                                                void *params,        /**<< Spin EOB parameters */
                                                gsl_vector *f        /**<< Function values for the given parameters*/
)
{
    ...
    REAL8 dHdr, dHdpr, dHdptheta, dHdpphi;
    REAL8 e0;
    int i;
    /* Populate the appropriate values */
    /* In the special theta=pi/2 phi=0 case, r is x */
    ...
    calculate_prDot_from_ezetapphiPrec(rootParams->params->eta, rootParams->c1n, rootParams->c1l, rootParams->c1e,
                                       rootParams->c2n, rootParams->c2l, rootParams->c2e, rootParams->c1c1,
                                       rootParams->c1c2, rootParams->c2c2, rootParams->e0, rootParams->sz,
                                       rootParams->cz, py, pz, L, rootParams->omega, &px, &prDot, &rDot, &tmpr, &tmpL);
    ...
    else
    {
        ...
        // chi2vec));
        dHdx = XLALSpinPrecHcapNumDerivWRTParam(0, rootParams->values, rootParams->params);
        dHdpx = XLALSpinPrecHcapNumDerivWRTParam(3, rootParams->values, rootParams->params);
        dHdpy = XLALSpinPrecHcapNumDerivWRTParam(4, rootParams->values, rootParams->params);
        dHdpz = XLALSpinPrecHcapNumDerivWRTParam(5, rootParams->values, rootParams->params);
    }
    if (IS_REAL8_FAIL_NAN(dHdx))
    ...
    //         dHdr, dHdptheta, dHdpphi);
    /* populate the function vector */
    // print_debug("(r, px, py, pz, L) = (%.16e, %.16e, %.16e, %.16e, %.16e),
    ...
    gsl_vector_set(f, 1, dHdptheta);
    gsl_vector_set(f, 2, dHdpphi - rootParams->omega);
    // print_debug("(r, px, py, pz, L) = (%.5e, %.5e, %.5e, %.5e, %.5e), eq1 =
    // %.5e, eq2 = %.5e, eq3 = %.5e, eq4 = %.5e\n",
    //     r, px, py, pz, L, eq1, eq2, eq3, eq4);
    // print_debug("(r, tmpr) = (%.5e, %.5e), (L, tmpL) = (%.5e, %.5e), (dHdpr,
    // rDot) = (%.5e, %.5e)\n\n",
...
INT EOBInitialConditionsPrec(REAL8Vector *initConds,          /**<< OUTPUT, Initial dynamical variables */
                             const REAL8 mass1,               /**<< mass 1 */
                             const REAL8 mass2,               /**<< mass 2 */
                             const REAL8 fMin,                /**<< Initial frequency (given) */
                             const REAL8 e0, const REAL8 inc, /**<< Inclination */
                             const REAL8 spin1[],             /**<< Initial spin vector 1 */
                             const REAL8 spin2[],             /**<< Initial spin vector 2 */
                             SpinEOBParams *params            /**<< Spin EOB parameters */
)
{
    // print_debug("Inside the XLALSimIMRSpinEOBInitialConditionsPrec
    // function!\n"); print_debug( "Inputs: m1 = %.16e, m2 = %.16e, fMin = %.16e,
    // inclination = %.16e\n",
    ...
    do
    {
        ...
        if (gslStatus == GSL_ENOPROG || gslStatus == GSL_ENOPROGJ)
        {
            ...
            if (cntGslNoProgress >= MAXcntGslNoProgress)
            {
                ...
                if (multFacGslNoProgress < 1.)
                {
                    multFacGslNoProgress *= 1.02;
                }
                else
                {
                    multFacGslNoProgress /= 1.01;
                }
            }
            /* Now that no progress is being made, we need to reset the initial
             * guess for the (r,pPhi, pTheta) and reset the integrator */
            ...
            memcpy(rootParams.values + 6, tmpS1, sizeof(tmpS1));
            memcpy(rootParams.values + 9, tmpS2, sizeof(tmpS2));
            // XLAL_PRINT_INFO("New ICs guess: x = %.16e, py = %.16e, pz =
            // %.16e\n",
            //         rootParams.values[0]/scale1, rootParams.values[4]/scale2,
            //         rootParams.values[5]/scale3);
            // fflush(NULL);
        ...
        if (jittered == 0)
        {
            ...
            {
                rootParams.values[0] =
                    scale1 * sqrt(1. / (v0 * v0) * 1 / (v0 * v0) * (1. + 1.e-8) - 36.0); /* Jitter on initial r */
    ...
    // qCart[1], qCart[2]); XLAL_PRINT_INFO("pCart after rotation2 %3.10f %3.10f
    // %3.10f\n", pCart[0], pCart[1], pCart[2]); XLAL_PRINT_INFO("S1 after
    // rotation2 %3.10f %3.10f %3.10f\n", tmpS1Norm[0], tmpS1Norm[1],
    // tmpS1Norm[2]); XLAL_PRINT_INFO("S2 after rotation2 %3.10f %3.10f
    ...
    dHdpphi = tmpDValues[1] /
              sqrt(cartValues[0] * cartValues[0] + cartValues[1] * cartValues[1] + cartValues[2] * cartValues[2]);
    ...
    if (d2Hdr2 != 0.0 && e0 == 0.0)
    {
        ...
        // chi1 = tmpS1[0] * LnHat[0] + tmpS1[1] * LnHat[1] + tmpS1[2] *
        // LnHat[2]; chi2 = tmpS2[0] * LnHat[0] + tmpS2[1] * LnHat[1] + tmpS2[2]
        // * LnHat[2];
         ...
         */
        a = sqrt(sKerr.data[0] * sKerr.data[0] + sKerr.data[1] * sKerr.data[1] + sKerr.data[2] * sKerr.data[2]);
        // XLALSimIMREOBCalcSpinPrecFacWaveformCoefficients(params->eobParams->hCoeffs,
        // mass1, mass2, eta, /* a */ 0.0, chiS, chiA);
        // XLALSimIMRCalculateSpinPrecEOBHCoeffs(params->seobCoeffs, eta, a);
        ham = XLALSimIMRSpinPrecEOBHamiltonian(eta, &qCartVec, &pCartVec, &s1VecNorm, &s2VecNorm, &sKerr, &sStar,
                                               params->tortoise, params->seobCoeffs, params->hParams);
        ...
        flux = XLALInspiralPrecSpinFactorizedFlux(&polarDynamics, &cartDynamics, nqcCoeffs, omega, 0, qSph[0] * omega,
                                                  params, ham, lMax, SpinAlignedEOBversion);
        /*
        ...
        REAL8 csi = sqrt(XLALSimIMRSpinPrecEOBHamiltonianDeltaT(params->seobCoeffs, qSph[0], eta, a) *
                         XLALSimIMRSpinPrecEOBHamiltonianDeltaR(params->seobCoeffs, qSph[0], eta, a)) /
                    (qSph[0] * qSph[0] + a * a);
        ...
        // XLAL_PRINT_INFO("flux = %.16e, dEdr = %.16e, dHdpr = %.16e, dHdpr/pr =
    ...
    // if ( initr< 3.)
    // {
    //     PRINT_LOG_INFO(LOG_CRITICAL, "the initial polar r = %.8e is too small,
    //     abort.", initr); return CEV_FAILURE;
    // }

    // XLAL_PRINT_INFO("THE FINAL INITIAL CONDITIONS:\n");
    // XLAL_PRINT_INFO(" %.16e %.16e %.16e\n%.16e %.16e %.16e\n%.16e %.16e
    // %.16e\n%.16e %.16e %.16e\n", initConds->data[0], initConds->data[1],
    // initConds->data[2],
    //         initConds->data[3], initConds->data[4], initConds->data[5],
    //         initConds->data[6], initConds->data[7], initConds->data[8],
    //         initConds->data[9], initConds->data[10], initConds->data[11]);
...
INT EOBInitialConditionsPrec_Conserve(REAL8Vector *initConds,          /**<< OUTPUT, Initial dynamical variables */
                                      const REAL8 mass1,               /**<< mass 1 */
                                      const REAL8 mass2,               /**<< mass 2 */
                                      const REAL8 fMin,                /**<< Initial frequency (given) */
                                      const REAL8 e0, const REAL8 inc, /**<< Inclination */
...
{
    // 	XLAL_PRINT_INFO("Inside the XLALSimIMRSpinEOBInitialConditionsPrec
    // function!\n"); 	XLAL_PRINT_INFO(
    ...
    // XLAL_PRINT_INFO("Spherical orbit conditions here given by the
    // following:\n"); XLAL_PRINT_INFO(" x = %.16e, py = %.16e, pz = %.16e\n",
    // gsl_vector_get(finalValues, 0)/scale1,
    // gsl_vector_get(finalValues, 1)/scale2,
    // gsl_vector_get(finalValues, 2)/scale3);
...
/* Spin EOB root */
typedef struct tagSEOBRootParams_egw
{
    // REAL8          values[12]; /**<< Dynamical variables, x, y, z, px, py, pz,
    // S1x, S1y, S1z, S2x, S2y and S2z */
    SpinEOBParams *params; /**<< Spin EOB parameters -- physical, pre-computed, etc. */
 ...
 */
static int XLALFindSphericalOrbitByegw(const gsl_vector *x, /**<< Parameters requested by gsl root finder */
                                       void *params,        /**<< Spin EOB parameters */
                                       gsl_vector *f        /**<< Function values for the given parameters */
)
{
...
static int XLALFindSphericalOrbitSA(const gsl_vector *x, /**<< Parameters requested by gsl root finder */
                                    void *params,        /**<< Spin EOB parameters */
                                    gsl_vector *f        /**<< Function values for the given parameters */
)
{
...
/* Initial Condition Core function */
/* For nonprecessing case */
INT EOBInitialConditionsSA_egw(REAL8Vector *initConds, const REAL8 mass1, const REAL8 mass2, const REAL8 fMin,
                               const REAL8 ecc, const REAL8 inc, const REAL8 spin1[], const REAL8 spin2[],
                               SpinEOBParams *params)
{
    if (!initConds)
...
static REAL8 diffHamiltonianBype(const REAL8 p, const REAL8 e, const REAL8 pphi, SpinEOBParams *params)
{
    REAL8 Hp, Hm;
    REAL8 cartValues[6];
...
typedef struct tagEAnomalyParams
{
    SpinEOBParams *params; /**<< Spin EOB parameters -- physical, pre-computed, etc. */
    REAL8 omega;           /**<< Orbital frequency */
    REAL8 e0;
...
static int XLALFindSphericalOrbitSAEAnomaly(const gsl_vector *x, /**<< Parameters requested by gsl root finder */
                                            void *params,        /**<< Spin EOB parameters */
                                            gsl_vector *f        /**<< Function values for the given parameters */
)
{
...
int calc_rprpphi_from_e_anomaly(REAL8 e, REAL8 zeta, REAL8 omega, SpinEOBParams *params, REAL8 *r, REAL8 *pr,
                                REAL8 *pphi)
{
...
typedef struct tagSEOBRootParamsV2
{
    REAL8 values[12];      /**<< Dynamical variables, x, y, z, px, py, pz, S1x, S1y,
                              S1z, S2x, S2y and S2z */
    SpinEOBParams *params; /**<< Spin EOB parameters -- physical, pre-computed, etc. */
...
static int XLALFindSphericalOrbitSAWithAnomaly(const gsl_vector *x, /**<< Parameters requested by gsl root finder */
                                               void *params,        /**<< Spin EOB parameters */
                                               gsl_vector *f        /**<< Function values for the given parameters */
)
{
    ...
    rootParams->values[0] = r = gsl_vector_get(x, 0);
    pphi = gsl_vector_get(x, 1);
    rootParams->values[4] = pphi / r;
    e0 = rootParams->e0;
    // pphi   = r * py;
    // print_debug( "Values r = %.16e, py = %.16e\n", r, py );
    calculate_prDot_from_ezetapphi(rootParams->params->eta, rootParams->params->chi1, rootParams->params->chi2, e0,
                                   rootParams->sz, rootParams->cz, pphi, rootParams->omega, &prT, &prDot, &tmpr, &rDot,
                                   &tmpL);
    ...
    //         rootParams->values[0], rootParams->values[1],
    //         rootParams->values[2], rootParams->values[3],
    //         rootParams->values[4], rootParams->values[5]);
    //     print_log("ham, pphi, dHdr, dHdpr, dHdpphi = %.16e, %.16e, %.16e,
    //     %.16e, %.16e\n", ham*rootParams->params->eta, pphi, dHdr, dHdpr,
    //     dHdpphi);
    // }
    /* populate the function vector */
...
static REAL8 EOBSolvingInitialPr_from_ezeta(REAL8 pr, void *params)
{
    ...
    REAL8 ham;
    ham = EOBHamiltonian(fparams->eobpms->eta, &qCartVec, &pCartVec, fparams->eobpms->s1Vec, fparams->eobpms->s2Vec,
                         fparams->eobpms->sigmaKerr, fparams->eobpms->sigmaStar, 1, fparams->eobpms->seobCoeffs);
    // print_debug( "hamiltonian at this point is %.16e\n", ham );
    ...
    // print_debug("ham = %g\n", ham);
    REAL8 flux;
    flux = XLALInspiralSpinFactorizedFlux(&polarDynamics, &cartDynamics, eobpms->nqcCoeffs, omega0, 0, r0 * omega0,
                                          eobpms, ham, 8);
    // flux  = InspiralSpinFactorizedFlux_elip( &polarDynamics, polarData,
    // tmpdvalues, nqcCoeffs, omega, params, ham, lMax, eobVersion );
...
int calc_rpphi_from_eanomaly(REAL8 e, REAL8 anomaly, REAL8 omega, SpinEOBParams *params, REAL8 *r, REAL8 *pr,
                             REAL8 *pphi)
{
    REAL8 r0, pphi0, mT2;
    REAL8 sz, cz;
    ...
    gsl_vector_set(initValues, 1,
                   rootParams.values[0] * rootParams.values[4]); // py | pphi
    // test
    // {
    //     DEBUG_START;
    ...
    //         params->eta, params->chi1, params->chi2); print_debug("omega0,
    //         ecc, zeta, pphi0 = (%.16e, %.16e, %.16e, %.16e)\n", omega, e,
    //         anomaly, pphi0);
    //             calculate_prDot_from_ezetapphi(params->eta, params->chi1,
    //             params->chi2,
    //                 e, sin(anomaly), cos(anomaly), pphi0, &pr0, &prDot0);
    //         print_debug("pr0, prDot0 = (%.16e, %.16e)\n", pr0, prDot0);
    //     }

    //     XLALFindSphericalOrbitSAWithAnomaly(tmpVecIn, &rootParams, tmpVecOut);
    ...
    *pphi = pphi0 = gsl_vector_get(finalValues, 1);
    ...
    calculate_prDot_from_ezetapphi(params->eta, params->chi1, params->chi2, e, sz, cz, pphi0, params->omega, &pr0,
                                   &prDot0, &tmpr, &rDot0, &tmpL);
    ...
    F.params = &fparams2;
    // print_debug("tmpeq(0) = %.16e\n", EOBSolvingInitialPr_from_ezeta(0,
    // &fparams2)); print_debug("tmpeq(pr0) = %.16e\n",
    ...
    if (gslStatus != GSL_SUCCESS)
    {
        PRINT_LOG_INFO(LOG_WARNING, "cannot find initial condition for pr, now we "
                                    "try more initial setting");
        int i_try = 8;
        while (i_try > 0)
        {
    ...
    *pr = root;
    ...
    // EOBSolvingInitialPr_from_ezeta(pr0, &fparams2)); print_debug("root =
    // %.16e, eq(root) = %.16e\n", root, EOBSolvingInitialPr_from_ezeta(root,
    // &fparams2));
#else
...
REAL8
eval_by_3p_cspline_g1(REAL8 dx, REAL8 x0, REAL8 ym1, REAL8 y0, REAL8 y1, REAL8 h)
{
    REAL8 dx2;
    dx2 = dx * dx;
    return (dx * h * (4 * y0 - y1 - 3 * ym1) + 2 * dx2 * ym1 + h * h * (-2 * y0 + y1 + ym1)) / (2. * dx2);
}

REAL8
eval_by_3p_cspline_gm1(REAL8 dx, REAL8 x0, REAL8 ym1, REAL8 y0, REAL8 y1, REAL8 h)
{
    REAL8 dx2;
    dx2 = dx * dx;
    return (2 * dx2 * y0 + dx * h * (y1 - ym1) + h * h * (-2 * y0 + y1 + ym1)) / (2. * dx2);
}

/* For nonprecessing case */
INT EOBInitialConditionsSA_e_anomaly(REAL8Vector *initConds, const REAL8 mass1, const REAL8 mass2, const REAL8 fMin,
                                     const REAL8 ecc, const REAL8 anomaly, const REAL8 inc, const REAL8 spin1[],
                                     const REAL8 spin2[], SpinEOBParams *params)
{
    if (!initConds)
        return CEV_FAILURE;
    static const int lMax = 8;
...
#else
    if (fabs(anomaly) < 1e-2)
    {
        REAL8 step_size = 0.015;
        REAL8 zeta_m1, zeta_0, zeta_1;
        REAL8 pr_m1, pr_1, pr_0;
        REAL8 r_m1, r_1, r_0;
        REAL8 L_m1, L_1, L_0;
        if (anomaly < 0)
        {
        ...
        calc_rpphi_from_eanomaly(ecc, zeta_m1, omega, params, &r_m1, &pr_m1, &L_m1);
        calc_rpphi_from_eanomaly(ecc, zeta_1, omega, params, &r_1, &pr_1, &L_1);
        calc_rpphi_from_eanomaly(ecc, zeta_0, omega, params, &r_0, &pr_0, &L_0);
        REAL8 h;
        if (anomaly < zeta_0)
        {
            h = anomaly - zeta_m1;
            qCart[0] = eval_by_3p_cspline_gm1(step_size, zeta_0, r_m1, r_0, r_1, h);
            pCart[0] = eval_by_3p_cspline_gm1(step_size, zeta_0, pr_m1, pr_0, pr_1, h);
            pCart[1] = eval_by_3p_cspline_gm1(step_size, zeta_0, L_m1, L_0, L_1, h);
        }
        else
        {
            h = anomaly - zeta_0;
            qCart[0] = eval_by_3p_cspline_g1(step_size, zeta_0, r_m1, r_0, r_1, h);
            pCart[0] = eval_by_3p_cspline_g1(step_size, zeta_0, pr_m1, pr_0, pr_1, h);
            pCart[1] = eval_by_3p_cspline_g1(step_size, zeta_0, L_m1, L_0, L_1, h);
        }
        // print_debug("L = (%.16e, %.16e, %.16e)\n", L_m1, L_0, L_1);
    }
    else if (fabs(anomaly - CST_PI) < 1e-2)
    {
        REAL8 step_size = 0.015;
        REAL8 zeta_m1, zeta_0, zeta_1;
        REAL8 pr_m1, pr_1, pr_0;
    ...
    // if ( tmpTortoise )
    // {
    //     REAL8 r = sqrt(qCart[0]*qCart[0] + qCart[1]*qCart[1] +
    //     qCart[2]*qCart[2] ); REAL8 deltaR =
    //     XLALSimIMRSpinEOBHamiltonianDeltaR( params->seobCoeffs, r, eta, a );
    //     REAL8 deltaT = XLALSimIMRSpinEOBHamiltonianDeltaT( params->seobCoeffs,
    //     r, eta, a ); REAL8 csi    = sqrt( deltaT * deltaR )/(r*r + a*a);

    //     REAL8 pr = (qCart[0]*pCart[0] + qCart[1]*pCart[1] +
    //     qCart[2]*pCart[2])/r; params->tortoise = tmpTortoise;
...
REAL8 calculate_rDot_from_prPrec(REAL8 eta, REAL8 c1n, REAL8 c2n, REAL8 c1c1, REAL8 c1c2, REAL8 c2c2, REAL8 r,
                                 REAL8 prT, REAL8 L);
...
static REAL8 EOBSolvingInitialPr_from_ezetaPrec(REAL8 pr, void *params)
{
    ...
    // EOBCalculateSigmaKerr(&sKerr, &s1VecNorm, &s2VecNorm);
    // EOBCalculateSigmaStar(&sStar, eobpms->m1, eobpms->m2, &s1VecNorm,
    // &s2VecNorm);
    ...
    // mass1, mass2, eta, /* a */ 0.0, chiS, chiA);
    // XLALSimIMRCalculateSpinPrecEOBHCoeffs(params->seobCoeffs, eta, a);
    REAL8 ham = XLALSimIMRSpinPrecEOBHamiltonian(eobpms->eta, &qCartVec, &pCartVec, &s1VecNorm, &s2VecNorm, &sKerr,
                                                 &sStar, eobpms->tortoise, eobpms->seobCoeffs, eobpms->hParams);
    // print_debug("here 4\n");
    ...
    //     polarDynamics.data[0], polarDynamics.data[1], polarDynamics.data[2],
    //     polarDynamics.data[3]);
    // print_log("cartDynamics = (%.16e, %.16e, %.16e, %.16e)\n",
    //     cartDynamics.data[0], cartDynamics.data[1], cartDynamics.data[2],
    //     cartDynamics.data[3]);

    REAL8 omega = fparams->omega;

    flux = XLALInspiralPrecSpinFactorizedFlux(&polarDynamics, &cartDynamics, eobpms->nqcCoeffs, omega, 0,
                                              qSph[0] * omega, eobpms, ham, 8, 4);
    ...
    REAL8 csi = sqrt(XLALSimIMRSpinPrecEOBHamiltonianDeltaT(eobpms->seobCoeffs, qSph[0], eobpms->eta, a) *
                     XLALSimIMRSpinPrecEOBHamiltonianDeltaR(eobpms->seobCoeffs, qSph[0], eobpms->eta, a)) /
                (qSph[0] * qSph[0] + a * a);
    ...
    rDot0 = calculate_rDot_from_prPrec(eobpms->eta, fparams->c1n, fparams->c2n, fparams->c1c1, fparams->c1c2,
                                       fparams->c2c2, fparams->r0, pr / csi, fparams->L);
    rDot = rDot0 + rDot1;
    eq = dHdpr - (rDot0 + rDot1);
// #if 0
// #endif
...
INT Calculate_rpypz_from_ezeta(SEOBRootParams *rootParams, REAL8 e0, REAL8 zeta, REAL8 *ret_r, REAL8 *ret_py,
                               REAL8 *ret_pz)
{
    int i;
    INT gslStatus;
    ...
    // INT cntGslNoProgress = 0, MAXcntGslNoProgress = 50;
    REAL8 multFacGslNoProgress = 3. / 5.;
    REAL8 v0 = cbrt(rootParams->omega);
    REAL8 tmpS1[3], tmpS2[3];
    memcpy(tmpS1, rootParams->values + 6, sizeof(tmpS1));
    memcpy(tmpS2, rootParams->values + 9, sizeof(tmpS2));
    // const int	maxIter = 2000;
    const int maxIter = 10000;
    const gsl_multiroot_fsolver_type *T = gsl_multiroot_fsolver_hybrids;
    gsl_multiroot_fsolver *rootSolver = NULL;
    ...
    *ret_r = sqrt(gsl_vector_get(finalValues, 0) * gsl_vector_get(finalValues, 0) + 36.0);
    *ret_py = gsl_vector_get(finalValues, 1) / scale2;
    *ret_pz = gsl_vector_get(finalValues, 2) / scale3;
...
/* For precessing case */
INT EOBInitialConditionsPrec_e_anomaly(REAL8Vector *initConds, const REAL8 mass1, const REAL8 mass2, const REAL8 fMin,
                                       const REAL8 e0, const REAL8 zeta, const REAL8 xi, const REAL8 inc,
                                       const REAL8 spin1[], const REAL8 spin2[], SpinEOBParams *params)
{
    static const int lMax = 8;
    ...
    // rootParams.c1n = tmpS1[0] / m1sq;
    // rootParams.c1l = tmpS1[1] / m1sq;
    // rootParams.c1e = tmpS1[2] / m1sq;
    // rootParams.c2n = tmpS2[0] / m2sq;
    // rootParams.c2l = tmpS2[1] / m2sq;
    // rootParams.c2e = tmpS2[2] / m2sq;
    rootParams.c1c1 =
        rootParams.c1n * rootParams.c1n + rootParams.c1l * rootParams.c1l + rootParams.c1e * rootParams.c1e;
    rootParams.c1c2 =
        rootParams.c1n * rootParams.c2n + rootParams.c1l * rootParams.c2l + rootParams.c1e * rootParams.c2e;
    rootParams.c2c2 =
        rootParams.c2n * rootParams.c2n + rootParams.c2l * rootParams.c2l + rootParams.c2e * rootParams.c2e;
    omega = CST_PI * mTotal * CST_MTSUN_SI * fMin;
    v0 = cbrt(omega);
    ...
    // qCart[0] = sqrt(gsl_vector_get(finalValues, 0)*gsl_vector_get(finalValues,
    // 0)+36.0); pCart[1] = gsl_vector_get(finalValues, 1)/scale2; pCart[2] =
    // gsl_vector_get(finalValues, 2)/scale3;
    if (Calculate_rpypz_from_ezeta(&rootParams, e0, zeta, &(qCart[0]), &(pCart[1]), &(pCart[2])) != CEV_SUCCESS)
    {
        gsl_matrix_free(rotMatrix);
        gsl_matrix_free(invMatrix);
        return CEV_FAILURE;
    ...
    calculate_prDot_from_ezetapphiPrec(params->eta, rootParams.c1n, rootParams.c1l, rootParams.c1e, rootParams.c2n,
                                       rootParams.c2l, rootParams.c2e, rootParams.c1c1, rootParams.c1c2,
                                       rootParams.c2c2, rootParams.e0, rootParams.sz, rootParams.cz, tmp_py, tmp_pz,
                                       tmp_L, rootParams.omega, &tmp_px, &tmp_prDot, &tmp_rDot, &tmp_r, &tmp_LPN);
    pCart[0] = tmp_px;
    // if (CODE_VERSION == 2)
    // {
    //     qCart[0] /= 1. + e0;
    ...
    else
    {
        ...
        do
        {
            ...
            if (gslStatus != GSL_SUCCESS)
            {
                // PRINT_LOG_INFO(LOG_WARNING, "cannot find initial condition for
                // pr");
                print_debug("cannot find initial condition for pr0\n");
                gsl_root_fsolver_free(rootSolver1D);
                res_pr = tmp_px;
...
/**------------------------------------------------------------------------------------------------------------**/
/** **/
/*                  2024.02.28: New version to calculate initial condition with
 * e0 and anomaly                  */
/** **/
/**------------------------------------------------------------------------------------------------------------**/

static void calculate_prT_from_ezetapphi(REAL8 eta, REAL8 chi1, REAL8 chi2, REAL8 e, REAL8 sz, REAL8 cz, REAL8 pf,
                                         REAL8 Omg, REAL8 *ret_prT)
{
    ...
    pr4 = (e * (-18 - 3 * chi12 - 3 * chi22 - 3 * chi12 * dm + 3 * chi22 * dm + 23 * e2 + 2 * chi12 * e2 +
                ...
                4 * chi1 * chi2 * e2 * eta - 6 * chi22 * e2 * eta - 2 * chi12 * dm * e2 * eta +
                2 * chi22 * dm * e2 * eta - 6 * chi12 * eta2 - 12 * chi1 * chi2 * eta2 - 6 * chi22 * eta2 +
                2 * chi12 * e2 * eta2 + 4 * chi1 * chi2 * e2 * eta2 + 2 * chi22 * e2 * eta2 +
                cz2 * e2 *
                    (-1 + chi22 * (-1 + dm + 4 * eta - 2 * dm * eta - 2 * eta2) - 4 * chi1 * chi2 * eta2 -
                     chi12 * (1 + dm - 4 * eta - 2 * dm * eta + 2 * eta2)) -
                2 * cz * e *
                    (18 + 4 * e2 - 2 * eta + 2 * chi1 * chi2 * eta * (1 + 5 * eta) +
                     chi12 * (3 + dm * (3 - 5 * eta) - 11 * eta + 5 * eta2) +
                     chi22 * (3 - 3 * dm - 11 * eta + 5 * dm * eta + 5 * eta2)))) /
          (4. * pf5);
...
static int estimate_eanomaly_by_sim_orbit(SpinEOBParams *seobParams, REAL8 r, REAL8 prT, REAL8 pphi, REAL8 *rec_e0,
                                          REAL8 *rec_sz, REAL8 *rec_cz)
{
    ...
    // status = SEOBComputeExtendedSEOBdynamics_Conserve(&seobdynamicsAdaS,
    // dynamics, retLenPlus, seobParams); if (status != CEV_SUCCESS) {failed = 1;
    // goto QUIT;}
...
static int XLALFindSphericalOrbitSAWithAnomaly_v240228(
...
)
{
    ...
    e0 = rootParams->e0;
    // pphi   = r * py;
    calculate_prT_from_ezetapphi(rootParams->params->eta, rootParams->params->chi1, rootParams->params->chi2, e0,
                                 rootParams->sz, rootParams->cz, pphi, rootParams->omega, &prT);
    ...
    // REAL8 tmprDotMax = e0 / tmpL;
    // REAL8 tmpprDotMax = e0*(1.+e0)*(1.+e0)/(tmpL*tmpL*tmpL*tmpL);
    // gsl_vector_set( f, 0, ((dHdr + prDot) + 1e-2*(r - tmpr) + (dHdpr - rDot) +
    // 1e-2*(pphi - tmpL))); gsl_vector_set( f, 0, ((dHdr + prDot)));
...
int calc_rpphi_from_eanomaly_v240228(REAL8 e, REAL8 anomaly, REAL8 omega, SpinEOBParams *params, REAL8 *r, REAL8 *pr,
                                     REAL8 *pphi)
{
    REAL8 r0, pphi0, mT2;
    REAL8 sz, cz;
    ...
    //     XLALFindSphericalOrbitSAWithAnomaly_v240228(tmpVecIn, &rootParams,
    //     tmpVecOut); gsl_vector_free(tmpVecIn); gsl_vector_free(tmpVecOut);
    //     DEBUG_END;
    // }
...
/* For nonprecessing case */
INT EOBInitialConditionsSA_e_anomaly_v240228(REAL8Vector *initConds, const REAL8 mass1, const REAL8 mass2,
                                             const REAL8 fMin, const REAL8 ecc, const REAL8 anomaly, const REAL8 inc,
                                             const REAL8 spin1[], const REAL8 spin2[], SpinEOBParams *params)
{
    if (!initConds)
        return CEV_FAILURE;
    ...
    // calc_rpphi_from_e(root, omega, params, &(qCart[0]), &(pCart[1]));
    calc_rpphi_from_eanomaly(ecc, anomaly, omega, params, &(qCart[0]), &(pCart[0]), &(pCart[1]));
    // if (calc_rpphi_from_eanomaly_v240228(ecc, anomaly, omega, params,
    // &(qCart[0]), &(pCart[0]), &(pCart[1])) != CEV_SUCCESS)
    //     calc_rpphi_from_eanomaly(ecc, anomaly, omega, params, &(qCart[0]),
    //     &(pCart[0]), &(pCart[1]));
    REAL8 rec_e0, rec_sz, rec_cz;
