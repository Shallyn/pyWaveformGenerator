...
INT evolve(REAL8 m1, REAL8 m2, REAL8 s1x, REAL8 s1y, REAL8 s1z, REAL8 s2x, REAL8 s2y, REAL8 s2z, REAL8 phi0,
           REAL8 distance, REAL8 ecc, REAL8 f_min, REAL8 INdeltaT, REAL8 inc, INT is_only22, HyperParams *hparams,
           REAL8TimeSeries **hPlusOut, REAL8TimeSeries **hCrossOut, SEOBCoreOutputs *all);

INT evolve_adaptive(REAL8 m1, REAL8 m2, REAL8 s1x, REAL8 s1y, REAL8 s1z, REAL8 s2x, REAL8 s2y, REAL8 s2z, REAL8 phi0,
                    REAL8 distance, REAL8 ecc, REAL8 f_min, REAL8 INdeltaT, REAL8 inc, INT is_only22,
                    HyperParams *hparams, REAL8TimeSeries **hPlusOut, REAL8TimeSeries **hCrossOut,
                    SEOBCoreOutputs *all);

INT evolve_conserv(REAL8 m1, REAL8 m2, REAL8 s1x, REAL8 s1y, REAL8 s1z, REAL8 s2x, REAL8 s2y, REAL8 s2z, REAL8 phi0,
                   REAL8 distance, REAL8 ecc, REAL8 f_min, REAL8 INdeltaT, REAL8 inc, HyperParams *hparams,
                   SEOBCoreOutputs *all);

INT evolve_SA(REAL8 m1, REAL8 m2, REAL8 s1z, REAL8 s2z, REAL8 ecc, REAL8 f_min, REAL8 INdeltaT, HyperParams *hparams,
              REAL8Vector **tRetVec, SphHarmListCAmpPhaseSequence **hlm, int is_noringdown, int is_dyn_debug,
...
INT choose_debug(INT debug_id, REAL8 m1, REAL8 m2, REAL8 s1x, REAL8 s1y, REAL8 s1z, REAL8 s2x, REAL8 s2y, REAL8 s2z,
                 REAL8 phi0, REAL8 distance, REAL8 ecc, REAL8 f_min, REAL8 INdeltaT, REAL8 inc, HyperParams *hparams);
...
void print_SphHarmListCAmpPhaseSequenceAndDynamics(REAL8Vector *tVec, SphHarmListCAmpPhaseSequence *hLM,
                                                   SEOBSAdynamics *dyn_debug);
...
INT usage(const CHAR *program)
{
    ...
    print_err("\t-D DISTANCE, --distance=DISTANCE  \n\t\tLum distance in Mpc of "
              "the source, if > 0[100]\n");

    print_err("\t-d D, --d=D  \n\t\tInitial seperation, if > 0, use given "
              "initial condition[%g]\n",
              -1);
    print_err("\t-r PR, --pr=PR  \n\t\tInitial radius momentum [%g]\n", 0);
    print_err("\t-p PPHI, --pphi=PPHI  \n\t\tinitial phi augular momentum, if > "
              "0, use given initial condition[%g]\n",
              -1);
    print_err("\t-t PTHETA, --ptheta=PTHETA  \n\t\tInitial theta augular "
              "momentum, if > 0, use given initial condition[%g]\n",
              -1);

    print_err("\t-I RISCO, --risco=RISCO  \n\t\tA hyper parameter that controls "
              "stop condition when (e0>0)[%g]\n",
              6.);
    ...
    print_err("\t\t10l+|m|:\t\toutput (l,|m|) mode, print time[M], hlm_real, "
              "hlm_imag\n");
    print_err("\t\t-1:\t\toutput all modes, print time[M], 22, 21, 33, 44, 55\n");
    print_err("\t\t-2:\t\toutput dynamics, print time[M], x, y, z, vx, vy, vz, "
              "px, py, pz\n");
...
InputParams parseargs(INT argc, CHAR **argv, HyperParams *hparams, CtrlParams *ctpms)
{
    ...
    OPTION long_options[] = {{"help", opt_no_argument, 0, 'h'},
                             ...
                             {0, 0, 0, 0}};
    CHAR args[] = "h:j:g:A:v:P:m:M:x:y:z:X:Y:Z:e:s:R:f:i:u:D:d:r:p:t:I:k:<:>:T:"
                  "B:C:E:l:o:U";
    while (1)
    {
        INT option_index = 0;
...
INT main(INT argc, CHAR **argv)
{
    ...
    PRINT_LOG_INFO(LOG_DEBUG,
                   "CMD:\n%s --m1=%g --m2=%g --spin1x=%g --spin1y=%g --spin1z=%g "
                   "--spin2x=%g --spin2y=%g --spin2z=%g --eccentricity=%g "
                   "--delta-t=%g --f-min=%g --inclination=%g --phiRef=%g\n",
                   argv[0], p.m1, p.m2, p.s1x, p.s1y, p.s1z, p.s2x, p.s2y, p.s2z, p.ecc, p.deltaT, p.f_min,
                   p.inc * 180 / CST_PI, p.phiRef * 180 / CST_PI);
    PRINT_LOG_INFO(LOG_INFO, "CodeVersionFlag = %d\n", CODE_VERSION);
    // print_debug("prec flag = %d\n", ctpms.level);
    if (p.debug_id)
    {
        choose_debug(p.debug_id, p.m1, p.m2, p.s1x, p.s1y, p.s1z, p.s2x, p.s2y, p.s2z, p.phiRef, p.distance, p.ecc,
                     p.f_min, p.deltaT, p.inc, &hparams);
        return 0;
    }
    if (get_PrecFlag() != 0)
    {
        ...
        status = evolve_prec(p.m1, p.m2, p.s1x, p.s1y, p.s1z, p.s2x, p.s2y, p.s2z, p.phiRef, p.distance, p.ecc, p.f_min,
    ...
    else if (CONSERVE_FLAG != 0)
    {
        ...
        status = evolve_conserv(p.m1, p.m2, p.s1x, p.s1y, p.s1z, p.s2x, p.s2y, p.s2z, p.phiRef, p.distance, p.ecc,
                                p.f_min, p.deltaT, p.inc, &hparams, All);
        if (status != CEV_SUCCESS)
            goto EXIT;
        // this function in myFileIO.c/.h is already commented out
    ...
    else if (sqrt(p.s1x * p.s1x + p.s1y * p.s1y) < 1e-5 && sqrt(p.s2x * p.s2x + p.s2y * p.s2y) < 1e-5)
    {
        ...
        status = evolve_SA(p.m1, p.m2, p.s1z, p.s2z, p.ecc, p.f_min, p.deltaT, &hparams, &tVec, &hLM, p.is_noringdown,
                           p.mode == -2, &dyn_debug);
        if (status != CEV_SUCCESS)
            goto EXIT;
        if (p.mode == -2)
    ...
    {
        if (p.is_constsp)
            status = evolve(p.m1, p.m2, p.s1x, p.s1y, p.s1z, p.s2x, p.s2y, p.s2z, p.phiRef, p.distance, p.ecc, p.f_min,
                            p.deltaT, p.inc, p.is_only22, &hparams, &hplus, &hcross, All);
        else
            status = evolve_adaptive(p.m1, p.m2, p.s1x, p.s1y, p.s1z, p.s2x, p.s2y, p.s2z, p.phiRef, p.distance, p.ecc,
                                     p.f_min, p.deltaT, p.inc, p.is_only22, &hparams, &hplus, &hcross, All);
        if (status != CEV_SUCCESS)
            goto EXIT;
        // Output
        switch (p.mode)
...
void print_SphHarmListCAmpPhaseSequence_debug(SphHarmListCAmpPhaseSequence *hLM)
{
    ...
    print_out("#time\t#h22r\t#h22i\t#h21r\t#h21i\t#h33r\t#h33i\t#h44r\t#"
              "h44i\th55r\th55i\n");
    for (i = 0; i < length; i++)
    {
        print_out(
            ...
            ".18e\t%.18e\n",
            h22->xdata->data[i], h22->camp_real->data[i] * cos(h22->phase->data[i]),
            h22->camp_real->data[i] * sin(h22->phase->data[i]), h21->camp_real->data[i] * cos(h21->phase->data[i]),
            h21->camp_real->data[i] * sin(h21->phase->data[i]), h33->camp_real->data[i] * cos(h33->phase->data[i]),
            h33->camp_real->data[i] * sin(h33->phase->data[i]), h44->camp_real->data[i] * cos(h44->phase->data[i]),
            h44->camp_real->data[i] * sin(h44->phase->data[i]), h55->camp_real->data[i] * cos(h55->phase->data[i]),
            h55->camp_real->data[i] * sin(h55->phase->data[i]));
    }
    return;
}
...
void print_SphHarmTimeSeries(SphHarmTimeSeries *hlm, INT mode)
{
    ...
    if (mode < 0)
    {
        ...
        print_out("#time\t#h22r\t#h22i\t#h21r\t#h21i\t#h33r\t#h33i\t#h44r\t#h44i\t#"
                  "h55r\t#h55i\n");
        for (i = 0; i < length; i++)
        {
            print_out("%.18e\t%.18e\t%.18e\t%.18e\t%.18e\t%.18e\t%.18e\t%.18e\t%."
                      "18e\t%.18e\t%.18e\n",
                      i * deltaT - hlm->tAttach, creal(h22->data->data[i]), cimag(h22->data->data[i]),
                      creal(h21->data->data[i]), cimag(h21->data->data[i]), creal(h33->data->data[i]),
                      cimag(h33->data->data[i]), creal(h44->data->data[i]), cimag(h44->data->data[i]),
    ...
    for (i = 0; i < length; i++)
    {
        print_out("%.18e\t%.18e\t%.18e\n", i * deltaT, creal(hIlmTS->data->data[i]), cimag(hIlmTS->data->data[i]));
    }
    return;
}
...
void print_SEOBdynamics(SEOBdynamics *dyn)
{
    INT i, length;
    length = dyn->length;
    print_out("#time\t#x\t#y\t#z\t#vx\t#vy\t#vz\t#px\t#py\t#pz\t#ham\t#flux\t#"
              "Fr\t#Ff\t#polarprDot\n");
    for (i = 0; i < length; i++)
    {
        print_out("%.18e\t%.18e\t%.18e\t%.18e\t%.18e\t%.18e\t%.18e\t%.18e\t%.18e\t%"
                  ".18e\t%.18e\t%.18e\t%.18e\t%.18e\t%.18e\t%.18e\t%.18e\n",
                  dyn->tVec[i], dyn->posVecx[i], dyn->posVecy[i], dyn->posVecz[i], dyn->momVecx[i], dyn->momVecy[i],
                  dyn->momVecz[i], dyn->s1Vecx[i], dyn->s1Vecy[i], dyn->s1Vecz[i], dyn->s2Vecx[i], dyn->s2Vecy[i],
                  dyn->s2Vecz[i], dyn->omegaVec[i], dyn->hamVec[i], dyn->fluxVec[i], dyn->polarprDotVec[i]);
    }
    return;
}
...
void print_SEOBPrecdynamics(SEOBPrecdynamics *dyn)
{
    INT i, length;
    length = dyn->length;
    print_out("#time\t#x\t#y\t#z\t#pTx\t#pTy\t#"
              "pTz\ts1x\ts1y\ts1z\ts2x\ts2y\ts2z\tomega\tHam\tflux\tprTDot\n");
    for (i = 0; i < length; i++)
    {
        print_out("%.18e\t%.18e\t%.18e\t%.18e\t%.18e\t%.18e\t%.18e\t%.18e\t%.18e\t%"
                  ".18e\t%.18e\t%.18e\t%.18e\t%.18e\t%.18e\t%.18e\t%.18e\n",
                  dyn->tVec[i], dyn->posVecx[i], dyn->posVecy[i], dyn->posVecz[i], dyn->momTVecx[i], dyn->momTVecy[i],
                  dyn->momTVecz[i], dyn->s1Vecx[i], dyn->s1Vecy[i], dyn->s1Vecz[i], dyn->s2Vecx[i], dyn->s2Vecy[i],
                  dyn->s2Vecz[i], dyn->omegaVec[i], dyn->HamVec[i], dyn->fluxVec[i], dyn->prTDotVec[i]);
    }
    return;
}
...
void print_SEOBPrecWaveform(SphHarmTimeSeries *hLM)
{
    ...
    for (i = 0; i < length; i++)
    {
        print_out("%.18e\t%.18e\t%.18e\t%.18e\t%.18e\t%.18e\t%.18e\t%.18e\t%.18e\t%"
                  ".18e\t%.18e\n",
                  deltaT * i - hLM->tAttach, creal(h22->data->data[i]), cimag(h22->data->data[i]),
                  creal(h21->data->data[i]), cimag(h21->data->data[i]), creal(h33->data->data[i]),
                  cimag(h33->data->data[i]), creal(h44->data->data[i]), cimag(h44->data->data[i]),
                  creal(h55->data->data[i]), cimag(h55->data->data[i]));
    }
...
void print_waveform(REAL8TimeSeries *hp, REAL8TimeSeries *hc)
{
    if (hp->data->length != hc->data->length || hp->deltaT != hc->deltaT)
    {
    ...
    for (i = 0; i < length; i++)
    {
        print_out("%.18e\t%.18e\t%.18e\n", t0 + i * deltaT, hp->data->data[i], hc->data->data[i]);
    }
    return;
}
                                                   ...
                                                   SEOBSAdynamics *dyn_debug)
{
    ...
    for (i = 0; i < length; i++)
    {
        ...
        print_out(
            "%.18e\t%.18e\t%.18e\t%.18e\t%.18e\t%.18e\t%.18e\t%.18e\t%.18e\t%"
            ".18e\t%.18e\t%.18e\t%.18e\t%.18e\n",
            tVec->data[i], h22->camp_real->data[i] * cos(h22->phase->data[i]),
            h22->camp_real->data[i] * sin(h22->phase->data[i]), h21->camp_real->data[i] * cos(h21->phase->data[i]),
            h21->camp_real->data[i] * sin(h21->phase->data[i]), h33->camp_real->data[i] * cos(h33->phase->data[i]),
            h33->camp_real->data[i] * sin(h33->phase->data[i]), h44->camp_real->data[i] * cos(h44->phase->data[i]),
            h44->camp_real->data[i] * sin(h44->phase->data[i]), h55->camp_real->data[i] * cos(h55->phase->data[i]),
            h55->camp_real->data[i] * sin(h55->phase->data[i]), r, dr, omega);
    }
    return;
}
