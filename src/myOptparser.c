...
static INT _getopt_internal(INT argc, CHAR *const *argv, const CHAR *optstring, const OPTION *longopts, INT *longind,
                            INT long_only)
{
    INT print_errors = 1;
    if (optstring[0] == ':')
...
#define NONOPTION_P (argv[EXT_optind][0] != '-' || argv[EXT_optind][1] == '\0')
    ...
    if (nextchar == NULL || *nextchar == '\0')
    {
         ...
         */
        nextchar = (argv[EXT_optind] + 1 + (longopts != NULL && argv[EXT_optind][1] == '-'));
    }
    ...
    if (longopts != NULL && (argv[EXT_optind][1] == '-' ||
                             (long_only && (argv[EXT_optind][2] || !my_index(optstring, argv[EXT_optind][1])))))
    {
        CHAR *nameend;
        const OPTION *p;
                    ...
                    if (print_errors)
                    {
                        if (argv[EXT_optind - 1][1] == '-')
                            ...
                            fprintf(stderr, "%s: option '%c%s' doesn't allow an argument\n", argv[0],
                                    argv[EXT_optind - 1][0], pfound->name);
                    }
                    nextchar += strlen(nextchar);
        ...
        if (!long_only || argv[EXT_optind][1] == '-' || my_index(optstring, *nextchar) == NULL)
        {
            union {
                const CHAR *cs;
                CHAR *c;
