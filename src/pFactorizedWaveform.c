...
/**
 * Function to calculate associated Legendre function used by the spherical
 * harmonics function
 */
...
/**
 * Function to calculate the numerical prefix in the Newtonian amplitude. Eqs.
 * 5
 * - 7.
 */
static int CalculateThisMultipolePrefix(COMPLEX16 *prefix, /**<< OUTPUT, Prefix value */
...
)
{
     ...
     * For odd m, c is proportional to dM = m1-m2. In the equal-mass case, c = dM
     * = 0. In the equal-mass unequal-spin case, however, when spins are
     * different, the odd m term is generally not zero. In this case, c can be
     * written as c0 * dM, while spins terms in PN expansion may take the form
     * chiA/dM. Although the dM's cancel analytically, we can not implement c and
     * chiA/dM with the possibility of dM -> 0. Therefore, for this case, we give
     * numerical values of c0 for relevant modes, and c0 is calculated as c / dM
     * in the limit of dM -> 0. Consistently, for this case, we implement chiA
     * instead of chiA/dM in LALSimIMRSpinEOBFactorizedWaveform.c.
     */
    if (m1 != m2 || sign == 1)
    {
    ...
    /* Eqs 5 and 6. Dependent on the value of epsilon (parity), we get different
     * n
     */
    if (epsilon == 0)
    {
...
static int XLALScalarSphHarmThetaPiBy2(COMPLEX16 *y, /**<< OUTPUT, Ylm(0,phi) */
                                       INT l,        /**<< Mode l */
                                       INT m,        /**<< Mode m */
...
static int XLALAbsScalarSphHarmThetaPiBy2(COMPLEX16 *y, /**<< OUTPUT, Ylm(0,phi) */
                                          INT l,        /**<< Mode l */
                                          INT m         /**<< Mode m */
)
{
 ...
 * waveform. The coefficients are pre-computed and stored in the SpinEOBParams
 * structure. Appendix of the paper, and papers DIN (PRD 79, 064004 (2009)
 * [arXiv:0811.2069]) and PBFRT (PRD 83, 064003 (2011)). Concerning SEOBNRv4
 * see also https://dcc.ligo.org/T1600383
 */
                                                 ...
                                                 const REAL8 chiS, const REAL8 chiA)
{
    ...
    {
        coeffs->delta22vh6 = -4. / 3. * (dM * chiA + chiS * (1 - 2 * eta)) + (428. * CST_PI) / 105.;
    }
    ...
    {
    case 1:
        coeffs->rho22v4 = -20555. / 10584. + 0.5 * a2 - (33025. * eta) / 21168. + (19583. * eta2) / 42336.;
        break;
    case 2:
    case 4:
        coeffs->rho22v4 = -20555. / 10584. + 0.5 * (chiS + chiA * dM) * (chiS + chiA * dM) - (33025. * eta) / 21168. +
                          (19583. * eta2) / 42336.;
        /* If spin-induced quadrupole coefficients kappa_1,2 are not 1, include
         * the leading-order correction at 2PN in rho22  */
        /* Relevant for BNS - TEOBv4 */
        // if((params->seobCoeffs->tidal1->quadparam - 1.) != 0. ||
        // (params->seobCoeffs->tidal2->quadparam - 1.) != 0.)
        // {
        //     kappaS = 0.5 * (params->seobCoeffs->tidal1->quadparam +
        //     params->seobCoeffs->tidal2->quadparam); kappaA = 0.5 *
        //     (params->seobCoeffs->tidal1->quadparam -
        //     params->seobCoeffs->tidal2->quadparam); coeffs->rho22v4 +=
        //     chiA*chiA*(0.5*dM*kappaA - kappaS*eta + 0.5*kappaS + eta - 0.5) +
        //     chiA*chiS*(dM*kappaS - dM - 2*kappaA*eta + kappaA) +
        //     chiS*chiS*(0.5*dM*kappaA - kappaS*eta + 0.5*kappaS + eta - 0.5);
        // }
        break;
    default:
        PRINT_LOG_INFO(LOG_CRITICAL, "SpinAlignedEOBversion = %u is wrong, must be 1 or 2 or 4!",
    ...
    {
        coeffs->rho22v5 =
            (-34. / 21. + 49. * eta / 18. + 209. * eta2 / 126.) * chiS + (-34. / 21. - 19. * eta / 42.) * dM * chiA;
    }
    ...
    {
        coeffs->rho22v7 =
            a3 / 3. + chiA * dM * (18733. / 15876. + (50140. * eta) / 3969. + (97865. * eta2) / 63504.) +
    ...
    case 1:
        coeffs->rho22v8 =
            eta * (-5.6 - 117.6 * eta) - 387216563023. / 160190110080. + (18353. * a2) / 21168. - a2 * a2 / 8.;
        break;
    case 2:
    case 4:
    ...
    /*printf( "v2 = %.16e, v3 = %.16e, v4 = %.16e, v5 = %.16e\n"
        "v6 = %.16e, v6l = %.16e v7 = %.16e v8 = %.16e, v8l = %.16e v10 = %.16e
       v10l = %.16e\n", coeffs->rho22v2, coeffs->rho22v3, coeffs->rho22v4,
       coeffs->rho22v5, coeffs->rho22v6, coeffs->rho22v6l, coeffs->rho22v7,
       coeffs->rho22v8, coeffs->rho22v8l, coeffs->rho22v10, coeffs->rho22v10l );
     */

    /*RC the HM model does not include spinning test mass terms in the
     * higher-order modes */
    if (use_hm)
    {
        a = 0;
    ...
    // RC: The delta coefficient before were put inside the if(dM2). This is
    // wrong. it didn't effect the models before because this function is used
    // only to calculate the 22 mode and the flux of the other modes, but for the
    // flux you only need |h_lm|. This error was present in all the modes, and
    // now is fixed. Is not a problem for the 22 mode because there is no if(dM2)
    // for the 22
    ...
    if (dM2)
    {
        ...
        coeffs->rho21v1 = 0.0;
        // coeffs->rho21v2   = -59./56 -
        // (9.*chiAPlusChiSdM*chiAPlusChiSdM)/(32.*dM2) + (23.*eta)/84.;
        switch (SpinAlignedEOBversion)
        {
        case 1:
        ...
        coeffs->rho21v7 = (-3859374457. * a) / 1159065600. - (55169. * a3) / 16384. + (18603. * a2 * a3) / 65536. -
                          (72171. * a2 * a2 * a3) / 262144.;
        ...
        case 4:
            ...
            if (use_hm)
            {
                // RC: This terms are in Eq.A11 in
                // https://journals.aps.org/prd/abstract/10.1103/PhysRevD.98.084028
                coeffs->f21v4 =
                    (-3. - 2. * eta) * chiA2 + (-3. + eta / 2.) * chiS2 + (-6. + 21. * eta / 2.) * chiS * chiA / dM;
                coeffs->f21v5 =
                    (3. / 4. - 3. * eta) * chiA3 / dM +
                    (-81. / 16. + 1709. * eta / 1008. + 613. * eta2 / 1008. + (9. / 4. - 3 * eta) * chiA2) * chiS +
                    3. / 4. * chiS3 +
                    (-81. / 16. - 703. * eta2 / 112. + 8797. * eta / 1008. + (9. / 4. - 6. * eta) * chiS2) * chiA / dM;
                coeffs->f21v6 = (4163. / 252. - 9287. * eta / 1008. - 85. * eta2 / 112.) * chiA2 +
                                (4163. / 252. - 2633. * eta / 1008. + 461. * eta2 / 1008.) * chiS2 +
                                (4163. / 126. - 1636. * eta / 21. + 1088. * eta2 / 63.) * chiS * chiA / dM;
                coeffs->f21v7c = 0; // RC: this is the calibration parameter
    ...
    else
    {
        ...
        case 4:
            ...
            if (use_hm)
            {
                ...
                coeffs->f21v5 =
                    (3. / 4. - 3. * eta) * chiA3 +
                    (-81. / 16. - 703. * eta2 / 112. + 8797. * eta / 1008. + (9. / 4. - 6. * eta) * chiS2) * chiA;
    ...
    if (dM2)
    {
        ...
        // coeffs->rho33v3 = (chiS*dM*(-4. + 5.*eta) + chiA*(-4.
        // + 19.*eta))/(6.*dM);
        coeffs->rho33v3 = 0.0;
        ...
        if (use_hm)
        {
            // RC: This terms are in Eq.A6 in
            // https://journals.aps.org/prd/abstract/10.1103/PhysRevD.98.084028
            coeffs->rho33v6 = 3203101567. / 227026800. + (5. * a2) / 36. +
                              (-129509. / 25740. + 41. / 192. * CST_PI * CST_PI) * eta - 274621. / 154440. * eta2 +
                              12011. / 46332. * eta3;
        ...
        if (use_hm)
        {
            ...
            // https://journals.aps.org/prd/abstract/10.1103/PhysRevD.98.084028
            coeffs->f33v4 =
                (3. / 2. * chiS2 * dM + (3. - 12 * eta) * chiA * chiS + dM * (3. / 2. - 6. * eta) * chiA2) / (dM);
            coeffs->f33v5 = (dM * (241. / 30. * eta2 + 11. / 20. * eta + 2. / 3.) * chiS +
                             (407. / 30. * eta2 - 593. / 60. * eta + 2. / 3.) * chiA) /
                            (dM);
            coeffs->f33v6 =
                (dM * (6. * eta2 - 27. / 2. * eta - 7. / 4.) * chiS2 + (44. * eta2 - 1. * eta - 7. / 2.) * chiA * chiS +
                 dM * (-12 * eta2 + 11. / 2. * eta - 7. / 4.) * chiA2) /
                dM;
            coeffs->f33vh6 =
                (dM * (593. / 108. * eta - 81. / 20.) * chiS + (7339. / 540. * eta - 81. / 20.) * chiA) / (dM);
        }
    }
    else
    {
    ...
    coeffs->rho32v2 = (-4. * a2 * eta2) / (9. * m1Plus3eta2) + (328. - 1115. * eta + 320. * eta2) / (270. * m1Plus3eta);
    if (SpinAlignedEOBversion == 4)
    {
        coeffs->rho32v2 = (328. - 1115. * eta + 320. * eta2) / (270. * m1Plus3eta);
    }
    coeffs->rho32v3 =
        (2. * (45. * a * m1Plus3eta3 - a * eta * (328. - 2099. * eta + 5. * (733. + 20. * a2) * eta2 - 960. * eta3))) /
        (405. * m1Plus3eta3);
    coeffs->rho32v3 = 2. / 9. * a;
    coeffs->rho32v4 =
        a2 / 3. + (-1444528. + 8050045. * eta - 4725605. * eta2 - 20338960. * eta3 + 3085640. * eta2 * eta2) /
                      (1603800. * m1Plus3eta2);
    ...
    if (dM2)
    {
        ...
        // coeffs->rho31v3  = (chiA*(-4. + 11.*eta) + chiS*dM*(-4.
        // + 13.*eta))/(6.*dM);
        coeffs->rho31v3 = 0.0;
    ...
    coeffs->rho44v3 = (chiA * (10. - 39. * eta) * dM + chiS * (10. - 41. * eta + 42. * eta2)) / (15. * m1Plus3eta);
    coeffs->rho44v4 = a2 / 2. + (-511573572. + 2338945704. * eta - 313857376. * eta2 - 6733146000. * eta3 +
                                 1252563795. * eta2 * eta2) /
    ...
    if (use_hm)
    {
        ...
        // https://journals.aps.org/prd/abstract/10.1103/PhysRevD.98.084028
        coeffs->rho44v4 =
            (-511573572. + 2338945704. * eta - 313857376. * eta2 - 6733146000. * eta3 + 1252563795. * eta2 * eta2) /
                (317116800. * m1Plus3eta2) +
            chiS2 / 2. + dM * chiS * chiA + dM2 * chiA2 / 2.;
        coeffs->rho44v5 = chiA * dM * (-8280. + 42716. * eta - 57990. * eta2 + 8955 * eta3) / (6600. * m1Plus3eta2) +
                          chiS * (-8280. + 66284. * eta - 176418. * eta2 + 128085. * eta3 + 88650 * eta2 * eta2) /
                              (6600. * m1Plus3eta2);
    ...
    coeffs->rho42v3 = (chiA * (10. - 21. * eta) * dM + chiS * (10. - 59. * eta + 78. * eta2)) / (15. * m1Plus3eta);
    ...
    if (dM2)
    {
        ...
        if (use_hm)
        {
            ...
            coeffs->f55v3 =
                chiA / dM *
                    (10. / (3. * (-1. + 2. * eta)) - 70. * eta / (3. * (-1. + 2. * eta)) +
                     110. * eta2 / (3. * (-1. + 2. * eta))) +
                chiS * (10. / (3. * (-1. + 2. * eta)) - 10. * eta / (-1. + 2. * eta) + 10 * eta2 / (-1. + 2. * eta));
            coeffs->f55v4 =
                chiS2 * (-5. / (2. * (-1. + 2. * eta)) + 5. * eta / (-1. + 2. * eta)) +
                chiA * chiS / dM *
                    (-5. / (-1. + 2. * eta) + 30. * eta / (-1. + 2. * eta) - 40. * eta2 / (-1. + 2. * eta)) +
                chiA2 * (-5. / (2. * (-1. + 2. * eta)) + 15. * eta / (-1. + 2. * eta) - 20. * eta2 / (-1. + 2. * eta));
            coeffs->f55v5c = 0; // RC: this is the calibration parameter which is
                                // initially set to 0.
        }
    }
    else
    {
        ...
        if (use_hm)
        {
            ...
            // https://journals.aps.org/prd/abstract/10.1103/PhysRevD.98.084028
            coeffs->f55v3 = chiA * (10. / (3. * (-1. + 2. * eta)) - 70. * eta / (3. * (-1. + 2. * eta)) +
                                    110. * eta2 / (3. * (-1. + 2. * eta)));
            coeffs->f55v4 =
                chiA * chiS * (-5. / (-1. + 2. * eta) + 30. * eta / (-1. + 2. * eta) - 40. * eta2 / (-1. + 2. * eta));
            coeffs->f55v5c = 0;
        }
    }
    ...
    coeffs->rho54v2 =
        (-17448. + 96019. * eta - 127610. * eta2 + 33320. * eta3) / (13650. * (1. - 5. * eta + 5. * eta2));
    ...
    coeffs->rho52v2 =
        (-15828. + 84679. * eta - 104930. * eta2 + 21980. * eta3) / (13650. * (1. - 5. * eta + 5. * eta2));
    ...
    if (dM2)
    {
        coeffs->rho65v2 = (-185. + 838. * eta - 910. * eta2 + 220. * eta3) / (144. * (dM2 + 3. * eta2));
    ...
    if (dM2)
    {
        ...
        coeffs->rho63v2 = (-169. + 742. * eta - 750. * eta2 + 156. * eta3) / (144. * (dM2 + 3. * eta2));
    ...
    if (dM2)
    {
        ...
        coeffs->rho61v2 = (-161. + 694. * eta - 670. * eta2 + 124. * eta3) / (144. * (dM2 + 3. * eta2));
    ...
    if (dM2)
    {
        ...
        coeffs->rho77v2 = (-906. + 4246. * eta - 4963. * eta2 + 1380. * eta3) / (714. * (dM2 + 3. * eta2));
        coeffs->rho77v3 = -2. * a / 3.;
    }

    coeffs->rho76v2 = (2144. - 16185. * eta + 37828. * eta2 - 29351. * eta3 + 6104. * eta2 * eta2) /
                      (1666. * (-1 + 7 * eta - 14 * eta2 + 7 * eta3));

    coeffs->delta75vh3 = 95. / 252.;
    if (dM2)
    {

        coeffs->rho75v2 = (-762. + 3382. * eta - 3523. * eta2 + 804. * eta3) / (714. * (dM2 + 3. * eta2));
        coeffs->rho75v3 = -2. * a / 3.;
    }

    coeffs->rho74v2 = (17756. - 131805. * eta + 298872. * eta2 - 217959. * eta3 + 41076. * eta2 * eta2) /
                      (14994. * (-1. + 7. * eta - 14. * eta2 + 7. * eta3));

    coeffs->delta73vh3 = 19. / 84.;
    if (dM2)
    {

        coeffs->rho73v2 = (-666. + 2806. * eta - 2563. * eta2 + 420. * eta3) / (714. * (dM2 + 3. * eta2));
        coeffs->rho73v3 = -2. * a / 3.;
    }

    coeffs->rho72v2 = (16832. - 123489. * eta + 273924. * eta2 - 190239. * eta3 + 32760. * eta2 * eta2) /
                      (14994. * (-1. + 7. * eta - 14. * eta2 + 7. * eta3));
    ...
    if (dM2)
    {
        ...
        coeffs->rho71v2 = (-618. + 2518. * eta - 2083. * eta2 + 228. * eta3) / (714. * (dM2 + 3. * eta2));
        coeffs->rho71v3 = -2. * a / 3.;
    }

    /* l = 8, Eqs. A14a - A14h */

    coeffs->rho88v2 = (3482. - 26778. * eta + 64659. * eta2 - 53445. * eta3 + 12243. * eta2 * eta2) /
                      (2736. * (-1. + 7. * eta - 14. * eta2 + 7. * eta3));

    if (dM2)
    {
        coeffs->rho87v2 = (23478. - 154099. * eta + 309498. * eta2 - 207550. * eta3 + 38920 * eta2 * eta2) /
                          (18240. * (-1 + 6 * eta - 10 * eta2 + 4 * eta3));
    }

    coeffs->rho86v2 = (1002. - 7498. * eta + 17269. * eta2 - 13055. * eta3 + 2653. * eta2 * eta2) /
                      (912. * (-1. + 7. * eta - 14. * eta2 + 7. * eta3));

    if (dM2)
    {
        coeffs->rho85v2 = (4350. - 28055. * eta + 54642. * eta2 - 34598. * eta3 + 6056. * eta2 * eta2) /
                          (3648. * (-1. + 6. * eta - 10. * eta2 + 4. * eta3));
    }

    coeffs->rho84v2 = (2666. - 19434. * eta + 42627. * eta2 - 28965. * eta3 + 4899. * eta2 * eta2) /
                      (2736. * (-1. + 7. * eta - 14. * eta2 + 7. * eta3));

    if (dM2)
    {
        coeffs->rho83v2 = (20598. - 131059. * eta + 249018. * eta2 - 149950. * eta3 + 24520. * eta2 * eta2) /
                          (18240. * (-1. + 6. * eta - 10. * eta2 + 4. * eta3));
    }

    coeffs->rho82v2 = (2462. - 17598. * eta + 37119. * eta2 - 22845. * eta3 + 3063. * eta2 * eta2) /
                      (2736. * (-1. + 7. * eta - 14. * eta2 + 7. * eta3));

    if (dM2)
    {
        coeffs->rho81v2 = (20022. - 126451. * eta + 236922. * eta2 - 138430. * eta3 + 21640. * eta2 * eta2) /
                          (18240. * (-1. + 6. * eta - 10. * eta2 + 4. * eta3));
    }
    ...
    coeffs->h22T4ff20 =
        -425. / 504. + 1355. * eta / 252. - 20. * eta2 / 63. + 0.5 * pow(dM * chiA + (1. - 2. * eta) * chiS, 2.);
    ...
    coeffs->h22T4ff02 =
        -10379. / 3024. - 635. * eta / 189. + 26. * eta2 / 189. + 0.5 * pow(dM * chiA + (1. - 2. * eta) * chiS, 2.);
    coeffs->h22T4ff00 = 65. / 252. - 31. * eta / 36. + 205. * eta2 / 252. +
                        0.5 * pow((dM * chiA + (1. + eta) * chiS), 2.) - 3. * eta2 * chiS * chiS;
...
)
{
    ...
    /* Actual values of the coefficients are defined in the next function of this
     * file */
                                                  ...
                                                  v * (hCoeffs->rho21v6 + hCoeffs->rho21v6l * eulerlogxabs +
                                                       v * (hCoeffs->rho21v7 + hCoeffs->rho21v7l * eulerlogxabs +
                                                            v * (hCoeffs->rho21v8 + hCoeffs->rho21v8l * eulerlogxabs +
                                                                 v2 * (hCoeffs->rho21v10 +
                                                                       hCoeffs->rho21v10l * eulerlogxabs)))))))));
            if (use_hm)
            {
                // RC: This terms are in Eq.A11 in
                // https://journals.aps.org/prd/abstract/10.1103/PhysRevD.98.084028
                auxflm =
                    v * (hCoeffs->f21v1 + v2 * (hCoeffs->f21v3 + v * hCoeffs->f21v4 +
                                                  ...
                                                  v * (hCoeffs->rho33v6 + hCoeffs->rho33v6l * eulerlogxabs +
                                                       v * (hCoeffs->rho33v7 +
                                                            v * (hCoeffs->rho33v8 + hCoeffs->rho33v8l * eulerlogxabs +
                                                                 v2 * (hCoeffs->rho33v10 +
                                                                       hCoeffs->rho33v10l * eulerlogxabs))))))));
                // RC: This terms are in Eq.A10 in
                // https://journals.aps.org/prd/abstract/10.1103/PhysRevD.98.084028
                auxflm =
                    v * (v2 * (hCoeffs->f33v3 + v * (hCoeffs->f33v4 + v * (hCoeffs->f33v5 + v * hCoeffs->f33v6)))) +
        ...
        case 3:
            rholm =
                ...
                v * (hCoeffs->rho43v +
                     v * (hCoeffs->rho43v2 +
                          v2 * (hCoeffs->rho43v4 +
                                v * (hCoeffs->rho43v5 + v * (hCoeffs->rho43v6 + hCoeffs->rho43v6l * eulerlogxabs)))));
        ...
        case 2:
            rholm =
                ...
                v2 * (hCoeffs->rho42v2 +
                      v * (hCoeffs->rho42v3 +
                           v * (hCoeffs->rho42v4 +
                                v * (hCoeffs->rho42v5 + v * (hCoeffs->rho42v6 + hCoeffs->rho42v6l * eulerlogxabs)))));
            break;
        case 1:
            rholm =
                1. +
                v * (hCoeffs->rho41v +
                     v * (hCoeffs->rho41v2 +
                          v2 * (hCoeffs->rho41v4 +
                                v * (hCoeffs->rho41v5 + v * (hCoeffs->rho41v6 + hCoeffs->rho41v6l * eulerlogxabs)))));
        ...
        case 5:
            ...
            else
            {
                rholm = 1. + v2 * (hCoeffs->rho55v2 +
                                   v * (hCoeffs->rho55v3 +
                                        v * (hCoeffs->rho55v4 + v * (hCoeffs->rho55v5 + v * hCoeffs->rho55v6))));
            }
            break;
        case 4:
            rholm = 1. + v2 * (hCoeffs->rho54v2 + v * (hCoeffs->rho54v3 + v * hCoeffs->rho54v4));
            break;
        case 3:
            rholm =
                1. + v2 * (hCoeffs->rho53v2 + v * (hCoeffs->rho53v3 + v * (hCoeffs->rho53v4 + v * hCoeffs->rho53v5)));
            break;
        case 2:
            rholm = 1. + v2 * (hCoeffs->rho52v2 + v * (hCoeffs->rho52v3 + v * hCoeffs->rho52v4));
            break;
        case 1:
            rholm =
                1. + v2 * (hCoeffs->rho51v2 + v * (hCoeffs->rho51v3 + v * (hCoeffs->rho51v4 + v * hCoeffs->rho51v5)));
            break;
        default:
            return CEV_FAILURE;
            break;
     ...
     * and the only contribution comes from the auxflm term that is proportional
     * to chiA (asymmetric spins). In this case, we must ignore the nonspin terms
     * directly, since the leading term defined by CalculateThisMultipolePrefix
     * in LALSimIMREOBNewtonianMultipole.c is not zero (see comments there).
     */
    if (eta == 0.25 && m % 2)
    {
        rholmPwrl = auxflm;
 ...
 * This function calculates coefficients for hlm mode factorized-resummed
 * waveform. The coefficients are pre-computed and stored in the SpinEOBParams
 * structure. Eq. 17 and the entire Appendix of PRD 86, 024011 (2012) + changes
...
)
{
    // int		debugPK = 0;
    // if (debugPK) {
    // 	XLAL_PRINT_INFO("In XLALSimIMREOBCalcSpinPrecFacWaveformCoefficients:
    // Renewing hlm coefficients.\n"); 		XLAL_PRINT_INFO("PK:: chiS = %.12e,
    // chiA =
    // %.12e, a = %.12e (UNUSED), EOBVERSION = %d\n", 			   chiS, chiA,
    // tmpa, SpinAlignedEOBversion);
    // }
    REAL8 a = tmpa;
    INT waveform = 0;
    REAL8 eta2 = eta * eta;
    ...
    // RC: Here I am taking care of the fact that the flux used to calibrate
    // SEOBNRv4 is missing some terms in the 21 mode that I have added later in
    // SEOBNRv4HM. For this reason when computing the Coefficients for the 21
    // mode one has to specify if they are needed for the flux or for the
    // waveform.
    ...
    if (SpinAlignedEOBversion == 2 && chiS + chiA * dM / (1. - 2. * eta) > 0.)
    {
        ...
        //		coeffs->delta22v6 = eta*(1./4.*(1. - 1080.*chi -
        // sqrt((1. - 1080.*chi)*(1. - 1080*chi) + 8.*(13.5 +270.*chi
        // +13.5*chi*chi))));
    }
    if (SpinAlignedEOBversion == 3 /*&& chiS + chiA * dM / (1. - 2. * eta) > 0.*/)
    {
        //				coeffs->delta22v6 = -540. * eta * (chiS
        //+ chiA * dM / (1. - 2. * eta));
        double chi = (chiS + chiA * dM / (1. - 2. * eta));
        coeffs->delta22v6 =
            eta * (1. / 4. *
                   (1. - 1080. * chi -
                    sqrt((1. - 1080. * chi) * (1. - 1080 * chi) + 8. * (13.5 + 270. * chi + 13.5 * chi * chi))));
    }
    coeffs->rho22v2 = -43. / 42. + (55. * eta) / 84.;
    coeffs->rho22v3 = (-2. * (chiS + chiA * dM - chiS * eta)) / 3.;
    switch (SpinAlignedEOBversion)
    ...
    // if (debugPK) {
    // 	XLAL_PRINT_INFO("\nPK:: dM, eta, chiS, chiA while renewing hlm coeffs:
    // %e, %e, %e, %e\n", 		   dM, eta, chiS, chiA); 	XLAL_PRINT_INFO("PK::
    // Renewed rho-lm coeffs: v2 = %.16e, v3 = %.16e, v4 = %.16e, v5 = %.16e\nv6
    // = %.16e, v6l = %.16e v7 = %.16e v8 = %.16e, v8l = %.16e v10 = %.16e v10l =
    // %.16e\n", 		   coeffs->rho22v2, coeffs->rho22v3, coeffs->rho22v4,
    // coeffs->rho22v5, 		   coeffs->rho22v6, coeffs->rho22v6l,
    // coeffs->rho22v7,
    // coeffs->rho22v8, 		 coeffs->rho22v8l, coeffs->rho22v10,
    // coeffs->rho22v10l);
    // }
    if (waveform == 1)
    {
    ...
    if (dM2)
    {
        ...
        // coeffs->rho21v1 = (-3. * (chiS + chiA / dM)) / (4.);
        coeffs->rho21v1 = 0.0;
        // coeffs->rho21v2 = -59. / 56 - (9. * chiAPlusChiSdM * chiAPlusChiSdM) /
        // (32. * dM2) + (23. * eta) / 84.;
        switch (SpinAlignedEOBversion)
        {
        case 1:
    ...
    {
        coeffs->f21v1 = -3. * chiA / 2.; // Odd modes in dm->0 limit, note that there is a typo
                                         // in Taracchini et.al. discussion after A7 (even->odd)
        switch (SpinAlignedEOBversion)
        {
        case 1:
            coeffs->f21v3 = 0.0;
        ...
        case 4:
            ...
            else
            {
                ...
                // printf("f21v1 = %.16f\n f21v3 = %.16f\n f21v4 = %.16f\n f21v5
                // =
                // %.16f\n f21v6 = %.16f\n", coeffs->f21v1, coeffs->f21v3,
                // coeffs->f21v4, coeffs->f21v5, coeffs->f21v6);
            }
            /* End new terms for SEOBNRv4HM */
            break;
        default:
    ...
    {
        coeffs->rho33v2 = -7. / 6. + (2. * eta) / 3.;
        // coeffs->rho33v3 = (chiS * dM * (-4. + 5. * eta) + chiA * (-4. + 19. *
        // eta)) / (6. * dM);
        coeffs->rho33v3 = 0.0;
        coeffs->rho33v4 = -6719. / 3960. + a2 / 2. - (1861. * eta) / 990. + (149. * eta2) / 330.;
        coeffs->rho33v5 = (-4. * a) / 3.;
    ...
    coeffs->rho32v = (4. * chiS * eta) / (-3. * m1Plus3eta);
    /** TODO The term proportional to eta^2 a^2 in coeffs->rho32v2 is wrong, but
     * it was used in the calibration of SEOBNRv2 */
    coeffs->rho32v2 = (-4. * a2 * eta2) / (9. * m1Plus3eta2) + (328. - 1115. * eta + 320. * eta2) / (270. * m1Plus3eta);
    if (SpinAlignedEOBversion == 4)
    {
        coeffs->rho32v2 = (328. - 1115. * eta + 320. * eta2) / (270. * m1Plus3eta);
    ...
    if (dM2)
    {
        ...
        coeffs->rho31v2 = -13. / 18. - (2. * eta) / 9.;
        // coeffs->rho31v3 = (chiA * (-4. + 11. * eta) + chiS * dM * (-4. + 13. *
        // eta)) / (6. * dM);
        coeffs->rho31v3 = 0.0;
        coeffs->rho31v4 = 101. / 7128. - (5. * a2) / 6. - (1685. * eta) / 1782. - (829. * eta2) / 1782.;
        coeffs->rho31v5 = (4. * a) / 9.;
    ...
    if (dM2)
    {
        ...
        // coeffs->rho43v = (5. * (chiA - chiS * dM) * eta) / (8. * dM * (-1.
        // + 2. * eta));
        coeffs->rho43v = 0.0;
        coeffs->rho43v2 = (222. - 547. * eta + 160. * eta2) / (176. * (-1. + 2. * eta));
        coeffs->rho43v4 = -6894273. / 7047040. + (3. * a2) / 8.;
    ...
    if (dM2)
    {
        ...
        // coeffs->rho41v = (5. * (chiA - chiS * dM) * eta) / (8. * dM * (-1.
        // + 2. * eta));
        coeffs->rho41v = 0.0;
        coeffs->rho41v2 = (602. - 1385. * eta + 288. * eta2) / (528. * (-1. + 2. * eta));
        coeffs->rho41v4 = -7775491. / 21141120. + (3. * a2) / 8.;
 ...
 */
REAL8
XLALSimIMRSpinPrecEOBNonKeplerCoeff(const REAL8 values[],     /**<< Dynamical variables */
...
)
{
    ...
    //     for(int i =0; i < 12; i++)
    //     if( isnan(values[i]) ) {
    //         XLAL_PRINT_INFO("XLALSimIMRSpinPrecEOBNonKeplerCoeff::values
    //         %3.10f %3.10f %3.10f %3.10f %3.10f %3.10f %3.10f %3.10f %3.10f
    //         %3.10f %3.10f %3.10f\n", values[0], values[1], values[2],
    //         values[3], values[4], values[5], values[6], values[7], values[8],
    //         values[9], values[10], values[11]); XLALPrintError( "XLAL Error -
...
)
{
    // int		debugPK = 0;
    INT l, m;
    // PRINT_LOG_INFO(LOG_CRITICAL, "polvalues %3.10f %3.10f %3.10f %3.10f",
    // values->data[0], values->data[1], values->data[2], values->data[3]);
    // PRINT_LOG_INFO(LOG_CRITICAL, "values %3.10f %3.10f %3.10f %3.10f %3.10f
    // %3.10f %3.10f %3.10f %3.10f %3.10f %3.10f %3.10f",
    //     cartvalues->data[0], cartvalues->data[1], cartvalues->data[2],
    //     cartvalues->data[3], cartvalues->data[4], cartvalues->data[5],
    //     cartvalues->data[6], cartvalues->data[7], cartvalues->data[8],
    //     cartvalues->data[9], cartvalues->data[10], cartvalues->data[11]);
        ...
        for (m = 1; m <= l; m++)
        {
            ...
            case 2:
                ...
                case 2:
                    ...
                    // FIXME
                    //  if (debugPK){
                    //     XLAL_PRINT_INFO("PK:: rho22v2 = %.12e, rho22v3 =
                    //     %.12e, rho22v4 = %.12e,\n rho22v5 = %.16e, rho22v6 =
                    //     %.16e, rho22v6LOG =
                    //     %.16e, \n rho22v7 = %.12e, rho22v8 = %.16e, rho22v8LOG
                    //     = %.16e, \n rho22v10 = %.16e, rho22v10LOG = %.16e\n,
                    //     rho22v6 = %.12e, rho22v8 = %.12e, rho22v10 = %.12e\n",
                    //        hCoeffs->rho22v2, hCoeffs->rho22v3,
                    //        hCoeffs->rho22v4, hCoeffs->rho22v5,
                    //        hCoeffs->rho22v6, hCoeffs->rho22v6l,
                    //        hCoeffs->rho22v7, hCoeffs->rho22v8,
                    //        hCoeffs->rho22v8l, hCoeffs->rho22v10,
                    //        hCoeffs->rho22v10l, hCoeffs->rho22v6 +
                    //        hCoeffs->rho22v6l * eulerlogxabs, hCoeffs->rho22v8
                              ...
                              v * (hCoeffs->rho33v3 +
                                   v * (hCoeffs->rho33v4 +
                                        v * (hCoeffs->rho33v5 +
                                             v * (hCoeffs->rho33v6 + hCoeffs->rho33v6l * eulerlogxabs +
                                                  v * (hCoeffs->rho33v7 +
                                                       (hCoeffs->rho33v8 + hCoeffs->rho33v8l * eulerlogxabs) * v))))));
                    auxflm = v * v2 * hCoeffs->f33v3;
                    break;
                case 2:
                    rholm =
                ...
                case 4:
                    ...
                    rholm = 1. + v2 * (hCoeffs->rho44v2 +
                                       v * (hCoeffs->rho44v3 +
                                            v * (hCoeffs->rho44v4 +
                                                 v * (hCoeffs->rho44v5 +
                                                      (hCoeffs->rho44v6 + hCoeffs->rho44v6l * eulerlogxabs) * v))));
                    break;
                case 3:
            ...
            case 5:
                ...
                case 5:
                    rholm = 1. + v2 * (hCoeffs->rho55v2 +
                                       v * (hCoeffs->rho55v3 +
                                            v * (hCoeffs->rho55v4 + v * (hCoeffs->rho55v5 + hCoeffs->rho55v6 * v))));
                    break;
                case 4:
...
)
{
    ...
    for (l = 2; l <= lMax; l++)
    {
        for (m = 1; m <= l; m++)
        {
            ...
            // cartvalues->data[2] * cartvalues->data[4]; rcrossp_y =
            // cartvalues->data[2] * cartvalues->data[3] - cartvalues->data[0] *
            // cartvalues->data[5]; rcrossp_z = cartvalues->data[0] *
            // cartvalues->data[4] - cartvalues->data[1] * cartvalues->data[3];
...
)
{
    ...
    // RC: the function XLALSimIMRSpinPrecEOBNonKeplerCoeff is calculating again
    // the coefficients hCoeffs, for the omega. These are different because
    // for the dynamics we are using different coefficients for the 21 mode. I
    // store the old coefficients and the I recover them ofter vPhi is computed.
    ...
    // 	// print_debug("f21v1 = %.16f f21v3 = %.16f f21v4 = %.16f f21v5 =
    // %.16f\n", hCoeffs->f21v1, hCoeffs->f21v3, hCoeffs->f21v4, hCoeffs->f21v5);
                                                   ...
                                                   v * (hCoeffs->rho21v7 + hCoeffs->rho21v7l * eulerlogxabs +
                                                        v * (hCoeffs->rho21v8 + hCoeffs->rho21v8l * eulerlogxabs +
                                                             (hCoeffs->rho21v10 + hCoeffs->rho21v10l * eulerlogxabs) *
                                                                 v2))))))));
            auxflm = v * (hCoeffs->f21v1 +
                          v2 * (hCoeffs->f21v3 + v * hCoeffs->f21v4 + v2 * (hCoeffs->f21v5 + v * hCoeffs->f21v6)));
        }
        break;
        default:
            return CEV_FAILURE;
        ...
        case 4:
            rholm =
                ...
                v2 * (hCoeffs->rho44v2 +
                      v * (hCoeffs->rho44v3 +
                           v * (hCoeffs->rho44v4 +
                                v * (hCoeffs->rho44v5 + (hCoeffs->rho44v6 + hCoeffs->rho44v6l * eulerlogxabs) * v))));
