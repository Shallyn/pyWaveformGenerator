...
int PrecHcapNumericalDerivative(double t, const REAL8 values[], REAL8 dvalues[], void *funcParams)
{
    // int		debugPK = 0;
    ...
    // rMag = sqrt(rVec.data[0] * rVec.data[0] + rVec.data[1] * rVec.data[1] +
    // rVec.data[2] * rVec.data[2]);
    rMag = vars.r;
    /* This is p^*.r/|r| */
    // prT = pVec.data[0] * (rVec.data[0] / rMag) + pVec.data[1] * (rVec.data[1]
    // / rMag)
    ...
    // {
    // print_debug("INPUT: values|dvalues = (%.5e, %.5e, %.5e), (%.5e, %.5e,
    // %.5e)|(%.5e, %.5e, %.5e), (%.5e, %.5e, %.5e)\n", 		values[0],
    // values[1], values[2], 		values[3], values[4], values[5],
    // dvalues[0], dvalues[1], dvalues[2], 		dvalues[3], dvalues[4],
    // dvalues[5]);

    // 	print_debug("tmpDValues = (%.5e, %.5e, %.5e), (%.5e, %.5e, %.5e),
    // (%.5e,
    // %.5e, %.5e), (%.5e, %.5e, %.5e)\n", 			tmpDValues[0],
    // tmpDValues[1],
    // tmpDValues[2], 			tmpDValues[3], tmpDValues[4], tmpDValues[5],
    // tmpDValues[6],
    // tmpDValues[7], tmpDValues[8], 			tmpDValues[9],
    // tmpDValues[10], tmpDValues[11]); 	print_debug("tmpP = (%.5e, %.5e,
    ...
    // r = polarDynamics.data[0] = sqrt(values[0] * values[0] + values[1] *
    // values[1]
    // 		      + values[2] * values[2]);
    ...
    // if ( params.params->use_hm ) SpinAlignedEOBversionForWaveformCoefficients
    // = 451; else SpinAlignedEOBversionForWaveformCoefficients =
    // SpinAlignedEOBversion;
    XLALSimIMREOBCalcSpinPrecFacWaveformCoefficients(params.params->hCoeffs, mass1, mass2, eta, tplspin, chiS, chiA,
                                                     SpinAlignedEOBversionForWaveformCoefficients);
    // if (SpinAlignedEOBversion == 4)
    // {
    ...
    // tmpPdotT1[1], tmpPdotT1[2]); print_debug("tmpPdotT2 = (%.5e, %.5e,
    // %.5e)\n", tmpPdotT2[0], tmpPdotT2[1], tmpPdotT2[2]);
    // print_debug("tmpPdotT3 = (%.5e,
    // %.5e, %.5e)\n", tmpPdotT3[0], tmpPdotT3[1], tmpPdotT3[2]);
    // if (IS_DEBUG) {
    // 	print_debug("t, values = %.16e, (%.5e, %.5e, %.5e), (%.5e, %.5e, %.5e),
    // (%.5e, %.5e, %.5e), (%.5e, %.5e, %.5e)\n", t, 			values[0],
    // values[1], values[2], 			values[3], values[4],
    // values[5], 			values[6], values[7], values[8],
    // values[9], values[10], values[11]); 	print_debug("dvalues =
    // (%.5e, %.5e, %.5e), (%.5e, %.5e, %.5e), (%.5e, %.5e, %.5e), (%.5e, %.5e,
    // %.5e)\n", 			dvalues[0], dvalues[1], dvalues[2],
    // dvalues[3], dvalues[4],
    // dvalues[5], 			dvalues[6], dvalues[7], dvalues[8],
    // dvalues[9], dvalues[10], dvalues[11]); 	print_debug("prTDot, fRRVec =
...
int PrecHcapNumericalDerivative_inverse(double t, const REAL8 values[], REAL8 dvalues[], void *funcParams)
{
    // int		debugPK = 0;
    /** lMax: l index up to which h_{lm} modes are included in the computation of
     * the GW enegy flux: see Eq. in 13 in PRD 86,  024011 (2012) */
