...
*
* This module contains basic functions for calculating EOB real
SpinHamiltonian.
* Functions list:
...
INT EOBCalculateSpinEOBHamCoeffs(SpinEOBHCoeffs *coeffs, const REAL8 eta, const REAL8 a, HyperParams *params)
{
    if (!coeffs)
        return CEV_FAILURE;
    ...
    coeffs->KK = KK = coeff00K + coeff01K * chi + coeff02K * chi2 + coeff03K * chi3 + coeff10K * eta +
                      coeff11K * eta * chi + coeff12K * eta * chi2 + coeff13K * eta * chi3 + coeff20K * eta2 +
                      coeff21K * eta2 * chi + coeff22K * eta2 * chi2 + coeff23K * eta2 * chi3 + coeff30K * eta3 +
    ...
    coeffs->k3 = k3 = -k1 * k1 * k1 / 3. + k1 * k2 + k1 * k1 * m1PlusEtaKK - 2. * (k2 - m1PlusEtaKK) * m1PlusEtaKK -
                      a * a * k1 * m1PlusEtaKK * m1PlusEtaKK;
    coeffs->k4 = k4 =
        (24. * k1 * k1 * k1 * k1 - 96. * k1 * k1 * k2 + 48. * k2 * k2 - 64. * k1 * k1 * k1 * m1PlusEtaKK +
         48. * a * a * (k1 * k1 - 2. * k2) * m1PlusEtaKK * m1PlusEtaKK + 96. * k1 * (k3 + 2. * k2 * m1PlusEtaKK) -
         m1PlusEtaKK * (192. * k3 + m1PlusEtaKK * (-3008. + 123. * CST_PI * CST_PI))) /
        96.;
    /* Include eta^2 terms at 4PN from arXiv:1305.4884 */
    coeffs->k5 = k5 =
        m1PlusEtaKK * m1PlusEtaKK *
        (-4237. / 60. + 128. / 5. * CST_GAMMA + 2275. * CST_PI * CST_PI / 512. -
         1. / 3. * a * a * (k1p3 - 3. * k1 * k2 + 3. * k3) -
         (k1p3 * k1p2 - 5. * k1p3 * k2 + 5. * k1 * k2 * k2 + 5. * k1p2 * k3 - 5. * k2 * k3 - 5. * k1 * k4) / 5. /
             m1PlusEtaKK / m1PlusEtaKK +
         (k1p2 * k1p2 - 4. * k1p2 * k2 + 2. * k2 * k2 + 4. * k1 * k3 - 4. * k4) / 2. / m1PlusEtaKK + 256. / 5. * ln2 +
         (41. * CST_PI * CST_PI / 32. - 221. / 6.) * eta);
    coeffs->k5l = k5l = m1PlusEtaKK * m1PlusEtaKK * 64. / 5.;
    coeffs->d1v2 = coeff00dSO + coeff01dSO * chi + coeff02dSO * chi2 + coeff03dSO * chi3 + coeff10dSO * eta +
                   coeff11dSO * eta * chi + coeff12dSO * eta * chi2 + coeff13dSO * eta * chi3 + coeff20dSO * eta2 +
                   coeff21dSO * eta2 * chi + coeff22dSO * eta2 * chi2 + coeff23dSO * eta2 * chi3 + coeff30dSO * eta3 +
                   coeff31dSO * eta3 * chi + coeff32dSO * eta3 * chi2 + coeff33dSO * eta3 * chi3;

    // dSS
    coeffs->dheffSSv2 = coeff00dSS + coeff01dSS * chi + coeff02dSS * chi2 + coeff03dSS * chi3 + coeff10dSS * eta +
                        coeff11dSS * eta * chi + coeff12dSS * eta * chi2 + coeff13dSS * eta * chi3 + coeff20dSS * eta2 +
                        coeff21dSS * eta2 * chi + coeff22dSS * eta2 * chi2 + coeff23dSS * eta2 * chi3 +
                        coeff30dSS * eta3 + coeff31dSS * eta3 * chi + coeff32dSS * eta3 * chi2 +
                        coeff33dSS * eta3 * chi3;
    if (params && params->flagTuning)
    {
...
INT XLALSimIMRCalculateSpinPrecEOBHCoeffs_v2(SpinEOBHCoeffs *coeffs, /**<< OUTPUT, EOB parameters including pre-computed
                                                                        coefficients */
                                             const REAL8 eta,        /**<< symmetric mass ratio */
                                             REAL8 a,                /**<< Normalized deformed Kerr spin */
                                             REAL8 chi, /**<< The augmented spin, with correct aligned-spin limit */
                                             const UINT SpinAlignedEOBversion, /**<< 4 for SEOBNRv4P; Possible to extend
                                                                                  this later later */
                                             HyperParams *params)
{
    ...
    // if (debugPK)
    // {
    //     PRINT_LOG_INFO(LOG_CRITICAL, "In
    //     XLALSimIMRCalculateSpinPrecEOBHCoeffs: SpinAlignedEOBversion =
    //     %d,%d\n",
    //                     (int)SpinAlignedEOBversion,
    //                     (int)coeffs->SpinAlignedEOBversion);
    //     fflush(NULL);
    // }
    ...
    coeffs->k3 = k3 = -(k1 * k1) * k1 * third + k1 * k2 + (k1 * k1) * m1PlusEtaKK -
                      2. * (k2 - m1PlusEtaKK) * m1PlusEtaKK - a * a * k1 * (m1PlusEtaKK * m1PlusEtaKK);
    coeffs->k4 = k4 =
        ((24. / 96.) * (k1 * k1) * (k1 * k1) - (96. / 96.) * (k1 * k1) * k2 + (48. / 96.) * k2 * k2 -
         (64. / 96.) * (k1 * k1) * k1 * m1PlusEtaKK +
         (48. / 96.) * (a * a) * (k1 * k1 - 2. * k2) * (m1PlusEtaKK * m1PlusEtaKK) +
         (96. / 96.) * k1 * (k3 + 2. * k2 * m1PlusEtaKK) -
         m1PlusEtaKK * ((192. / 96.) * k3 + m1PlusEtaKK * (-(3008. / 96.) + (123. / 96.) * CST_PI * CST_PI)));
    if (SpinAlignedEOBversion == 4)
    {
        // Look at Eq.(A2C) in Steinhoff et al. The last term from Eq.(2.3) in
        // Bohe et al.
        coeffs->k5 = k5 =
            m1PlusEtaKK * m1PlusEtaKK *
            (-4237. / 60. + 128. / 5. * CST_GAMMA + 2275. * CST_PI * CST_PI / 512. -
             third * (a * a) * (k1p3 - 3. * (k1 * k2) + 3. * k3) -
             ((k1p3 * k1p2) - 5. * (k1p3 * k2) + 5. * k1 * k2 * k2 + 5. * k1p2 * k3 - 5. * k2 * k3 - 5. * k1 * k4) *
                 fifth * invm1PlusEtaKK * invm1PlusEtaKK +
             ((k1p2 * k1p2) - 4. * (k1p2 * k2) + 2. * k2 * k2 + 4. * k1 * k3 - 4. * k4) * 0.5 * invm1PlusEtaKK +
             (256. / 5.) * ln2 + (41. * CST_PI * CST_PI / 32. - 221. / 6.) * eta);
    ...
    if (SpinAlignedEOBversion == 4)
    {
        ...
        coeffs->d1v2 = coeff00dSO + coeff01dSO * chi + coeff02dSO * chi2 + coeff03dSO * chi3 + coeff10dSO * eta +
                       coeff11dSO * eta * chi + coeff12dSO * eta * chi2 + coeff13dSO * eta * chi3 + coeff20dSO * eta2 +
                       coeff21dSO * eta2 * chi + coeff22dSO * eta2 * chi2 + coeff23dSO * eta2 * chi3 +
                       coeff30dSO * eta3 + coeff31dSO * eta3 * chi + coeff32dSO * eta3 * chi2 +
                       coeff33dSO * eta3 * chi3;

        // dSS: Eq.(4.14) in Bohe et al
        coeffs->dheffSSv2 = coeff00dSS + coeff01dSS * chi + coeff02dSS * chi2 + coeff03dSS * chi3 + coeff10dSS * eta +
                            coeff11dSS * eta * chi + coeff12dSS * eta * chi2 + coeff13dSS * eta * chi3 +
                            coeff20dSS * eta2 + coeff21dSS * eta2 * chi + coeff22dSS * eta2 * chi2 +
                            coeff23dSS * eta2 * chi3 + coeff30dSS * eta3 + coeff31dSS * eta3 * chi +
                            coeff32dSS * eta3 * chi2 + coeff33dSS * eta3 * chi3;
...
void CalculateSpinEOBHSACoeffs(REAL8 m1, REAL8 m2, REAL8 s1z, REAL8 s2z, SpinEOBHSACoeffs *coeffs)
{
    ...
    KK = 1.7336 + (-1.62045) * chi + (-1.38086) * chi2 + (1.43659) * chi3 + (10.2573) * eta + (2.26831) * eta * chi +
         (-0.426958) * eta * chi3 + (-126.687) * eta2 + (17.3736) * eta2 * chi + (6.16466) * eta2 * chi2 +
         (267.788) * eta3 + (-27.5201) * eta3 * chi + (31.1746) * eta3 * chi2 + (-59.1658) * eta3 * chi3;
    ...
    coeffs->k5 =
        ...
        (-4237. / 60. + 128. / 5. * CST_GAMMA + 2275. * CST_PI * CST_PI / 512. -
         (1. / 3.) * (a * a) * (k1p3 - 3. * (k1 * k2) + 3. * k3) -
         ((k1p3 * k1p2) - 5. * (k1p3 * k2) + 5. * k1 * k2 * k2 + 5. * k1p2 * k3 - 5. * k2 * k3 - 5. * k1 * k4) *
             (1. / 5.) * invm1PlusEtaKK * invm1PlusEtaKK +
         ((k1p2 * k1p2) - 4. * (k1p2 * k2) + 2. * k2 * k2 + 4. * k1 * k3 - 4. * k4) * 0.5 * invm1PlusEtaKK +
         (256. / 5.) * CST_LN2 + (41. * CST_PI * CST_PI / 32. - 221. / 6.) * eta);
    coeffs->k5l = (m1PlusEtaKK * m1PlusEtaKK) * (64. / 5.);
    REAL8 d1v2 = -44.5324 + 66.1987 * chi3 + (-343.313) * eta * chi2 + (-568.651) * eta * chi3 +
                 (2495.29) * eta2 * chi + (147.481) * eta2 * chi3;
    REAL8 dheffSSv2 = 6.06807 + (-36.0272) * eta + 37.1964 * eta * chi + (-41.0003) * eta * chi3 +
                      (-326.325) * eta2 * chi2 + (528.511) * eta2 * chi3 + (706.958) * eta3 + (1161.78) * eta3 * chi2;
    // print_debug("k1 = %.16e, k2 = %.16e, k3 = %.16e, k4 = %.16e, k5 = %.16e,
    // k5l = %.16e\n",
    ...
    // print_debug("PdsKu2 = %.16e, PdsKpn4 = %.16e, PdsKup2 = %.16e, PdsKp2pn2 =
    // %.16e, PdsKupn2 = %.16e, PdsKu3 = %.16e\n",
    //         coeffs->PdsKu2, coeffs->PdsKpn4, coeffs->PdsKup2,
    //         coeffs->PdsKp2pn2, coeffs->PdsKupn2, coeffs->PdsKu3);
    coeffs->D2 = 6. * eta;
...
REAL8
EOBSAHamiltonian(REAL8 r, REAL8 prT, REAL8 pphi, SpinEOBHSACoeffs *coeffs, REAL8 *csi)
{
    REAL8 r2 = r * r;
    ...
    // const REAL8 logarg = coeffs->k1*u + coeffs->k2*u2 + coeffs->k3*u3 +
    // coeffs->k4*u4
    //                                             + coeffs->k5*u5 +
    //                                             coeffs->k5l*u5*logu;
    const REAL8 logarg =
        u *
        (coeffs->k1 + u * (coeffs->k2 + u * (coeffs->k3 + u * (coeffs->k4 + u * (coeffs->k5 + coeffs->k5l * logu)))));
    // deltaULog = 1. + coeffs->eta*coeffs->k0 + coeffs->eta*log1p(fabs(1. +
    // logarg) - 1.);
    ...
    //     deltaU0 * coeffs->eta * (coeffs->k1 + 2.*coeffs->k2*u
    //     + 3.*coeffs->k3*u2 + 4.*coeffs->k4*u3 + 5.*(coeffs->k5 +
    //     coeffs->k5l*logu)*u4) / (1.+logarg);
    deltaU_u = 2. * (coeffs->invm1PlusEtaKK + coeffs->a2 * u) * deltaULog +
               deltaU0 * coeffs->eta *
                   (coeffs->k1 +
                    u * (2. * coeffs->k2 +
                         u * (3. * coeffs->k3 + u * (4. * coeffs->k4 + u * 5. * (coeffs->k5 + coeffs->k5l * logu))))) /
                   (1. + logarg);
    ...
    *csi = B * CC / w2;
    ...
    // if(IS_DEBUG){
    // print_debug( "term 1 in Hns: %.16e\n",  coeffs->PQ4 * prT * prT *prT *prT
    // * u2 ); print_debug( "term 2 in Hns: %.16e\n", 0.0 ); print_debug( "term 3
    // in Hns = %.16e\n", gammaphi * pf*pf ); print_debug( "term 4 in Hns =
    // %.16e\n", pnBar2 ); print_debug( "term 5 in Hns =
    // %.16e\n", 1./(alpha*alpha) ); print_debug( "term 6 in Hns = %.16e\n", pf
    // );}
    deltaSigma0 = coeffs->Pds0u * u + coeffs->Pds0p2 * pBar2 + coeffs->Pds0pn2 * pnBar2;
    deltaSigmaS = coeffs->PdsSu2 * u2 + coeffs->PdsSup2 * u * pBar2 + coeffs->PdsSpn4 * pnBar2 * pnBar2 +
                  coeffs->PdsSp4 * pBar2 * pBar2 + coeffs->PdsSupn2 * u * pnBar2 + coeffs->PdsSp2pn2 * pBar2 * pnBar2;
    deltaSigmaK = coeffs->PdsKu2 * u2 + coeffs->PdsKup2 * u * pBar2 + coeffs->PdsKpn4 * pnBar2 * pnBar2 +
                  coeffs->PdsKp4 * pBar2 * pBar2 + coeffs->PdsKupn2 * u * pnBar2 + coeffs->PdsKp2pn2 * pBar2 * pnBar2 +
                  coeffs->PdsKu3 * u3;
    SStar = coeffs->sign *
            (coeffs->sigmaStar + deltaSigma0 + deltaSigmaS * coeffs->sigmaStar + deltaSigmaK * coeffs->sigmaKerr);
    ...
    HTwr = 0.5 * CC * SStar * (r2 * alpha2 * pf * pf + deltaT * (r2 * QTilde - pr * pr * deltaR)) * u3 /
           (B * QTilde * alpha);
    ...
    //     logarg, deltaU0, deltaULog); print_debug("deltaU = %.16e, deltaU_u =
    //     %.16e\n", deltaU, deltaU_u); print_debug("deltaT = %.16e, deltaT_r =
    //     %.16e\n", deltaT, deltaT_r); print_debug("DD = %.16e, deltaR =
    //     %.16e\n", DD, deltaR); print_debug("wfd = %.16e, wfd_r = %.16e\n",
    //     wfd, wfd_r); print_debug("pr = %.16e, pnBar2 = %.16e, pBar2 =
    //     %.16e\n", pr, pnBar2, pBar2); print_debug("alpha = %.16e, beta =
    //     %.16e, gammar =
    //     %.16e, gammaphi = %.16e\n", alpha, beta, gammar, gammaphi);
    //     print_debug("deltasigma0 = %.16e, deltasigmaS = %.16e, deltasigmaK =
    //     %.16e\n", deltaSigma0, deltaSigmaS, deltaSigmaK); print_debug("BR =
    //     %.16e, nur = %.16e, wr = %.16e, SStar = %.16e\n", BR, nur, wr, SStar);
    //     print_debug("Hns = %.16e, HTwr = %.16e, HSOL = %.16e, HSONL =
    //     %.16e\n", HNS, HTwr, HSOL, HSONL); print_debug("HeffSS = %.16e, HS =
    //     %.16e, Heff = %.16e\n", HSSeff, HS, Heff); print_debug("Hreal =
    //     %.16e\n", Hreal);
...
REAL8
EOBHamiltonian(const REAL8 eta, REAL8Vector *x, REAL8Vector *p, REAL8Vector *s1Vec, REAL8Vector *s2Vec,
               REAL8Vector *sigmaKerr, REAL8Vector *sigmaStar, INT tortoise, SpinEOBHCoeffs *coeffs)
{
    REAL8 r, r2, nx, ny, nz;
    ...
    logTerms = 1. + eta * coeffs->k0 +
               eta * GET_LN(1. + coeffs->k1 * u + coeffs->k2 * u2 + coeffs->k3 * u3 + coeffs->k4 * u4 +
                            coeffs->k5 * u5 + coeffs->k5l * u5 * GET_LN(u));
    ...
    deltaU_u =
        ...
        bulk *
            (eta * (coeffs->k1 + u * (2. * coeffs->k2 +
                                      u * (3. * coeffs->k3 +
                                           u * (4. * coeffs->k4 + 5. * (coeffs->k5 + coeffs->k5l * GET_LN(u)) * u))))) /
            (1. + coeffs->k1 * u + coeffs->k2 * u2 + coeffs->k3 * u3 + coeffs->k4 * u4 +
             (coeffs->k5 + coeffs->k5l * GET_LN(u)) * u5);
    /* ddeltaT/dr */
    deltaT_r = 2. * r * deltaU - deltaU_u;
    /* Eq. 5.39 of BB1 */
    ...
    qq = 2. * eta * (4. - 3. * eta);
    /* See Hns below. In Sec. II D of BB2 b3 and bb3 coeffs are chosen to be
     * zero.
     */
    ...
    // printf( " a = %.16e, r = %.16e\n", a, r );
    // printf( "D = %.16e, ww = %.16e, rho = %.16e, Lambda = %.16e, xi =
    // %.16e\npr = %.16e, pf = %.16e, deltaR = %.16e, deltaT = %.16e\n", D, ww,
    // sqrt(rho2), Lambda, sqrt(xi2), pr, pf, deltaR, deltaT );
    /* Eqs. 5.36 - 5.46 of BB1 */
    /* Note that the tortoise prT appears only in the quartic term, explained in
     * Eqs. 14 and 15 of Tarrachini et al. */
    Hns = GET_SQRT(1. + prT * prT * prT * prT * qq * u2 + ptheta2 / rho2 + pf * pf * rho2 / (Lambda * xi2) +
                   pr * pr * deltaR / rho2) /
              GET_SQRT(Lambda / (rho2 * deltaT)) +
          pf * ww / Lambda;
    ...
    BR = (-2. * deltaT + GET_SQRT(deltaR) * deltaT_r) / (2. * GET_SQRT(deltaR * deltaT));
    ...
    // Q = 1. + pvr*pvr/(exp(2.*MU)*xi2) + exp(2.*nu)*pxir*pxir/(B*B*xi2) +
    // pn*pn*deltaR/exp(2.*MU);
    ...
    /*
     deltaSigmaStar_x=(- 8.*aa*(1. + 3.*pn2*r - pp*r)*sKerr_x - 8.*bb*(1.
     + 3.*pn2*r - pp*r)*sStar_x + eta*(-8.*sKerr_x - 36.*pn2*r*sKerr_x
     + 3.*pp*r*sKerr_x + 14.*sStar_x - 30.*pn2*r*sStar_x
     + 4.*pp*r*sStar_x))/(12.*r);

     deltaSigmaStar_y=(-8.*aa*(1. + 3.*pn2*r - pp*r)*sKerr_y - 8.*bb*(1.
     + 3.*pn2*r - pp*r)*sStar_y + eta*(-8.*sKerr_y - 36.*pn2*r*sKerr_y
     + 3.*pp*r*sKerr_y + 14.*sStar_y - 30.*pn2*r*sStar_y
     + 4.*pp*r*sStar_y))/(12.*r);

     deltaSigmaStar_z=(-8.*aa*(1. + 3.*pn2*r - pp*r)*sKerr_z - 8.*bb*(1.
     + 3.*pn2*r - pp*r)*sStar_z + eta*(-8.*sKerr_z - 36.*pn2*r*sKerr_z
     + 3.*pp*r*sKerr_z + 14.*sStar_z - 30.*pn2*r*sStar_z
     + 4.*pp*r*sStar_z))/(12.*r);
     */
    deltaSigmaStar_x = eta *
                       (-8. * sKerr_x - 36. * pn2 * r * sKerr_x + 3. * pp * r * sKerr_x + 14. * sStar_x -
                        30. * pn2 * r * sStar_x + 4. * pp * r * sStar_x) /
                       (12. * r);

    deltaSigmaStar_y = eta *
                       (-8. * sKerr_y - 36. * pn2 * r * sKerr_y + 3. * pp * r * sKerr_y + 14. * sStar_y -
                        30. * pn2 * r * sStar_y + 4. * pp * r * sStar_y) /
                       (12. * r);

    deltaSigmaStar_z = eta *
                       (-8. * sKerr_z - 36. * pn2 * r * sKerr_z + 3. * pp * r * sKerr_z + 14. * sStar_z -
                        30. * pn2 * r * sStar_z + 4. * pp * r * sStar_z) /
                       (12. * r);
    ...
    /*
     ...
     + 2.*(24.*b13P5 - 24.*b23P5 + bbb*(14. - 66.*eta) + 103.*eta
     - 60.*eta*eta)*pp* r + 120.*(2.*b33P5 - 3.*eta*(bbb + eta))*pn2*pn2*r*r +
    ...
    /*
     ...
     + 4.*(-24.*a13P5 + 24.*a23P5 - 109.*eta + 51.*eta*eta + 2.*aaa*(-7.
     + 33.*eta))* pp*r + 30.*(-16.*a33P5 + 3.*eta*(8.*aaa + 9.*eta))*pn2*pn2*r*r
    ...
    // printf( "deltaSigmaStar_x = %.16e, deltaSigmaStar_y = %.16e,
    // deltaSigmaStar_z = %.16e\n",
    ...
    Hwr =
        (GET_EXP(-3. * MU - nu) * GET_SQRT(deltaR) *
         (GET_EXP(2. * (MU + nu)) * pxir * pxir * sv - B * GET_EXP(MU + nu) * pvr * pxir * sxi +
          B * B * xi2 *
              (GET_EXP(2. * MU) * (GET_SQRT(Q) + Q) * sv + pn * pvr * sn * GET_SQRT(deltaR) - pn * pn * sv * deltaR))) /
        (2. * B * (1. + GET_SQRT(Q)) * GET_SQRT(Q) * xi2);
    /* Eq. 3.45 of BB1, third term */
    Hwcos = (GET_EXP(-3. * MU - nu) * (sn * (-(GET_EXP(2. * (MU + nu)) * pxir * pxir) +
                                             B * B * (pvr * pvr - GET_EXP(2. * MU) * (GET_SQRT(Q) + Q) * xi2)) -
                                       B * pn * (B * pvr * sv - GET_EXP(MU + nu) * pxir * sxi) * GET_SQRT(deltaR))) /
            (2. * B * (1. + GET_SQRT(Q)) * GET_SQRT(Q));
    /* Eq. 3.44 of BB1, leading term */
    HSOL = (GET_EXP(-MU + 2. * nu) * (-B + GET_EXP(MU + nu)) * pxir * s3) / (B * B * GET_SQRT(Q) * xi2);
    /* Eq. 3.44 of BB1, next-to-leading term */
    HSONL = (GET_EXP(-2. * MU + nu) *
             (-(B * GET_EXP(MU + nu) * nucos * pxir * (1. + 2. * GET_SQRT(Q)) * sn * xi2) +
              (-(BR * GET_EXP(MU + nu) * pxir * (1. + GET_SQRT(Q)) * sv) +
               B * (GET_EXP(MU + nu) * nur * pxir * (1. + 2. * GET_SQRT(Q)) * sv + B * mur * pvr * sxi +
                    B * sxi *
                        (-(mucos * pn * xi2) + GET_SQRT(Q) * (mur * pvr - nur * pvr + (-mucos + nucos) * pn * xi2)))) *
                  GET_SQRT(deltaR))) /
            (B * B * (GET_SQRT(Q) + Q) * xi2);
    /* Eq. 3.43 and 3.45 of BB1 */
    Hs = w * s3 + Hwr * wr + Hwcos * wcos + HSOL + HSONL;
    /* Eq. 5.70 of BB1, last term */
    Hss = -0.5 * u3 * (sx * sx + sy * sy + sz * sz - 3. * sn * sn);
    /* Add correction for leading-order spin-induced quadrupole, relevant for BNS
     * - this is zero when kappa_1,2=1 */
    /* Eq. 5.70 of BB1 */
    H = Hns + Hs + Hss;

    /* Add the additional calibrated term */
    REAL8 Hess = 0;
    Hess = coeffs->dheffSS * eta * (sKerr_x * sStar_x + sKerr_y * sStar_y + sKerr_z * sStar_z) / (r * r * r * r);

    /* One more calibrated term proportional to S1^2+S2^2. Note that we use
     * symmetric expressions of m1,m2 and S1,S2 */
    /*H += coeffs->dheffSSv2 * eta / (r*r*r*r) / (1.-4.*eta)
     * ( (sKerr_x*sKerr_x + sKerr_y*sKerr_y +
     sKerr_z*sKerr_z)*(1.-4.*eta+2.*eta*eta)
...
double GSLSpinHamiltonianWrapper(double x, void *params)
{
    ...
    return EOBHamiltonian(seobParams->eta, &r, &p, &spin1norm, &spin2norm, &sigmaKerr, &sigmaStar, seobParams->tortoise,
                          seobHamCoeffs) /
           seobParams->eta;
}
...
static double GSLSpinAlignedHamiltonianWrapper(double x, void *params)
{
    ...
    return EOBHamiltonian(seobParams->eta, &r, &p, s1Vec, s2Vec, sigmaKerr, sigmaStar, seobParams->tortoise,
 ...
 */
REAL8
XLALSpinHcapNumDerivWRTParam(const INT paramIdx,       /**<< Index of the parameters */
...
)
{
    ...
    {
        gslStatus = gsl_deriv_central(&F, values[paramIdx], STEP_SIZE * mass1 * mass1, &result, &absErr);
    }
    else if (paramIdx >= 9)
    {
        gslStatus = gsl_deriv_central(&F, values[paramIdx], STEP_SIZE * mass2 * mass2, &result, &absErr);
    }
    else
    {
...
/**
 * This function calculates the function \f$\Delta_t(r)\f$ which appears in the
 * spinning EOB potential function. Eqs. 7a and 8.
 */
REAL8
...
)
{
    ...
    logTerms = 1. + eta * coeffs->k0 +
               eta * log(1. + coeffs->k1 * u + coeffs->k2 * u2 + coeffs->k3 * u3 + coeffs->k4 * u4 + coeffs->k5 * u5 +
                         coeffs->k5l * u5 * log(u));
    /*printf(" a = %.16e, u = %.16e\n",a,u);
        printf( "k0 = %.16e, k1 = %.16e, k2 = %.16e, k3 = %.16e , k4 = %.16e, k5
       =
       %.16e, k5l = %.16e\n",coeffs->k0,
    ...
    // if ( (coeffs->tidal1->lambda2Tidal != 0. && coeffs->tidal1->omega02Tidal
    // != 0.) || (coeffs->tidal2->lambda2Tidal != 0. &&
    // coeffs->tidal2->omega02Tidal
    // != 0.) ) {
    //     deltaU += XLALSimIMRTEOBdeltaUTidal(u, eta, coeffs->tidal1,
    //     coeffs->tidal2);
    // }
...
/**
 * This function calculates the function \f$\Delta_r(r)\f$ which appears in the
 * spinning EOB potential function. Eqs. 10a and 10b
 */
REAL8
 ...
 * Can NOT be used in precessing cases. This omega is defined as
 * \f$\dot{y}/r\f$ by setting \f$y=0\f$. The function calculates omega = v/r,
 * by first converting (r,phi,pr,pphi) to Cartesian coordinates in which
 * rVec={r,0,0} and pVec={0,pphi/r,0}, i.e. the effective-test-particle is
 * positioned at x=r, and its velocity along y-axis. Then it computes omega,
 * which is now given by dydt/r = (dH/dp_y)/r.
 */
REAL8
...
/**
 * Function to calculate the non-Keplerian coefficient for the spin-aligned EOB
 * model. radius \f$r\f$ times the cuberoot of the returned number is
 * \f$r_\Omega\f$ defined in Eq. A2. i.e. the function returns \f$(r_{\Omega} /
 * r)^3\f$.
 */
REAL8
 ...
 */
int XLALSpinAlignedHcapDerivative(double t,             /**< UNUSED */
...
)
{
    ...
    {
        params.varyParam = i;
        gslStatus = gsl_deriv_central(&F, cartValues[i], STEP_SIZE, &tmpDValues[i], &absErr);
    ...
    H = EOBHamiltonian(eta, &rVec, &pVec, s1Vec, s2Vec, sKerr, sStar, params.params->tortoise,
                       params.params->seobCoeffs);
    // t_p2 = clock();
    params.params->cache[0] = H;
    // print_debug( "invcsi = %.16e, csi = %.16e, ham = %.16e ( tortoise =
    ...
    /*if ( values[0] > 1.3 && values[0] < 3.9 ) printf( "Cartesian
       derivatives:\n%f %f %f %f %f %f\n", tmpDValues[3], tmpDValues[4],
       tmpDValues[5], -tmpDValues[0], -tmpDValues[1], -tmpDValues[2] );*/

    /* Now calculate omega, and hence the flux */
    omega = tmpDValues[4] / r;
    flux = XLALInspiralSpinFactorizedFlux(&polarDynamics, &cartDynamics, nqcCoeffs, omega, csi * tmpDValues[3],
                                          omega * r, params.params, H / (mass1 + mass2), lMax);
    // t_p3 = clock();
    /* Looking at the non-spinning model, I think we need to divide the flux by
     * eta */
    flux = flux / eta;
    ...
    dvalues[1] = omega;
    /* Note: in this special coordinate setting, namely y = z = 0, dpr/dt =
     * dpx/dt
     * + dy/dt * py/r, where py = pphi/r */
    dvalues[2] = -tmpDValues[0] + tmpDValues[4] * values[3] / (r * r);
    // CORRECTIONFR
    // print_debug("d0 = %.16e, d1 = %.16e, d2 = %.16e\n", dvalues[0],
    // dvalues[1], dvalues[2]); print_debug("values = (%.16e, %.16e, %.16e,
    // %.16e)\n", values[0], values[1], values[2], values[3]);
    // print_debug("cartvalues =
    // (%.16e, %.16e, %.16e, %.16e, %.16e, %.16e)\n",
    //         cartValues[0], cartValues[1], cartValues[2], cartValues[3],
    //         cartValues[4], cartValues[5]);

#if 1
    REAL8 cFr, cFf, prDot;
    // CalculateEccCorrectionToFlux(eta, params.params->chi1,
    // params.params->chi1, r, dvalues[0], dvalues[2], &cFr, &cFf);
    prDot = dvalues[2] - (values[2] / values[3] / csi) * flux / omega;
    CalculateEccCorrectionToFluxV3(eta, params.params->chi1, params.params->chi2, r, dvalues[0], prDot, &cFr, &cFf,
                                   params.params->e0);
    dvalues[2] = dvalues[2] * csi - (values[2] / values[3]) * flux * cFr / omega;
    dvalues[3] = -flux * cFf / omega;
    // print_debug("r = %.16e, dr = %.16e, prDot = %.16e\n", r, dvalues[0],
    // prDot); print_debug("Old:cFr = %.16e, cFf = %.16e, flux = %.16e\n", cFr,
    // cFf, flux);
#else
    ...
    // te = clock();
    // print_debug("Time Cost Old: t_p1 = %.16e, t_p12 = %.16e, t_p23 = %.16e,
    // t_p3e = %.16e\n",
...
int XLALSpinAlignedHcapDerivative_inverse(double t,             /**< UNUSED */
                                          const REAL8 values[], /**< dynamical varables */
                                          REAL8 dvalues[],      /**< time derivative of dynamical variables */
                                          void *funcParams      /**< EOB parameters */
)
...
int XLALSpinAlignedHcapDerivative_SA(double t,             /**< UNUSED */
                                     const REAL8 values[], /**< dynamical varables */
                                     REAL8 dvalues[],      /**< time derivative of dynamical variables */
                                     void *funcParams      /**< EOB parameters */
)
{
    ...
    /* Calculate the potential functions and the tortoise coordinate factor csi,
        given by Eq. 28 of Pan et al. PRD 81, 084041 (2010) */
    // DeltaT = XLALSimIMRSpinEOBHamiltonianDeltaT( params.params->seobCoeffs, r,
    // eta, a ); DeltaR = XLALSimIMRSpinEOBHamiltonianDeltaR(
    // params.params->seobCoeffs, r, eta, a ); csi    = sqrt( DeltaT * DeltaR ) /
    // (r*r + a*a);
    // printf("DeltaT = %.16e, DeltaR = %.16e, a = %.16e\n",DeltaT,DeltaR,a);
    // print_debug( "csi in derivatives function = %.16e\n", 1./csi );
    ...
    // t_p1 = clock();
    /* Calculate Hamiltonian using Cartesian vectors rVec and pVec */
    // H =  EOBHamiltonian( eta, &rVec, &pVec, s1Vec, s2Vec, sKerr, sStar,
    // params.params->tortoise, params.params->seobCoeffs );
    ...
    // CORRECTIONFR
// print_debug("d0 = %.16e, d1 = %.16e, d2 = %.16e\n", dvalues[0], dvalues[1],
// dvalues[2]); print_debug("values = (%.16e, %.16e, %.16e, %.16e)\n",
// values[0], values[1], values[2], values[3]); print_debug("cartvalues =
// (%.16e, %.16e, %.16e, %.16e, %.16e, %.16e)\n",
//         cartValues[0], cartValues[1], cartValues[2], cartValues[3],
//         cartValues[4], cartValues[5]);
// CORRECTIONFR
    ...
    CalculateEccCorrectionToFluxV3(eta, params.params->chi1, params.params->chi2, r, dvalues[0], prDot, &cFr, &cFf,
                                   params.params->e0);
    // CalculateEccCorrectionToFluxV3X(r, dvalues[0], prDot, &cFr, &cFf,
    // params.params->e0, params.params->eccCoeffs);
    dvalues[2] = dvalues[2] * csi - (values[2] / values[3]) * flux * cFr / omega;
    dvalues[3] = -flux * cFf / omega;
    // print_debug("r = %.16e, dr = %.16e, prDot = %.16e\n", r, dr, prDot);
...
int XLALSpinAlignedHcapDerivative_SA_inverse(double t,             /**< UNUSED */
                                             const REAL8 values[], /**< dynamical varables */
                                             REAL8 dvalues[],      /**< time derivative of dynamical variables */
                                             void *funcParams      /**< EOB parameters */
)
...
int XLALSpinAlignedHcapDerivative_SAConserve(double t,             /**< UNUSED */
                                             const REAL8 values[], /**< dynamical varables */
                                             REAL8 dvalues[],      /**< time derivative of dynamical variables */
                                             void *funcParams      /**< EOB parameters */
)
{
    ...
    /* Now calculate omega, and hence the flux */
    omega = tmpDValues[4] / r;
    // flux  = XLALInspiralSpinFactorizedFlux( &polarDynamics, &cartDynamics,
    // nqcCoeffs, omega, csi * tmpDValues[3], omega*r, params.params,
    // H/(mass1+mass2), lMax );
    // t_p3 = clock();
    /* Looking at the non-spinning model, I think we need to divide the flux by
     * eta */
...
int XLALSpinAlignedHcapDerivative_Conserve(double t,             /**< UNUSED */
                                           const REAL8 values[], /**< dynamical varables */
                                           REAL8 dvalues[],      /**< time derivative of dynamical variables */
                                           void *funcParams      /**< EOB parameters */
)
{
    ...
    // params.params->tortoise ); exit(1); if ( values[0] > 1.3 && values[0]
    // < 3.9 ) printf( "r = %e\n", values[0] ); if ( values[0] > 1.3 && values[0]
    // < 3.9 ) printf( "Hamiltonian = %e\n", H );
    H = H * (mass1 + mass2);
    ...
    if (isnan(dvalues[0]) || isnan(dvalues[1]) || isnan(dvalues[2]) || isnan(dvalues[3]))
    {
        ...
        print_debug("DValues = (%.16e, %.16e, %.16e)\n\t(%.16e, %.16e, %.16e)\n", tmpDValues[0], tmpDValues[1],
                    tmpDValues[2], tmpDValues[3], tmpDValues[4], tmpDValues[5]);
        return 1;
    }
    return CEV_SUCCESS;
}

int XLALSpinAlignedHcapDerivative_invConserve(double t,             /**< UNUSED */
                                              const REAL8 values[], /**< dynamical varables */
                                              REAL8 dvalues[],      /**< time derivative of dynamical variables */
                                              void *funcParams      /**< EOB parameters */
)
...
REAL8
XLALSimIMRSpinPrecEOBHamiltonian(const REAL8 eta,        /**<< Symmetric mass ratio */
                                 ...
                                 INT4 tortoise,          /**<< flag to state whether the momentum is the tortoise
                                                            co-ord */
                                 ...
                                 HyperParams *hParams)
{
    ...
    //     (int) tortoise ); PRINT_LOG_INFO(LOG_CRITICAL, "x =
    //     %.16e\t%.16e\t%.16e\n", x->data[0], x->data[1], x->data[2] );
    //     PRINT_LOG_INFO(LOG_CRITICAL, "p = %.16e\t%.16e\t%.16e\n", p->data[0],
    //     p->data[1], p->data[2] ); PRINT_LOG_INFO(LOG_CRITICAL, "sStar =
    //     %.16e\t%.16e\t%.16e\n", sigmaStar->data[0],
    ...
    cross_product3d(x->data, p->data,
                    L); // Note that L = r x p is invariant under tortoise transform
    ...
    if (UsePrecH && coeffs->updateHCoeffs)
    {
        ...
        tmpa = sqrt(sigmaKerr->data[0] * sigmaKerr->data[0] + sigmaKerr->data[1] * sigmaKerr->data[1] +
                    sigmaKerr->data[2] * sigmaKerr->data[2]);

        // Update coefficients, checking for errors
        // if (coeffs->SpinAlignedEOBversion ==4)
        // {
        if (XLALSimIMRCalculateSpinPrecEOBHCoeffs_v2(&tmpCoeffs, eta, tmpa, chi, 4, hParams) == CEV_FAILURE)
        {
            return REAL8_FAIL_NAN;
        }
        // }
    ...
    const REAL8 invlog_2e = 0.69314718055994530941723212145817656807550013436026;
    logu = log2(u) * invlog_2e;
    const REAL8 logarg = coeffs->k1 * u + coeffs->k2 * u2 + coeffs->k3 * u3 + coeffs->k4 * u4 + coeffs->k5 * u5 +
                         coeffs->k5l * u5 * logu;
    ...
    /* ddeltaU/du */
    deltaU_u =
        2. * (invm1PlusetaKK + a2 * u) * logTerms +
        bulk *
            (eta * (coeffs->k1 +
                    u * (2. * coeffs->k2 +
                         u * (3. * coeffs->k3 + u * (4. * coeffs->k4 + 5. * (coeffs->k5 + coeffs->k5l * logu) * u))))) /
            (1. + logarg);
    /* ddeltaT/dr */
    deltaT_r = 2. * r * deltaU - deltaU_u;
    /* Eq. 5.39 of BB1 */
    ...
    // XLAL_PRINT_INFO( "D = %.16e, ww = %.16e, rho = %.16e, Lambda = %.16e, xi =
    // %.16e\npr = %.16e, pf = %.16e, deltaR = %.16e, deltaT = %.16e\n",
    ...
    /* Note that the tortoise prT appears only in the quartic term, explained in
     * Eqs. 14 and 15 of Tarrachini et al. */
    Hns = sqrt((1. + ((prT * prT) * (prT * prT)) * qq * u2 + ptheta2 * invrho2 + pf * pf * rho2 * invLambda * invxi2 +
                pr * pr * deltaR * invrho2) *
               (rho2 * deltaT) * invLambda) +
          pf * ww * invLambda;
    // if(IS_DEBUG){
    // print_debug( "term 1 in Hns: %.16e\n",  prT*prT*prT*prT*qq*u2 );
    ...
    // if(debugPK){
    //     XLAL_PRINT_INFO( "Q = %.16e, pvr = %.16e, xi2 = %.16e , deltaT =
    //     %.16e, rho2 = %.16e, Lambda = %.16e, pxir = %.16e, B = %.16e\n", Q,
    //     pvr, xi2, deltaT, rho2, Lambda, pxir, B );
    // }
    ...
    /* Eq. 5.68 of BB1, (YP) simplified for aa=bb=0. */
    deltaSigmaStar_x = eta *
                       ((-8. - 3. * r * (12. * pn2 - pp)) * sKerr_x + (14. + (-30. * pn2 + 4. * pp) * r) * sStar_x) *
                       (1. / 12.) * u;

    deltaSigmaStar_y = eta *
                       ((-8. - 3. * r * (12. * pn2 - pp)) * sKerr_y + (14. + (-30. * pn2 + 4. * pp) * r) * sStar_y) *
                       (1. / 12.) * u;

    deltaSigmaStar_z = eta *
                       ((-8. - 3. * r * (12. * pn2 - pp)) * sKerr_z + (14. + (-30. * pn2 + 4. * pp) * r) * sStar_z) *
                       (1. / 12.) * u;

    /* Now compute the additional 3.5PN terms. */
    /* Eq. 52 of BB2, (YP) simplified for zero gauge parameters */
    // RH: below is horner(%, [eta,r])
    // sMultiplier1 = -(2.*eta*(-353. + 27.*eta) + 2.*(103.*eta
    // - 60.*eta*eta)*pp*r
    //             + (120.*(-3.))*(eta*eta)*(pn2*pn2)*(r*r) + (eta*(23.
    //             + 3.*eta))*(pp*pp)*(r*r )
    //             + 6.*pn2*r*(- 47.*eta + 54.*(eta*eta) + (- 16.*eta
    //             + 21.*(eta*eta))*pp*r))
    //             * (1./72.) * u2;
    sMultiplier1 =
        (-706.0 + (206.0 * pp - 282.0 * pn2 + (-96.0 * pn2 * pp + 23.0 * pp * pp) * r) * r +
         (54.0 + (-120.0 * pp + 324.0 * pn2 + (-360.0 * pn2 * pn2 + 126.0 * pn2 * pp + 3.0 * pp * pp) * r) * r) * eta) *
        eta * u2 * (-1. / 72.0);
    /* Eq. 52 of BB2, (YP) simplified for zero gauge parameters */
    // RH: below is horner(expand(%), [eta,r])
    //  sMultiplier2 = (-16.*(7.*eta*(8. + 3.*eta)) + 4.*(- 109.*eta
    //  + 51.*eta*eta)*pp*r
    //              + 810.*(eta*eta)*(pn2*pn2)*(r*r) - 45.*eta*(pp*pp)*(r*r)
    //              - 6.*pn2*r*(16.*eta + 147.*eta*eta + (- 6.*eta
    //              + 39.*(eta*eta))*pp*r))
    //              * (1./144.) * u2;
    sMultiplier2 =
        (-56.0 / 9.0 * u2 +
         (-2.0 / 3.0 * pn2 * u2 - 109.0 / 36.0 * pp * u2 + (pn2 * pp * u2 / 4.0 - 5.0 / 16.0 * pp * pp * u2) * r) * r +
         (-7.0 / 3.0 * u2 + (-49.0 / 8.0 * pn2 * u2 + 17.0 / 12.0 * pp * u2 +
                             (45.0 / 8.0 * pn2 * pn2 * u2 - 13.0 / 8.0 * pn2 * pp * u2) * r) *
                                r) *
             eta) *
        eta;
    ...
    Hwr = ((invexpMU * invexpMU * invexpMU * invexpnu) * sqrtdeltaR *
           ((expMU * expMU) * (expnu * expnu) * (pxir * pxir) * sv - B * (expMU * expnu) * pvr * pxir * sxi +
            B * B * xi2 * ((expMU * expMU) * (sqrtQ + Q) * sv + pn * pvr * sn * sqrtdeltaR - pn * pn * sv * deltaR))) *
          inv2B1psqrtQsqrtQ * invxi2;
    /* Eq. 3.45 of BB1, third term */
    Hwcos = ((invexpMU * invexpMU * invexpMU * invexpnu) *
             (sn * (-((expMU * expMU) * (expnu * expnu) * (pxir * pxir)) +
                    B * B * (pvr * pvr - (expMU * expMU) * (sqrtQ + Q) * xi2)) -
              B * pn * (B * pvr * sv - (expMU * expnu) * pxir * sxi) * sqrtdeltaR)) *
            inv2B1psqrtQsqrtQ;
    /* Eq. 3.44 of BB1, leading term */
    HSOL = ((expnu * expnu * invexpMU) * (-B + (expMU * expnu)) * pxir * s3) / (deltaT * sqrtQ) * invxi2;
    /* Eq. 3.44 of BB1, next-to-leading term */
    HSONL = ((expnu * (invexpMU * invexpMU)) *
             (-(B * expMU * expnu * nucos * pxir * (1. + 2. * sqrtQ) * sn * xi2) +
              (-(BR * (expMU * expnu) * pxir * (1. + sqrtQ) * sv) +
               B * ((expMU * expnu) * nur * pxir * (1. + 2. * sqrtQ) * sv + B * mur * pvr * sxi +
                    B * sxi * (-(mucos * pn * xi2) + sqrtQ * (mur * pvr - nur * pvr + (-mucos + nucos) * pn * xi2)))) *
                  sqrtdeltaR)) *
            invxi2 / (deltaT * (sqrtQ + Q));
    /* Eq. 3.43 and 3.45 of BB1 */
    Hs = w * s3 + Hwr * wr + Hwcos * wcos + HSOL + HSONL;
    /* Eq. 5.70 of BB1, last term */
    Hss = -0.5 * u3 * (sx * sx + sy * sy + sz * sz - 3. * sn * sn);
    ...
    if (isnan(Hreal))
    {
        // print_debug("tmp1 = %.16e, tmp2 = %.16e, tmp3 = %.16e, tmp4 =
        // %.16e\n",(invexpMU*invexpMU*invexpMU*invexpnu)*sqrtdeltaR*inv2B1psqrtQsqrtQ*invxi2,
        //     ((expMU*expMU)*(expnu*expnu)*(pxir*pxir)*sv), (-
        //     B*(expMU*expnu)*pvr*pxir*sxi),(B*B*xi2*((expMU*expMU)*(sqrtQ +
        //     Q)*sv
        //     + pn*pvr*sn*sqrtdeltaR - pn*pn*sv*deltaR)));
        PRINT_LOG_INFO(LOG_DEBUG, "Inside Hamiltonian: Hreal is a NAN. "
                                  "Printing its components below:");
        ...
        PRINT_LOG_INFO(LOG_DEBUG, "sStar = %.16e\t%.16e\t%.16e", sigmaStar->data[0], sigmaStar->data[1],
                       sigmaStar->data[2]);
        PRINT_LOG_INFO(LOG_DEBUG, "sKerr = %.16e\t%.16e\t%.16e", sigmaKerr->data[0], sigmaKerr->data[1],
                       sigmaKerr->data[2]);
        PRINT_LOG_INFO(LOG_DEBUG,
                       "csi = %.16e, Q = %.16e, pvr = %.16e, xi2 = %.16e , deltaT = %.16e, "
                       "rho2 = %.16e, Lambda = %.16e, pxir = %.16e, B = %.16e",
                       csi, Q, pvr, xi2, deltaT, rho2, Lambda, pxir, B);
        PRINT_LOG_INFO(LOG_DEBUG,
                       "alpha = %.16e, beta_f = %.16e, gamma_rr = %.16e, "
                       "gamma_thth = %.16e, gamma_ff = %.16e",
                       sqrt((rho2 * deltaT) * invLambda), ww * invLambda, deltaR * invrho2, invrho2,
                       rho2 * invLambda * invxi2);
        ...
        PRINT_LOG_INFO(LOG_DEBUG, "(deltaU, bulk, logTerms, log arg) = (%.16e, %.16e, %.16e, %.16e)", deltaU, bulk,
                       logTerms,
                       1. + coeffs->k1 * u + coeffs->k2 * u2 + coeffs->k3 * u3 + coeffs->k4 * u4 + coeffs->k5 * u5 +
                           coeffs->k5l * u5 * logu);
        ...
        PRINT_LOG_INFO(LOG_DEBUG,
                       "D = %.16e, ww = %.16e, rho = %.16e, Lambda = %.16e, xi "
                       "= %.16e pr = "
                       "%.16e, pf = %.16e, deltaR = %.16e, deltaT = %.16e",
                       D, ww, sqrt(rho2), Lambda, sqrt(xi2), pr, pf, deltaR, deltaT);
        ...
        PRINT_LOG_INFO(LOG_DEBUG,
                       "deltaSigmaStar_x = %.16e, deltaSigmaStar_y = %.16e, "
                       "deltaSigmaStar_z = %.16e",
                       deltaSigmaStar_x, deltaSigmaStar_y, deltaSigmaStar_z);
        ...
        PRINT_LOG_INFO(LOG_DEBUG,
                       "BR = %.16e, sv = %.16e, sn = %.16e, sxi = %.16e, nur = "
                       "%.16e, mur = %.16e, nucos = %.16e, mucos = %.16e\n",
...
GSLSpinPrecHamiltonianWrapper(double x, void *params)
{
    ...
    /* Calculate various spin parameters */
    /* Note that XLALSimIMRSpinEOBCalculateSigmaKerr and
     * XLALSimIMRSpinEOBCalculateSigmaStar return a unitless quantity */
    EOBCalculateSigmaKerr(&sigmaKerr, &spin1norm, &spin2norm);
    EOBCalculateSigmaStar(&sigmaStar, m1, m2, &spin1norm, &spin2norm);
    a = sqrt(sigmaKerr.data[0] * sigmaKerr.data[0] + sigmaKerr.data[1] * sigmaKerr.data[1] +
             sigmaKerr.data[2] * sigmaKerr.data[2]);
    ...
    REAL8 SpinEOBH = 0.0;
    SpinEOBH = XLALSimIMRSpinPrecEOBHamiltonian(seobParams->eta, &r, &p, &spin1norm, &spin2norm, &sigmaKerr, &sigmaStar,
                                                dParams->params->tortoise, dParams->params->seobCoeffs,
                                                dParams->params->hParams) /
               seobParams->eta;
...
REAL8
XLALSpinPrecHcapNumDerivWRTParam(const INT paramIdx,       /**<< Index of the parameters */
                                 const REAL8 values[],     /**<< Dynamical variables */
                                 SpinEOBParams *funcParams /**<< EOB Parameters */
)
{
...
int XLALSpinPrecHcapNumericalDerivative(double t,             /**<< UNUSED */
                                        const REAL8 values[], /**<< Dynamical variables */
...
{
    // int		debugPK = 0;
    /** lMax: l index up to which h_{lm} modes are included in the computation of
     * the GW enegy flux: see Eq. in 13 in PRD 86,  024011 (2012) */
    static const INT lMax = 8;
     ...
     * things
     */
    /* Here pvec is the reduced tortoise p^* vector of Pan et al. PRD 81, 084041
     * (2010) */
    rVec.length = pVec.length = 3;
    rVec.data = rData;
    pVec.data = pData;
    ...
    m1PlusetaKK = -1. + eta * coeffs->KK;
    /* Eq. 5.75 of BB1 */
    /* a as returned by XLALSimIMRSpinEOBCalculateSigmaKerr is S/M^2 so that a is
     * unitless, i.e. 1/M^2 is absorbed in a2. Also, u = M/|r| is unitless */
    bulk = 1. / (m1PlusetaKK * m1PlusetaKK) + (2. * u) / m1PlusetaKK + a2 * u2;
    /* Eq. 5.73 of BB1 */
    /* The 4PN term with coefficients k5 and k5l are defined in the SEOBNRv2
     * review document https://dcc.ligo.org/T1400476 */
    logTerms = 1. + eta * coeffs->k0 +
               eta * log(1. + coeffs->k1 * u + coeffs->k2 * u2 + coeffs->k3 * u3 + coeffs->k4 * u4 + coeffs->k5 * u5 +
                         coeffs->k5l * u5 * log(u));
    /* Eq. 5.73 of BB1 */
    ...
    for (i = 0; i < 3; i++)
        for (j = 0; j <= i; j++)
        {
            Tmatrix[i][j] = Tmatrix[j][i] = (rVec.data[i] * rVec.data[j] / rMag2) * (csi - 1.);

            invTmatrix[i][j] = invTmatrix[j][i] = -(csi - 1) / csi * (rVec.data[i] * rVec.data[j] / rMag2);
    ...
    /* This is dcsi/dr: this is needed in the last term of Eq. A5 of PRD 81,
     * 084041 (2010) */
    dcsi = csi * (2. / rMag + deltaU_r / deltaU) +
           csi * csi * csi / (2. * rMag2 * rMag2 * deltaU * deltaU) * (rMag * (-4. * w2) / D - eobD_r * (w2 * w2));
    ...
    for (i = 0; i < 3; i++)
        for (j = 0; j < 3; j++)
            ...
            {
                dTijdXk[i][j][k] =
                    (rVec.data[i] * KRONECKER_DELTA(j, k) + KRONECKER_DELTA(i, k) * rVec.data[j]) * (csi - 1.) / rMag2 +
    ...
    for (i = 0; i < 3; i++)
    {
        ...
        /* Note that in the 1st term on the last line of Eq. A5 of PRD 81, 084041
         * (2010) one needs dH/dX @ fixed P, not P^*, hence the need for what
         * follows  */
        gslStatus = gsl_deriv_central(&F, values[i], STEP_SIZE, &tmpDValues[i], &absErr);
        params.values = values;
    ...
    for (i = 6; i < 9; i++)
    {
        params.varyParam = i;
        params.params->seobCoeffs->updateHCoeffs = 1;
        gslStatus = gsl_deriv_central(&F, values[i], STEP_SIZE * mass1 * mass1, &tmpDValues[i], &absErr);
        if (gslStatus != GSL_SUCCESS)
        {
            PRINT_LOG_INFO(LOG_CRITICAL, "Failure in GSL function");
            return CEV_FAILURE;
    ...
    for (i = 9; i < 12; i++)
    {
        params.varyParam = i;
        params.params->seobCoeffs->updateHCoeffs = 1;
        gslStatus = gsl_deriv_central(&F, values[i], STEP_SIZE * mass2 * mass2, &tmpDValues[i], &absErr);
        if (gslStatus != GSL_SUCCESS)
        {
            PRINT_LOG_INFO(LOG_CRITICAL, "Failure in GSL function");
            return CEV_FAILURE;
    ...
    /* rVectorDot */
    // Eq. A4 of PRD 81, 084041 (2010).  Note that dvalues[i] = \dot{X^i} but
    // tmpDValues[j] = dH/dvalues[j]
    for (i = 0; i < 3; i++)
    ...
    r = polarDynamics.data[0] = sqrt(values[0] * values[0] + values[1] * values[1] + values[2] * values[2]);
    polarDynamics.data[1] = 0;
    /* Note that poldata[2] and poldata[3] differ from v2 in which one is
     * normalized by polData[0]. Behaviour is reverted. */
    polarDynamics.data[2] = (values[0] * values[3] + values[1] * values[4] + values[2] * values[5]) / polData[0];
    ...
    // magY = sqrt(dvalues[0]*dvalues[0] + dvalues[1]*dvalues[1] +
    // dvalues[2]*dvalues[2]);
    ...
    // c1y = sqrt( (s1.data[0]*s1.data[0] + s1.data[1]*s1.data[1] +
    // s1.data[2]*s1.data[2])/(m1sq*m1sq) - c1x*c1x - c1z*c1z);
    ...
    // c2y = sqrt( (s2.data[0]*s2.data[0] + s2.data[1]*s2.data[1] +
    // s2.data[2]*s2.data[2])/(m2sq*m2sq) - c2x*c2x - c2z*c2z);
    ...
    if (SpinAlignedEOBversion == 4)
    {
        s1dotLN = (s1.data[0] * Lhatx + s1.data[1] * Lhaty + s1.data[2] * Lhatz) / (mass1 * mass1);
        s2dotLN = (s2.data[0] * Lhatx + s2.data[1] * Lhaty + s2.data[2] * Lhatz) / (mass2 * mass2);
    }
    else
    {
    ...
    if (sKerr_norm > 1e-6)
    {
        ...
        S_con /= (1 - 2 * eta);
        S_con += (inner_product3d(S1_perp, sKerr.data) + inner_product3d(S2_perp, sKerr.data)) / sKerr_norm /
                 (1 - 2 * eta) / 2.;
    }
    REAL8 chi = S_con;
#if 0
    ...
    case 4:
        ...
        break;
    default:
        PRINT_LOG_INFO(LOG_INFO, "Unknown SEOBNR version! At present only v1 "
    ...
    {
        if (CODE_VERSION == 3)
            EccPrec_CalcSpinPrecFacWaveformCoefficients(params.params->hCoeffs, mass1, mass2, eta, tplspin, chiS, chiA,
                                                        csx, csy, csz, cax, cay, caz, 451);
        else
            XLALSimIMREOBCalcSpinPrecFacWaveformCoefficients(params.params->hCoeffs, mass1, mass2, eta, tplspin, chiS,
                                                             chiA, SpinAlignedEOBversion);
    }
    else
    {
        /* This is needed because SpinAlignedEOBversion is set to 2 for v3 */
        ...
        // if ( params.params->use_hm )
        // SpinAlignedEOBversionForWaveformCoefficients = 451; else
        // SpinAlignedEOBversionForWaveformCoefficients = SpinAlignedEOBversion;
        if (CODE_VERSION == 3)
            EccPrec_CalcSpinPrecFacWaveformCoefficients(params.params->hCoeffs, mass1, mass2, eta, tplspin, chiS, chiA,
                                                        csx, csy, csz, cax, cay, caz, 451);
            ...
            XLALSimIMREOBCalcSpinPrecFacWaveformCoefficients(params.params->hCoeffs, mass1, mass2, eta, tplspin, chiS,
                                                             chiA, SpinAlignedEOBversionForWaveformCoefficients);
    }
    // if (SpinAlignedEOBversion == 4)
    // {
    XLALSimIMRCalculateSpinPrecEOBHCoeffs_v2(params.params->seobCoeffs, eta, a, chi, SpinAlignedEOBversion,
                                             params.params->hParams);
    // }
    // else
    // {
    // 	XLALSimIMRCalculateSpinPrecEOBHCoeffs(params.params->seobCoeffs, eta,
    // a, 										  SpinAlignedEOBversion);
    // }

    H = XLALSimIMRSpinPrecEOBHamiltonian(eta, &rVec, &pVec, &s1norm, &s2norm, &sKerr, &sStar, params.params->tortoise,
                                         params.params->seobCoeffs, params.params->hParams);
    ...
    {
        flux = XLALInspiralPrecSpinFactorizedFlux(&polarDynamics, &cartDynamics, nqcCoeffs, omega, dr, ncrv,
                                                  params.params, H / (mass1 + mass2), lMax, SpinAlignedEOBversion);
    }
    ...
    rrTerm2 = 8. / 15. * eta * eta * pow(omega, 8. / 3.) / (magL * magL * r) *
              ((61. + 48. * mass2 / mass1) * pDotS1 + (61. + 48. * mass1 / mass2) * pDotS2);
    ...
    dLx = dvalues[1] * values[5] - dvalues[2] * values[4] + values[1] * dvalues[5] - values[2] * dvalues[4];

    dLy = dvalues[2] * values[3] - dvalues[0] * values[5] + values[2] * dvalues[3] - values[0] * dvalues[5];

    dLz = dvalues[0] * values[4] - dvalues[1] * values[3] + values[0] * dvalues[4] - values[1] * dvalues[3];
    ...
    /* These are ODEs for the phase that enters the h_{lm}: see Eq. 3.11 of PRD
     * 79, 104023 (2009) */
...
int XLALSpinPrecHcapNumericalDerivative_inverse(double t,             /**<< UNUSED */
                                                const REAL8 values[], /**<< Dynamical variables */
                                                REAL8 dvalues[],      /**<< Time derivatives of variables (returned) */
                                                void *funcParams      /**<< EOB parameters */
)
...
int XLALSpinPrecHcapNumericalDerivative_Conserve(double t,             /**<< UNUSED */
                                                 const REAL8 values[], /**<< Dynamical variables */
                                                 REAL8 dvalues[],      /**<< Time derivatives of variables (returned) */
                                                 void *funcParams      /**<< EOB parameters */
)
...
/**
 * Wrapper for GSL to call the Hamiltonian function. This is simply the
 * function GSLSpinPrecHamiltonianWrapper copied over. The alternative was to
 * make it non-static which increases runtime as static functions can be better
 * optimized.
 */
static double GSLSpinPrecHamiltonianWrapperFordHdpphi(double x, void *params)
{
    ...
    /* Calculate various spin parameters */
    EOBCalculateSigmaKerr(&sigmaKerr, &spin1norm, &spin2norm);
    EOBCalculateSigmaStar(&sigmaStar, seobParams->m1, seobParams->m2, &spin1norm, &spin2norm);
    a = sqrt(sigmaKerr.data[0] * sigmaKerr.data[0] + sigmaKerr.data[1] * sigmaKerr.data[1] +
             sigmaKerr.data[2] * sigmaKerr.data[2]);
    if (isnan(a))
    {
        PRINT_LOG_INFO(LOG_CRITICAL, "a is nan in GSLSpinPrecHamiltonianWrapperFordHdpphi !!");
        PRINT_LOG_INFO(LOG_CRITICAL, "rpolar, ppolar = %3.10f %3.10f %3.10f %3.10f %3.10f %3.10f", rpolar[0], rpolar[1],
                       rpolar[2], ppolar[0], ppolar[1], ppolar[2]);
        PRINT_LOG_INFO(LOG_CRITICAL, "rcart, pcart = %3.10f %3.10f %3.10f %3.10f %3.10f %3.10f", rcart[0], rcart[1],
                       rcart[2], pcart[0], pcart[1], pcart[2]);
        PRINT_LOG_INFO(LOG_CRITICAL, "a = nan");
        return CEV_FAILURE;
        ;
    }
    REAL8 SpinEOBH = XLALSimIMRSpinPrecEOBHamiltonian(seobParams->eta, &r, &p, &spin1norm, &spin2norm, &sigmaKerr,
                                                      &sigmaStar, dParams->params->tortoise,
                                                      dParams->params->seobCoeffs, dParams->params->hParams) /
                     seobParams->eta;
...
static double GSLSpinPrecHamiltonianWrapperForRvecDerivs(double x, void *params)
{
    ...
    //     for( i =0; i < 12; i++)
    //     if( isnan(tmpVec[i]) ) {
    //         XLAL_PRINT_INFO("GSLSpinPrecHamiltonianWrapperForRvecDerivs (from
    //         input)::tmpVec %3.10f %3.10f %3.10f %3.10f %3.10f %3.10f %3.10f
    //         %3.10f %3.10f %3.10f %3.10f %3.10f\n", tmpVec[0], tmpVec[1],
    //         tmpVec[2], tmpVec[3], tmpVec[4], tmpVec[5], tmpVec[6], tmpVec[7],
    //         tmpVec[8], tmpVec[9], tmpVec[10], tmpVec[11]);
    //         }
    //     }
    ...
    /* Calculate various spin parameters */
    XLALSimIMRSpinEOBCalculateSigmaKerr(&sigmaKerr, seobParams->m1, seobParams->m2, &spin1, &spin2);
    XLALSimIMRSpinEOBCalculateSigmaStar(&sigmaStar, seobParams->m1, seobParams->m2, &spin1, &spin2);
    a = sqrt(sigmaKerr.data[0] * sigmaKerr.data[0] + sigmaKerr.data[1] * sigmaKerr.data[1] +
             sigmaKerr.data[2] * sigmaKerr.data[2]);
    if (isnan(a))
    {
    ...
    // if(debugPK) {
    //     if(0 && magR < 1.96 * 1.96) {
    //     XLAL_PRINT_INFO("GSLSpinPrecHamiltonianWrapperForRvecDerivs (JUST
    //     inputs)::tmpVec %3.10f %3.10f %3.10f %3.10f %3.10f %3.10f %3.10f
    //     %3.10f %3.10f %3.10f %3.10f %3.10f\n", tmpVec[0], tmpVec[1],
    //     tmpVec[2], tmpVec[3], tmpVec[4], tmpVec[5], tmpVec[6], tmpVec[7],
    //     tmpVec[8], tmpVec[9], tmpVec[10], tmpVec[11]);
    ...
    if (isnan(SpinEOBH))
    {
        ...
        PRINT_LOG_INFO(LOG_CRITICAL,
                       "GSLSpinPrecHamiltonianWrapperForRvecDerivs (JUST "
                       "inputs)::tmpVec %.10e %.10e %.10e %.10e %.10e %.10e %.10e "
                       "%.10e %.10e %.10e %.10e %.10e",
                       tmpVec[0], tmpVec[1], tmpVec[2], tmpVec[3], tmpVec[4], tmpVec[5], tmpVec[6], tmpVec[7],
                       tmpVec[8], tmpVec[9], tmpVec[10], tmpVec[11]);
 ...
 */
int XLALSpinPrecHcapRvecDerivative(double t,             /**<< UNUSED */
                                   const REAL8 values[], /**<< Dynamical variables */
                                   REAL8 dvalues[],      /**<< Time derivatives of variables (returned) */
                                   void *funcParams      /**<< EOB parameters */
)
{
    ...
    // for(int i =0; i < 12; i++){
    //     if( isnan(values[i]) ) {
    //     XLAL_PRINT_INFO("XLALSpinPrecHcapRvecDerivative::values %3.10f %3.10f
    //     %3.10f %3.10f %3.10f %3.10f %3.10f %3.10f %3.10f %3.10f %3.10f
    //     %3.10f\n", values[0], values[1], values[2], values[3], values[4],
    //     values[5], values[6], values[7], values[8], values[9], values[10],
    //     values[11]);
    //         XLALPrintError( "XLAL Error - %s: nan in input values \n",
    //         __func__); XLAL_ERROR( XLAL_EINVAL );
    //     }

    //     if( isnan(dvalues[i]) ) {
    //     XLAL_PRINT_INFO("XLALSpinPrecHcapRvecDerivative::dvalues %3.10f %3.10f
    //     %3.10f %3.10f %3.10f %3.10f %3.10f %3.10f %3.10f %3.10f %3.10f
    //     %3.10f\n", dvalues[0], dvalues[1], dvalues[2], dvalues[3], dvalues[4],
    //     dvalues[5], dvalues[6], dvalues[7], dvalues[8], dvalues[9],
    ...
    for (i = 0; i < 3; i++)
        for (j = 0; j < 3; j++)
            ...
            {
                dTijdXk[i][j][k] =
                    (rData[i] * KRONECKER_DELTA(j, k) + KRONECKER_DELTA(i, k) * rData[j]) * (csi - 1.) / rMag2 +
    ...
    for (i = 0; i < 12; i++)
    {
        ...
        {
            PRINT_LOG_INFO(LOG_CRITICAL,
                           "values %3.10f %3.10f %3.10f %3.10f %3.10f %3.10f %3.10f "
                           "%3.10f %3.10f %3.10f %3.10f %3.10f",
        ...
        {
            PRINT_LOG_INFO(LOG_CRITICAL,
                           "dvalues %3.10f %3.10f %3.10f %3.10f %3.10f %3.10f %3.10f "
                           "%3.10f %3.10f %3.10f %3.10f %3.10f\n",
                           dvalues[0], dvalues[1], dvalues[2], dvalues[3], dvalues[4], dvalues[5], dvalues[6],
                           dvalues[7], dvalues[8], dvalues[9], dvalues[10], dvalues[11]);
    ...
    /* Now calculate derivatives w.r.t. each parameter */
    // RH: Check if components i=0..5 (position and momenta) do not change the a
    // parameter used for spin RH: this breaks the loop below for i>=6
    SpinEOBHCoeffs tmpCoeffs;
    {
        ...
        /* Calculate various spin parameters */
        XLALSimIMRSpinEOBCalculateSigmaKerr(&sigmaKerr, mass1, mass2, &spin1, &spin2);
        ...
        // if (SpinAlignedEOBversion == 4)
        // {
        if (XLALSimIMRCalculateSpinPrecEOBHCoeffs_v2(&tmpCoeffs, eta, tmpa, chi, SpinAlignedEOBversion,
                                                     params.params->hParams) == CEV_FAILURE)
        {
            return CEV_FAILURE;
        }
        // }
    ...
    //     {
    //         XLAL_PRINT_INFO("XLALSpinPrecHcapRvecDerivative (just after
    //         diff)::tmpDValues %3.10f %3.10f %3.10f %3.10f %3.10f %3.10f %3.10f
    //         %3.10f %3.10f %3.10f %3.10f %3.10f\n", tmpDValues[0],
    //         tmpDValues[1], tmpDValues[2], tmpDValues[3], tmpDValues[4],
    //         tmpDValues[5], tmpDValues[6], tmpDValues[7], tmpDValues[8],
    //         tmpDValues[9], tmpDValues[10], tmpDValues[11]);
    //     }
    // }
    ...
    s1dotL = (s1Data[0] * Lhatx + s1Data[1] * Lhaty + s1Data[2] * Lhatz) / (mass1 * mass1);
    s2dotL = (s2Data[0] * Lhatx + s2Data[1] * Lhaty + s2Data[2] * Lhatz) / (mass2 * mass2);
    ...
    c1x = (s1.data[0] * values[0] + s1.data[1] * values[1] + s1.data[2] * values[2]) / (r * m1sq);
    c1z =
        (s1.data[0] * rcrossrDot[0] + s1.data[1] * rcrossrDot[1] + s1.data[2] * rcrossrDot[2]) / (rcrossrDotMag * m1sq);
    // c1y = sqrt( (s1.data[0]*s1.data[0] + s1.data[1]*s1.data[1] +
    // s1.data[2]*s1.data[2])/(m1sq*m1sq) - c1x*c1x - c1z*c1z);
    c1y = (s1.data[0] * yhat[0] + s1.data[1] * yhat[1] + s1.data[2] * yhat[2]) / m1sq / magY;
    ...
    c2x = (s2.data[0] * values[0] + s2.data[1] * values[1] + s2.data[2] * values[2]) / (r * m2sq);
    c2z =
        (s2.data[0] * rcrossrDot[0] + s2.data[1] * rcrossrDot[1] + s2.data[2] * rcrossrDot[2]) / (rcrossrDotMag * m2sq);
    // c2y = sqrt( (s2.data[0]*s2.data[0] + s2.data[1]*s2.data[1] +
    // s2.data[2]*s2.data[2])/(m2sq*m2sq) - c2x*c2x - c2z*c2z);
    c2y = (s2.data[0] * yhat[0] + s2.data[1] * yhat[1] + s2.data[2] * yhat[2]) / m2sq / magY;
    ...
    {
        // if (params.params->alignedSpins==1)
        //     EccPrec_CalcSpinPrecFacWaveformCoefficients(params.params->hCoeffs,
        //     mass1, mass2, eta, tplspin,
        //                     chiS, chiA, csx, csy, csz, cax, cay, caz, 451);
        // else
        EccPrec_CalcSpinPrecFacWaveformCoefficients(params.params->hCoeffs, mass1, mass2, eta, tplspin, chiS, chiA, csx,
                                                    csy, csz, cax, cay, caz, 451);
    }
    else
    {
        if (params.params->alignedSpins == 1)
...
REAL8
XLALSimIMRSpinPrecEOBCalcOmega(const REAL8 values[],     /**<< Dynamical variables */
                               SpinEOBParams *funcParams /**<< EOB parameters */
)
{
    // int debugPK = 1;
    ...
    //     for(int i =0; i < 12; i++)
    //     if( isnan(values[i]) ) {
    //         XLAL_PRINT_INFO("XLALSimIMRSpinPrecEOBCalcOmega::values %3.10f
    //         %3.10f %3.10f %3.10f %3.10f %3.10f %3.10f %3.10f %3.10f %3.10f
    //         %3.10f %3.10f\n", values[0], values[1], values[2], values[3],
    //         values[4], values[5], values[6], values[7], values[8], values[9],
    //         values[10], values[11]); XLALPrintError( "XLAL Error - %s: nan in
    //         input values  \n", __func__); XLAL_ERROR( XLAL_EINVAL );
    //     }
    // }
    ...
    if (XLALSpinPrecHcapRvecDerivative(0, values, dvalues, (void *)funcParams) == CEV_FAILURE)
    {
        return REAL8_FAIL_NAN;
    }
    ...
    //     if( isnan(dvalues[ii]) ) {
    //         XLAL_PRINT_INFO("XLALSimIMRSpinPrecEOBCalcOmega::dvalues %3.10f
    //         %3.10f %3.10f %3.10f %3.10f %3.10f %3.10f %3.10f %3.10f %3.10f
    //         %3.10f %3.10f\n", dvalues[0], dvalues[1], dvalues[2], dvalues[3],
    //         dvalues[4], dvalues[5], dvalues[6], dvalues[7], dvalues[8],
    //         dvalues[9], dvalues[10], dvalues[11]); XLALPrintError( "XLAL Error
    //         - %s: nan in dvalues \n", __func__); XLAL_ERROR( XLAL_EINVAL );
    //     }
    // }
    ...
    polarvalues[4] = -inner_product3d(rvecprime_x_xhat_x_rvecprime, pvecprime) / polarvalues[0] / sin(polarvalues[1]);
    polarvalues[5] = -inner_product3d(rvecprime_x_xhat, pvecprime);

    /* ********************************************************************* */ /* Finally, Differentiate Hamiltonian
                                                                                   w.r.t. p_\phi, keeping p_r = 0 */
    /* ********************************************************************* */
    ...
    params.varyParam = 5;
    gslStatus = gsl_deriv_central(&F, polarvalues[5], STEP_SIZE, &omega, &absErr);
