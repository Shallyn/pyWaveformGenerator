 ...
 */
static int PrecStopConditionBasedOnPR(double t, const double values[], double dvalues[], void *funcParams)
{
    int debugPK = 0;
...
static int PrecStopConditionBasedOnPR_withfmax(double t, const double values[], double dvalues[], void *funcParams)
{
    int debugPK = 0;
    int debugPKverbose = 0;
    INT i;
...
static int PrecStopConditionBasedOnPR_inverse(double t, const double values[], double dvalues[], void *funcParams)
{
    int debugPK = 0;
    int debugPKverbose = 0;
    INT i;
...
void SEOBConcactInverseDynToAdaSDynPrec(REAL8Array **dyn_out, REAL8Array *dyn_inv, INT *retLen_out, INT retLen_inv)
{
...
INT SEOBIntegrateDynamics_prec_inverse(REAL8Array **dynamics, INT *retLenOut, REAL8Vector *ICvalues, REAL8 EPS_ABS,
                                       ...
                                       SpinEOBParams *seobParams, INT flagConstantSampling)
{
    ...
    integrator = XLALAdaptiveRungeKutta4Init(nb_Hamiltonian_variables, PrecHcapNumericalDerivative_inverse,
    ...
    // Prec
    retLen = XLALAdaptiveRungeKutta4NoInterpolateWithDerivPrec(integrator, seobParams, values->data, 0., tend - tstart,
...
INT SEOBIntegrateDynamics_prec_withFMax(REAL8Array **dynamics, INT *retLenOut, REAL8Vector *ICvalues, REAL8 EPS_ABS,
                                        REAL8 EPS_REL, REAL8 deltaT, REAL8 deltaT_min, REAL8 tstart, REAL8 tend,
                                        SpinEOBParams *seobParams, INT flagConstantSampling)
{
    INT retLen;
    ...
    if (tstart > 0)
    {
        integrator = XLALAdaptiveRungeKutta4Init(nb_Hamiltonian_variables, PrecHcapNumericalDerivative,
    ...
    if (!flagConstantSampling)
        retLen = XLALAdaptiveRungeKutta4NoInterpolateWithDerivPrec(integrator, seobParams, values->data, 0.,
                                                                   tend - tstart, deltaT, deltaT_min, dynamics);
    else
        retLen = XLALAdaptiveRungeKutta4WithDerivPrec(integrator, seobParams, values->data, 0., tend - tstart, deltaT,
                                                      deltaT_min, dynamics);
    if (retLen < 0)
    {
        PRINT_LOG_INFO(LOG_CRITICAL, "Integration Failed!!!");
...
INT SEOBIntegrateDynamics_prec(REAL8Array **dynamics, INT *retLenOut, REAL8Vector *ICvalues, REAL8 EPS_ABS,
                               REAL8 EPS_REL, REAL8 deltaT, REAL8 deltaT_min, REAL8 tstart, REAL8 tend,
                               SpinEOBParams *seobParams, INT flagConstantSampling)
{
    INT retLen;
...
INT SEOBComputeExtendedSEOBPrecdynamics(SEOBPrecdynamics **seobdynamics, REAL8Array *dynamics, INT retLen,
                                        SpinEOBParams *seobParams)
{
    ...
    // seobdyn->array->size, seobdyn->array->size/retLen); print_debug("sizeof
    // dyn = (%zu, %zu)\n", dynamics->dimLength->data[0],
    // dynamics->dimLength->data[1]);
    ...
    for (i = 0; i < retLen; i++)
    {
        ...
        seobdyn->s1dotZVec[i] =
            (rcrossp[0] * seobdyn->s1Vecx[i] + rcrossp[1] * seobdyn->s1Vecy[i] + rcrossp[2] * seobdyn->s1Vecz[i]) /
            magL;
        seobdyn->s2dotZVec[i] =
            (rcrossp[0] * seobdyn->s2Vecx[i] + rcrossp[1] * seobdyn->s2Vecy[i] + rcrossp[2] * seobdyn->s2Vecz[i]) /
            magL;
        // Debug:
        // print_debug("chi2 dot chi2 = %.16e\n", inner_product3d(chi2Vec,
        // chi2Vec));
    }
...
int SEOBInterpolatePrecDynamicsAtTime(REAL8Vector **seobdynamics_values, /**<< Output: pointer to vector for
...
)
{
    ...
    // print_debug("indext = %d, indexstart = %d, indexend = %d, interp_length =
    ...
    //     tVec[indexstart+i], seobdynamics->array->data[1 * retLen +
    //     indexstart]);
    // }
...
int SEOBPrecLocateTimePeakOmega(REAL8 *tPeakOmega,              /**<< Output: time of peak of Omega if found (see inside
...
INT SEOBPrecCalculateNQCWindowFactorsFromDyn(SEOBPrecdynamics *dyn, REAL8 thPeak, REAL8 tr6M, REAL8 tHiStart,
                                             REAL8 tThresh, INT is_first, SpinEOBParams *seobParams)
{
    REAL8 tp1, tp2;
    REAL8 pr_n, pr_o;
...
)
{
    ...
    for (i = 0; i < orbOmegaVec.length; i++)
    {
        ...
        if (PREC_FLAG == 2)
        {
            prec_CalculateSEOBPrecWaveformVariables(
                &vars, seobdynamics->nchiaVec[i], seobdynamics->nchisVec[i], seobdynamics->lchiaVec[i],
                seobdynamics->lchisVec[i], seobdynamics->echiaVec[i], seobdynamics->echisVec[i],
                seobdynamics->chi1chi1[i], seobdynamics->chi1chi2[i], seobdynamics->chi2chi2[i], seobdynamics->JnVec[i],
                seobdynamics->JlVec[i], seobdynamics->JeVec[i], seobdynamics->polarrVec[i],
                seobdynamics->polarprTVec[i], seobdynamics->prTDotVec[i]);
            if (prec_EOBGetPrecEccSpinFactorizedWaveform_v1(&(hLMVec->data[i]), &polarDynamics, &values, v,
                                                            HamVec.data[i], modeL, modeM, &vars, params) != CEV_SUCCESS)
            {
                PRINT_LOG_INFO(LOG_CRITICAL,
                               "failure in prec_EOBGetPrecEccSpinFactorizedWaveform at "
                               "step %d of the loop.",
                               i);
                return CEV_FAILURE;
            }
        ...
        else if (PREC_FLAG > 2)
        {
            ...
            prec_CalculateSEOBPrecWaveformVariables(
                ...
                seobdynamics->JlVec[i], seobdynamics->JeVec[i], seobdynamics->polarrVec[i],
                seobdynamics->polarprTVec[i], seobdynamics->prTDotVec[i]);
            if (prec_EOBGetPrecEccSpinFactorizedWaveform_v2(&(hLMVec->data[i]), &polarDynamics, &values, v,
                                                            HamVec.data[i], modeL, modeM, &vars, params) != CEV_SUCCESS)
            {
                PRINT_LOG_INFO(LOG_CRITICAL,
                               "failure in prec_EOBGetPrecEccSpinFactorizedWaveform at "
        ...
        else if (XLALSimIMRSpinEOBGetPrecSpinFactorizedWaveform(&(hLMVec->data[i]), &polarDynamics, &values, v,
    ...
    if (isnan(creal(params->cal21)) || isnan(cimag(params->cal21)) || isnan(creal(params->cal55)) ||
        isnan(cimag(params->cal55)))
    {
...
INT prec_calculateSEOBFactorizedWaveformCorrectionFromDynVectors(
    SpinEOBParams *params, const INT l, const INT m, REAL8Vector *xVec, REAL8Vector *yVec, REAL8Vector *zVec,
    REAL8Vector *vxVec, REAL8Vector *vyVec, REAL8Vector *vzVec, REAL8Vector *pTxVec, REAL8Vector *pTyVec,
    REAL8Vector *pTzVec, REAL8Vector *s1xVec, REAL8Vector *s1yVec, REAL8Vector *s1zVec, REAL8Vector *s2xVec,
    REAL8Vector *s2yVec, REAL8Vector *s2zVec, REAL8Vector *hamVec, REAL8Vector *prTDotVec, REAL8Vector **rholm_real,
    REAL8Vector **rholm_imag, REAL8Vector **flm_real, REAL8Vector **flm_imag)
{
    INT i, j, length;
    length = xVec->length;
    ...
    for (i = 0; i < length; i++)
    {
        ...
        ham = hamVec->data[i];
        // print_debug("s1Vec = (%.5e, %.5e, %.5e), s2Vec = (%.5e, %.5e,
        // %.5e)\n",
        ...
        if (XLALSimIMREOBCalcSpinPrecFacWaveformCoefficients(params->hCoeffs, m1, m2, eta, tplspin, chiS, chiA, 451) ==
            CEV_FAILURE)
        {
            PRINT_LOG_INFO(LOG_CRITICAL,
                           "failure in XLALSimIMREOBCalcSpinPrecFacWaveformCoefficients at "
        ...
        COMPLEX16 rholm = 0.;
        COMPLEX16 flm = 0.;
        prec_CalculateSEOBPrecWaveformVariables(&vars, nchia, nchis, lchia, lchis, echia, echis, chi1chi1, chi1chi2,
                                                chi2chi2, 0, 0, 0, polarr, polarpr, prTDot);
        if (prec_CalculateFactorizedWaveformCorrection(&rholm, &flm, polarDynamics, values, v, ham, l, m, &vars,
                                                       params) != CEV_SUCCESS)
        {
            PRINT_LOG_INFO(LOG_CRITICAL,
                           "failure in prec_EOBGetPrecEccSpinFactorizedWaveform at "
...
// loop on times
static int SEOBPrecCalculatehlmAmpPhase(CAmpPhaseSequence **hlm, /**<< Output: hlm in complex amplitude / phase form */
        ...
        if (((l == 2) && (m == 1)) || ((l == 5) && (m == 5)))
        {
                    ...
                    seobParams, l, m, seobdynamics, tPeakOmega - seobdynamics->tVec[0], m1, m2, deltaT) == CEV_FAILURE)
            {
                PRINT_LOG_INFO(LOG_CRITICAL, "failure in "
                                             "XLALSimIMREOBPrecCalcCalibCoefficientHigherModesPrec.");
                return CEV_FAILURE;
            }
        }
    }
    ...
    for (i = 0; i < retLen; i++)
    {
        ...
        if (PREC_FLAG == 2)
        {
            prec_CalculateSEOBPrecWaveformVariables(
                ...
                seobdynamics->JlVec[i], seobdynamics->JeVec[i], seobdynamics->polarrVec[i],
                seobdynamics->polarprTVec[i], seobdynamics->prTDotVec[i]);
            if (prec_EOBGetPrecEccSpinFactorizedWaveform_v1(&hlm_val, &polarDynamics, &values, v, ham, l, m, &vars,
        ...
        else if (PREC_FLAG > 2)
        {
            prec_CalculateSEOBPrecWaveformVariables(
                ...
                seobdynamics->JlVec[i], seobdynamics->JeVec[i], seobdynamics->polarrVec[i],
                seobdynamics->polarprTVec[i], seobdynamics->prTDotVec[i]);
            if (prec_EOBGetPrecEccSpinFactorizedWaveform_v2(&hlm_val, &polarDynamics, &values, v, ham, l, m, &vars,
                                                            seobParams) != CEV_SUCCESS)
            {
                PRINT_LOG_INFO(LOG_CRITICAL,
                               "failure in prec_EOBGetPrecEccSpinFactorizedWaveform at "
        ...
        else if (XLALSimIMRSpinEOBGetPrecSpinFactorizedWaveform(&hlm_val, &polarDynamics, &values, v, ham, l, m,
                                                                seobParams) != CEV_SUCCESS)
        {
            PRINT_LOG_INFO(LOG_CRITICAL,
                           "failure in XLALSimIMRSpinEOBGetPrecSpinFactorizedWaveform at "
    ...
    */
    ...
    for (nmode = 0; nmode < nmodes; nmode++)
    {
        ...
        else
        {
            ...
            // l, m); if (l==2 && m==1)
            //     DEBUG_START;
            SEOBPrecCalculatehlmAmpPhase(&hlm, l, m, seobdynamics, nqcCoeffs, seobParams, includeNQC);
            // if (l==2 && m==1)
            //     DEBUG_END;
...
INT SEOBPrecAttachRDToSphHarmListhPlm(SphHarmListCAmpPhaseSequence **listhPlm_RDattached, /**<< Output: list of
...
int SEOBPrecJoinTimeVector(REAL8Vector **tVecPmodes,           /**<< Output: vector of times for P-modes
                                                                  (AdaS+HiS+RDpatch) */
                           ...
                           REAL8 tAttach,                      /**<< Input: time of attachment */
                           SEOBPrecdynamics *seobdynamicsAdaS, /**<< Input: SEOB dynamics with adaptive-sampling */
                           SEOBPrecdynamics *seobdynamicsHiS   /**<< Input: SEOB dynamics with high-sampling */
)
{
    /* Read from inputs */
...
int SEOBPrecJoinDynamics(SEOBPrecdynamics **seobdynamicsJoined, /**<< Output: pointer to joined dynamics */
...
int SEOBPrecEulerJ2PFromDynamics(REAL8Vector **alphaJ2P, /**<< Output: pointer to vector for alpha J2P */
                                 ...
                                 SEOBPrecdynamics *seobdynamics, /**<<Input: SEOB dynamics (joined AdaS+HiS,
                                                                up to tAttach) */
                                 SpinEOBParams *seobParams       /**<< SEOB params */
)
{
