...
REAL8
NQCWindow(REAL8 t, REAL8 t0, REAL8 W)
{
                                   ...
                                   HyperParams *hparams)
{
    ...
    coeff32 = 0;
    coeff33 = 0;
    res = coeff00 + coeff01 * chi + coeff02 * chiTo2 + coeff03 * chiTo3 + coeff10 * eta + coeff11 * eta * chi +
          coeff12 * eta * chiTo2 + coeff13 * eta * chiTo3 + coeff20 * eta2 + coeff21 * eta2 * chi +
          coeff22 * eta2 * chiTo2 + coeff23 * eta2 * chiTo3 + coeff30 * eta3 + coeff31 * eta3 * chi +
          coeff32 * eta3 * chiTo2 + coeff33 * eta3 * chiTo3;
    if (hparams->flagTuning)
        res = hparams->dtPeak;
    // RC: for the 55 mode the attachment is done at tpeak22 -10M, note that
    // since here deltat22 is defined as -deltat22 with respect to SEOBNRv4
    // paper, here I need to add 10 M
    if ((l == 5) && (m == 5))
    {
        res = res + 10;
    }
...
)
{
    ...
    mag = 1. + (p * p / rOmegaSq) * (coeffs->a1 + coeffs->a2 / r + (coeffs->a3 + coeffs->a3S) / (r * sqrtR) +
...
)
{
    ...
    /* and XLALSimIMREOBCalculateNQCCoefficients() */
    mag = 1. +
          (p * p * nWind * nWind / rOmegaSq) * (coeffs->a1 + coeffs->a2 / r + (coeffs->a3 + coeffs->a3S) / (r * sqrtR) +
                                                coeffs->a4 / (r * r) + coeffs->a5 / (r * r * sqrtR));

    phase = coeffs->b1 * p * nWind / rOmega +
            p * p * p * nWind / rOmega * (coeffs->b2 + coeffs->b3 / sqrtR + coeffs->b4 / r);
...
XLALSimIMREOBGetNRSpinPeakAmplitudeV4(INT4 modeL, INT4 modeM, REAL8 m1, REAL8 m2, REAL8 chiS, REAL8 chiA)
{
    ...
    if (m1 < m2)
    {
        // RC: The fits for the HMs are done under the assumption m1>m2, so if
        // m2>m1 we just swap the two bodies
        tempm1 = m1;
        m1 = m2;
        m2 = tempm1;
        chiA = -chiA;
    ...
    case 2:
        ...
        case 2:
            // TPL fit
            fTPL = 1.4528573105413543 + 0.16613449160880395 * chi + 0.027355646661735258 * chi2 -
                   0.020072844926136438 * chi3;
            // Equal-mass fit
            fEQ =
                1.577457498227 - 0.0076949474494639085 * chi + 0.02188705616693344 * chi2 + 0.023268366492696667 * chi3;
        ...
        case 1:
            ...
            res = -((0.29256703361640224 - 0.19710255145276584 * eta) * eta * chi21 +
                    dM * eta *
                        (-0.42817941710649793 + 0.11378918021042442 * eta - 0.7736772957051212 * eta2 +
        ...
        default:
            PRINT_LOG_INFO(LOG_CRITICAL,
                           "(%d,%d) mode. At present only fits for the (2,2), (2,1), "
                           "(3,3), (4,4) and (5,5) mode are available.",
                           modeL, modeM);
    ...
    case 3:
        ...
        case 3:
            res = (0.10109183988848384 * eta - 0.4704095462146807 * eta2 + 1.0735457779890183 * eta2 * eta) * chi33 +
                  dM * (0.5636580081367962 * eta - 0.054609013952480856 * eta2 + 2.3093699480319234 * eta2 * eta +
                        chi33 * chi33 * (0.029812986680919126 * eta - 0.09688097244145283 * eta2));
            break;
        case 2:
            res = eta * (0.1957041633324017 - 0.1302947020703087 * chi32 + 0.020592651712965825 * chi32 * chi32 -
                         1.1620118898824288 * eta + 1.6490673639778208 * chi32 * eta + 2.488043327597838 * eta * eta -
    ...
    case 4:
        ...
        {
        case 4:
            res = eta * (0.2646580063832686 + 0.067584186955327 * chi44 + 0.02925102905737779 * chi44 * chi44) +
                  eta2 * (-0.5658246076387973 - 0.8667455348964268 * chi44 + 0.005234192027729502 * chi44 * chi44) +
                  eta * eta2 * (-2.5008294352355405 + 6.880772754797872 * chi44 - 1.0234651570264885 * chi44 * chi44) +
    ...
    case 5:
        ...
        case 5:
            ...
            res = 0.128621 * dM * eta - 0.474201 * dM * eta * eta + 1.0833 * dM * eta * eta * eta +
                  0.0322784 * eta * chi33 - 0.134511 * chi33 * eta * eta + 0.0990202 * chi33 * eta * eta * eta;
...

{
    ...
    case 2:
        ...
        case 2:
            ...
            break;
        case 1:
            res = dM * eta * (0.007147528020812309 - eta * 0.035644027582499495) +
                  dM * eta * chi21 * (-0.0087785131749995 + eta * 0.03054672006241107) +
                  eta * 0.00801714459112299 *
                      fabs(-dM * (0.7875612917853588 + eta * 1.161274164728927 + eta2 * 11.306060006923605) + chi21);
    ...
    case 3:
        ...
        case 3:
            res = dM * eta * (-0.00309943555972098 + eta * 0.010076527264663805) * chi33 * chi33 +
                  ...
                  eta * 0.0016309606446766923 *
                      sqrt(dM2 * (8.811660714437027 + 104.47752236009688 * eta) +
                           dM * chi33 * (-5.352043503655119 + eta * 49.68621807460999) + chi33 * chi33);
            break;
        case 2:
            res = eta *
                  (-0.0010440559175499686 + 0.02117114888170324 * chi + 0.0022577853352497845 * chi * chi +
                   0.11591042725217449 * eta - 0.5540819789292755 * chi * eta - 0.007561346932095425 * chi * chi * eta -
                   1.1025807310111304 * eta * eta + 3.974332676380982 * chi * eta * eta +
                   2.840194587808517 * eta * eta * eta - 8.461702608539895 * chi * eta * eta * eta);
        default:
            PRINT_LOG_INFO(LOG_CRITICAL,
                           "(%d,%d) mode. At present only fits for the (2,2), (2,1), "
                           "(3,3), (4,4) and (5,5) mode are available.",
    ...
    case 4:
        ...
        {
        case 4:
            res = eta * (0.004347588211099233 - 0.0014612210699052148 * chi44 - 0.002428047910361957 * chi44 * chi44) +
                  eta2 * (0.023320670701084355 - 0.02240684127113227 * chi44 + 0.011427087840231389 * chi44 * chi44) +
    ...
    case 5:
        ...
        {
        case 5:
            res = eta * (dM * (-0.008389798844109389 + 0.04678354680410954 * eta) +
...
XLALSimIMREOBGetNRSpinPeakADDotV4(INT4 modeL, INT4 modeM, REAL8 m1, REAL8 m2, REAL8 chiS, REAL8 chiA)
{
    ...
    case 2:
        ...
        case 2:
            // TPL fit
            fTPL = 0.002395610769995033 * chiMinus1 - 0.00019273850675004356 * chiMinus1 * chiMinus1 -
        ...
        case 1:
            ...
            res = eta * dM * 0.00037132201959950333 -
                  fabs(dM * eta * (-0.0003650874948532221 - eta * 0.003054168419880019) +
                       dM * eta * chi21 * chi21 *
                           (-0.0006306232037821514 - eta * 0.000868047918883389 + eta2 * 0.022306229435339213) +
                       eta * chi21 * chi21 * chi21 * 0.0003402427901204342 + dM * eta * chi21 * 0.00028398490492743);
    ...
    case 3:
        ...
        {
        case 3:
            res = dM * eta * (0.0009605689249339088 - 0.00019080678283595965 * eta) * chi33 -
                  0.00015623760412359145 * eta *
                      fabs(dM * (4.676662024170895 + 79.20189790272218 * eta - 1097.405480250759 * eta2 +
                                 6512.959044311574 * eta * eta2 - 13263.36920919937 * eta2 * eta2) +
                           chi33);
            break;
        case 2:
            res = eta * (-0.0004342957316275098 + 9.615115670591479e-5 * chi + 0.010905264231311218 * eta -
                         0.02174926197368876 * chi * eta - 0.11483435937608277 * eta * eta +
                         0.2322519020549123 * chi * eta * eta + 0.33088395733951614 * eta * eta * eta -
                         0.6124624286824769 * chi * eta * eta * eta);
        default:
            PRINT_LOG_INFO(LOG_CRITICAL,
                           "(%d,%d) mode. At present only fits for the (2,2), (2,1), "
                           "(3,3), (4,4) and (5,5) mode are available.",
    ...
    case 4:
        ...
        {
        case 4:
            res = eta * (-0.000301722928925693 + 0.0003215952388023551 * chi) +
    ...
    case 5:
        ...
        {
        case 5:
            res = eta * (dM * (0.00012727220842255978 + 0.0003211670856771251 * eta) +
...
XLALSimIMREOBGetNRSpinPeakOmegaV4(INT4 modeL, INT4 modeM, REAL8 eta, REAL8 a)
{
    ...
    case 2:
        ...
        case 2:
            ...
            break;
        case 1:
            res = (0.1743194440996283 + eta * 0.1938944514123048 + 0.1670063050527942 * eta2 +
                   0.053508705425291826 * chi - eta * chi * 0.18460213023455802 + eta2 * chi * 0.2187305149636044 +
    ...
    case 3:
        ...
        {
        case 3:
            res = 0.3973947703114506 + 0.16419332207671075 * chi + 0.1635531186118689 * chi * chi +
                  0.06140164491786984 * chi * chi * chi +
                  eta * (0.6995063984915486 - 0.3626744855912085 * chi - 0.9775469868881651 * chi * chi) +
                  eta2 * (-0.3455328417046369 + 0.31952307610699876 * chi + 1.9334166149686984 * chi * chi);
            break;
        case 2:
            res = 0.3246129326224718 + 0.030321110010291363 * chi + 0.11418286156731483 * chi * chi +
                  0.15513162878235676 * chi * chi * chi + 0.4024591608708761 * eta - 0.35708134048815054 * chi * eta -
                  1.0798096459784574 * chi * chi * eta - 0.6688159172167616 * chi * chi * chi * eta -
                  1.2611501769329152 * eta * eta + 2.2650853388684027 * chi * eta * eta +
                  3.4111245557578513 * chi * chi * eta * eta;
        default:
            PRINT_LOG_INFO(LOG_CRITICAL,
                           "(%d,%d) mode. At present only fits for the (2,2), (2,1), "
                           "(3,3), (4,4) and (5,5) mode are available.",
    ...
    case 4:
        ...
        {
        case 4:
            res = 0.5389359134370971 + 0.16635177426821202 * chi + 0.2075386047689103 * chi * chi +
                  0.15268115749910835 * chi * chi * chi +
                  eta * (0.7617423831337586 + 0.009587856087825369 * chi - 1.302303785053009 * chi * chi -
                         0.5562751887042064 * chi * chi * chi) +
                  eta2 * (0.9675153069365782 - 0.22059322127958586 * chi + 2.678097398558074 * chi * chi) -
    ...
    case 5:
        ...
        {
        case 5:
            res = 0.6437545281817488 + 0.22315530037902315 * chi + 0.2956893357624277 * chi * chi +
                  0.17327819169083758 * chi * chi * chi +
                  eta * (-0.47017798518175785 - 0.3929010618358481 * chi - 2.2653368626130654 * chi * chi -
...
XLALSimIMREOBGetNRSpinPeakOmegaDotV4(INT4 modeL, INT4 modeM, REAL8 eta, REAL8 a)
{
    ...
    case 2:
        ...
        case 2:
            // TPL fit
            fTPL = -0.011209791668428353 + (0.0040867958978563915 + 0.0006333925136134493 * chi) *
                                               log(68.47466578100956 - 58.301487557007206 * chi);
            // Equal-mass fit
            ...
            break;
        case 1:
            res = (0.0070987396362959514 + eta * 0.024816844694685373 - eta2 * 0.050428973182277494 +
                   eta * eta2 * 0.03442040062259341 - chi * 0.0017751850002442097 + eta * chi * 0.004244058872768811 -
                   eta2 * chi * 0.031996494883796855 - chi * chi * 0.0035627260615894584 +
                   eta * chi * chi * 0.01471807973618255 - chi * chi * chi * 0.0019020967877681962);
            break;
        default:
            PRINT_LOG_INFO(LOG_CRITICAL,
                           "(%d,%d) mode. At present only fits for the (2,2), (2,1), "
    ...
    case 3:
        ...
        case 3:
            res = 0.010337157192240338 - 0.0053067782526697764 * chi * chi - 0.005087932726777773 * chi * chi * chi +
                  eta * (0.027735564986787684 + 0.018864151181629343 * chi + 0.021754491131531044 * chi * chi +
                         0.01785477515931398 * chi * chi * chi) +
                  eta2 * (0.018084233854540898 - 0.08204268775495138 * chi);
            break;
        case 2:
            res = -0.029580297291900592 - 0.020322864238345062 * chi - 0.029980204436881763 * chi * chi +
                  0.003160452149112203 * chi * chi * chi + 0.9029169862683375 * eta + 0.17761004756695797 * chi * eta +
                  0.2696229073483778 * chi * chi * eta - 0.020975338265706966 * chi * chi * chi * eta -
                  5.825914309556538 * eta * eta - 0.437428871638474 * chi * eta * eta -
                  0.5391808879103337 * chi * chi * eta * eta + 11.590868632032652 * eta * eta * eta;
        default:
            PRINT_LOG_INFO(LOG_CRITICAL,
                           "(%d,%d) mode. At present only fits for the (2,2), (2,1), "
                           "(3,3), (4,4) and (5,5) mode are available.",
    ...
    case 5:
        ...
        {
        case 5:
            res = 0.01763430670755021 - 0.00024925743340389135 * chi - 0.009240404217656968 * chi * chi -
                  0.007907831334704586 * chi * chi * chi +
                  eta * (-0.1366002854361568 + 0.0561378177186783 * chi + 0.16406275673019852 * chi * chi +
                         0.07736232247880881 * chi * chi * chi) +
                  eta2 * (0.9875890632901151 - 0.31392112794887855 * chi - 0.5926145463423832 * chi * chi) -
                                                ...
                                                SpinEOBParams *ak)
{
    ...
    /* Since the vectors we actually want are q etc * A, we will have to generate
     * them here */
    ...
    REAL8 nWind = 1.0;
    /* Populate vectors as necessary. Eqs. 14 - 17 of the LIGO DCC document
     * T1100433v2 */
    ...
    for (i = 0; i < timeVec->length; i++)
    {
        ...
        //             timeVec->data[i], amplitude->data[i],
        //             prVec->data[i],rVec->data[i],orbOmegaVec->data[i],
        //             q3->data[i],q4->data[i],q5->data[i], p3->data[i],
        //             p4->data[i],phase->data[i], nWind);
        // print_out(out, "%.16e %.16e %.16e %.16e %.16e %.16e %.16e %.16e %.16e
        // %.16e %.16e %.16e\n",
        //     timeVec->data[i], amplitude->data[i],
        //     prVec->data[i],rVec->data[i],orbOmegaVec->data[i],
        //     q3->data[i],q4->data[i],q5->data[i], p3->data[i],
    ...
    /* nrDeltaT defined in XLALSimIMREOBGetNRSpinPeakDeltaT is a minus sign
     * different from Eq. (33) of Taracchini et al. Therefore, the plus sign in
     * Eq. (21) of Taracchini et al and Eq. (18) of DCC document T1100433v2 is
    ...
    /* We are now in a position to use the interp stuff to calculate the
     * derivatives we need */
    /* We will start with the quantities used in the calculation of the a
     * coefficients */
    GSL_START;
    int status;
    ...
    nra = fabs(XLALSimIMREOBGetNRSpinPeakAmplitudeV4(modeL, modeM, m1, m2, chiS, chiA));

    nraDot = XLALSimIMREOBGetNRSpinPeakADotV4(modeL, modeM, m1, m2, chiS, chiA);
    // RC: In SEOBNRv4 nraDot is zero because the NQC are defining the peak of
    // the 22 mode
    //  which by definition has a first derivative	equal to 0.
    // For SEOBNRv4HM we are not computing the NQC at the peak fo the modes (see
    // Eq.(4.3)) of https://arxiv.org/pdf/1803.10701.pdf, so first the derivative
    // is entering as a fitted coefficient.

    nraDDot = XLALSimIMREOBGetNRSpinPeakADDotV4(modeL, modeM, m1, m2, chiS, chiA);
    //    printf("eta, chiS, chiA, dM/M, chi = %.16e %.16e %.16e %.16e
    //    %.16e\n",eta,chiS,chiA, (m1 - m2)/(m1 + m2),chiS + chiA*(m1 - m2)/(m1 +
    //    m2)/(1. - 2.*eta));
    ...
    /* Since the phase can be decreasing, we need to take care not to have a -ve
     * frequency */
    ...
    nromega =
        XLALSimIMREOBGetNRSpinPeakOmegaV4(modeL, modeM, eta, chiS + chiA * (m1 - m2) / (m1 + m2) / (1. - 2. * eta));
    nromegaDot =
        XLALSimIMREOBGetNRSpinPeakOmegaDotV4(modeL, modeM, eta, chiS + chiA * (m1 - m2) / (m1 + m2) / (1. - 2. * eta));
    ...
    PRINT_LOG_INFO(LOG_DEBUG,
                   "nra = %f, eoba = %f, nraDot = %f, eobaDot = %f, nromega = "
                   "%f, eobomega = %f\n",
                   nra, amp, nraDot, aDot, nromega, omega);
    PRINT_LOG_INFO(LOG_DEBUG, "NQC coefficients:\n\t{%f,%f,%f,%f,%f,%f}\n\t{%f,%f,%f,%f}", coeffs->a1, coeffs->a2,
                   coeffs->a3, coeffs->a3S, coeffs->a4, coeffs->a5, coeffs->b1, coeffs->b2, coeffs->b3, coeffs->b4);
QUIT:
