                                 ...
                                 COMPLEX16 *facAmpRes)
{
        ...
        case 2:
            switch(ABS(m))
            {
                ...
                case 1:
                    ...
                    A2lm = g2*(hCoeffs->h21T2ff21 * ff21 + 
                        hCoeffs->h21T2ff12 *I* ff12 +
                        hCoeffs->h21T2ff10*I*ff10 +
        ...
        case 3:
            switch(ABS(m))
            {
                case 3:
                    ...
                    Flm3 = ( hCoeffs->h33T3ff40*ff40 + hCoeffs->h33T3ff31*I*ff31 + 
                            hCoeffs->h33T3ff22*ff22 + hCoeffs->h33T3ff13*I*ff13 + 
                            hCoeffs->h33T3ff20 * ff20 + 
    ...
    else
    {
        ...
        // if (isnan(rho2PN))
        // print_debug("(%d,%d), h21T2ff21 = %g, h21T2ff12 = %g, h21T2ff10 = %g, h21T2ff03 = %g, h21T2ff01 = %g + i %g\n", l, m, 
        //         hCoeffs->h21T2ff21, hCoeffs->h21T2ff12, 
        //         hCoeffs->h21T2ff10, hCoeffs->h21T2ff03, creal(hCoeffs->h21T2ff01), cimag(hCoeffs->h21T2ff01));
 ...
 */
INT
XLALSimIMRSpinEOBGetSpinFactorizedWaveformV2 (COMPLEX16 * ret,
  ...
  )
{
    ...
    if (CalculateFacWaveformAmpResV2(hCoeffs, eta, l, m, v, vPhi, vh3, eulerlogxabs, 1/sqrt(r), dr, ncrv, &rholmPwrl) != CEV_SUCCESS)
        return CEV_FAILURE;
    if (ret_type >1)
    {
        *ret = rholmPwrl;
...
)
{
    ...
    for (l = 2; l <= lMax; l++)
    {
        for (m = 1; m <= l; m++)
        {
            if (XLALSimIMRSpinEOBGetSpinFactorizedWaveformV2
...
/*--------------------------------------------------------------*/
INT
XLALSimIMRSpinEOBFluxGetPrecSpinFactorizedWaveformV2(
    ...
    case 2:
        ...
        case 2:
            ...
            //FIXME
                ...
                //         hCoeffs->rho22v8 + hCoeffs->rho22v8l * eulerlogxabs,
                //         hCoeffs->rho22v10 + hCoeffs->rho22v10l * eulerlogxabs);}
        //     fTe = (CST_PI*(-7.*I*dr*(-15*pow(prDot,4)*pow(r,8) + 180*pow(prDot,5)*pow(r,10) + 
        ...
        //     2*pl*(-3633*pow(prDot,5)*pow(r,10) + 3395*pow(prDot,6)*pow(r,12) - 
        ...
        //   60*(672 + 8*pow(r,2)*pow(dr,4) + 7*pow(r,3)*pow(dr,6)))))/(40320.*r);
        //   fTe = fTe - I*((omega*(-11 + 12*CST_GAMMA - 6.*I*CST_PI + 36*CST_LN2 + 36*Log(v)))/3.;
          //fTe = fTe - I * (Omega*(-11 + 12*CST_GAMMA - 6.*I*CST_PI + 36*CST_LN2 + 12*log(Omega)))/3.;
            break;
        case 1:
            {
...
INT XLALSimIMRSpinEOBGetAmplitudeResidualPrecV2(COMPLEX16 *rholmpwrl, 
                ...
                SpinEOBParams *params)
  {
    ...
    *rholmpwrl = rholmPwrl;
#endif
    if (CalculateFacWaveformAmpResV2(hCoeffs, eta, modeL, modeM, v, vPhi, vh3, eulerlogxabs, 1/sqrt(r), dr, ncrv, &rholmPwrl) != CEV_SUCCESS)
        return CEV_FAILURE;
    *rholmpwrl = rholmPwrl;
    return CEV_SUCCESS;
}
...
REAL8
XLALInspiralPrecSpinFactorizedFluxV2(
...
REAL8
XLALInspiralSASpinFactorizedFluxV2(
				   REAL8Vector * polvalues,	/**< \f$(r,\phi,p_r,p_\phi)\f$ */
				   EOBNonQCCoeffs * nqcCoeffs,	/**< pre-computed NQC coefficients */
				   const REAL8 omega,	/**< orbital frequency */
				   SpinEOBParams * ak,	/**< physical parameters */
...
)
{
    ...
    //XLAL_PRINT_INFO("v = %.16e\n", v);
    COMPLEX16 hLMTab[lMax+1][lMax+1];
    if (XLALSimIMRSpinEOBFluxGetSASpinFactorizedWaveform(&hLMTab[0][0], polvalues, v, H,
                lMax, ak) == CEV_FAILURE) {
        return REAL8_FAIL_NAN;
    }
    for (l = 2; l <= lMax; l++) 
