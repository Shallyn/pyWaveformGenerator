...
static int GSLSearchMfiniEcc(const gsl_vector *x,
                             void *params,
                             gsl_vector *f)
{
    REAL8 r, pphi;
    ...
    EstimateEquatorialEccentricity(r, pphi, &recMfMin, &rececc,
        pms->newecc, pms->newMfini, pms->core);
...
INT SEOBComputeExactEquatorialInitialCondition(REAL8Vector *ICvalues,
                          REAL8 MfMin,
                          REAL8 ecc,
                          SpinEOBParams *seobParams)
{
    ...
    if (EOBInitialConditionsPrec_Conserve(tmpInitvals, m1, m2, MfMin, ecc, 0, mSpin1data, mSpin2data, seobParams) != CEV_SUCCESS)
        return CEV_FAILURE;
...
INT EstimateEquatorialEccentricity(REAL8 r, REAL8 pphi, REAL8 *MfOrb, REAL8 *ecc,
    REAL8 inputfMin, REAL8 inputecc, SpinEOBParams *core)
{
    ...
    *MfOrb = avgDphi / CST_2PI;
    // print_debug("kk = %.16e\n", kk);
    // print_debug("retLen = %d, tend = %.16e\n", retLenAdaS, seobdynamicsAdaS->tVec[retLenAdaS-1]);
    // print_debug("dt0 = %.16e, dt1 = %.16e\n", 
...
static double TMPSpinAlignedHamiltonian(const REAL8 values[], SpinEOBParams *params)
{
    ...
    Ham = EOBHamiltonian( params->eta, &r, &p, &spin1norm, &spin2norm, &sigmaKerr, &sigmaStar, params->tortoise, seobHamCoeffs ) / params->eta;

    return Ham;
}

static int FindSphericalOrbit(const gsl_vector *x, 
                              void *params,
                              gsl_vector *f)
{
...
// print_debug("dHdpx_a = %.16e, dHdpx_p = %.16e, Ham_a - Ham_p = %.16e\n", dHdpx_a, dHdpx_p, Ham_a - Ham_p);
// print_debug("Ham_a = %.16e, Ham_p = %.16e\n", Ham_a, Ham_p);
// print_debug("px_a = %.16e, px_p = %.16e, pphi = %.16e\n", px_a, px_p, pphi);
...
INT EOBInitialConditionsPrec_epi(
...
)
{
    ...
    if (x0 != 1. || spin1[0] != 0 || spin1[1] != 0 || 
        spin2[0] != 0 || spin2[1] != 0)
    {
        PRINT_LOG_INFO(LOG_WARNING, "The input parameters denote spin-precessing orbits, which is un-supported");
    }
    int i, failed = 0;
    ...
    tmpS1[0] = tmpS1[1] = tmpS2[0] = tmpS2[1] = 
        tmpS1Norm[0] = tmpS1Norm[1] = tmpS2Norm[0] = tmpS2Norm[1] = 0.0;
