...
def calculate_QNMFrequenciesFromFinal(mFinal: float, chiFinal: float, modeL: int = 2, modeM: int = 2, nmodes: int = 1):
    FreqRVec = ctypes.POINTER(pyREAL8Vector)()
    FreqIVec = ctypes.POINTER(pyREAL8Vector)()
    value_list = [ctypes.c_double(mFinal), ctypes.c_double(chiFinal), ctypes.c_uint(modeL), ctypes.c_uint(modeM), ctypes.c_size_t(nmodes), ctypes.byref(FreqRVec), ctypes.byref(FreqIVec)]
    ...
    return npFreqRVec + 1.0j * npFreqIVec


def calculate_QNMFrequencies(m1: float, m2: float, chi1x: float, chi1y: float, chi1z: float, chi2x: float, chi2y: float, chi2z: float, modeL: int = 2, modeM: int = 2, nmodes: int = 1):
    FreqRVec = ctypes.POINTER(pyREAL8Vector)()
    FreqIVec = ctypes.POINTER(pyREAL8Vector)()
    value_list = [
        ctypes.c_double(m1),
        ...
        ctypes.byref(FreqRVec),
        ctypes.byref(FreqIVec),
    ]
...
def calculate_QNMFrequency(m1: float, m2: float, chi1x: float, chi1y: float, chi1z: float, chi2x: float, chi2y: float, chi2z: float, modeL: int = 5, modeM: int = 5):
    value_list = [ctypes.c_double(m1), ctypes.c_double(m2), ctypes.c_double(chi1x), ctypes.c_double(chi1y), ctypes.c_double(chi1z), ctypes.c_double(chi2x), ctypes.c_double(chi2y), ctypes.c_double(chi2z), ctypes.c_uint(modeL), ctypes.c_uint(modeM)]
    ...
    def __getitem__(self, key: int):
        return np.array(
            [
                ...
                self.__s2yVec[key],
                self.__s2zVec[key],
                self.__phiDModVec[key],
                self.__phiModVec[key],
            ]
        )
...
class SEOBNRWaveformCaller(object):
    ...
    def __parse_params(self, **kwargs):
        if 'use_geom' in kwargs:
            self.use_geom = 0 if kwargs['use_geom'] is False else 1
        ...
        self.inc = self.inc if 'inc' not in kwargs else kwargs['inc'] * np.pi / 180.0
        if 'inc_rad' in kwargs:
            self.inc = kwargs['inc_rad']
        self.phiRef = self.phiRef if 'phiRef' not in kwargs else kwargs['phiRef'] * np.pi / 180.0
        if 'phiRef_rad' in kwargs:
            self.phiRef = kwargs['phiRef_rad']
        self.beta = self.beta if 'beta' not in kwargs else kwargs['beta'] * np.pi / 180.0
        ...
        if 'deltaT' in kwargs:
            ...
            self.srate = 1.0 / self.deltaT
        if 'is_only22' in kwargs:
            self.is_only22 = 0 if kwargs['is_only22'] is False else 1
        if 'is_constp' in kwargs:
            self.is_constp = 0 if kwargs['is_constp'] is False else 1
        ...
        if 'is_noringdown' in kwargs:
            self.is_noringdown = 0 if kwargs['is_noringdown'] is False else 1
        ...
        if 'is_coframe' in kwargs:
            self.is_coframe = 0 if kwargs['is_coframe'] is False else 1
        if 'use_coaphase' in kwargs:
            self.use_coaphase = 0 if kwargs['use_coaphase'] is False else 1
        ...
        if 'zero_dyncoaphase' in kwargs:
            self.zero_dyncoaphase = 0 if kwargs['zero_dyncoaphase'] is False else 1
    ...
    def calculate_hcorrections(self, l: int, m: int, dyn: npDynamicData):
        ...
        params = my_waveform_lib.CreateSpinEOBParams(
        ...
        ret = my_waveform_lib.prec_calculateSEOBFactorizedWaveformCorrectionFromDynVectors(
            params,
            ctypes.c_int(l),
            ctypes.c_int(m),
            xVec,
            ...
            hamVec,
            prTDotVec,
            ctypes.byref(rholm_real),
            ctypes.byref(rholm_imag),
            ctypes.byref(flm_real),
            ctypes.byref(flm_imag),
        )
        if ret != 0:
            return None
        ...
        return rholm_real_np + 1.0j * rholm_imag_np, flm_real_np + 1.0j * flm_imag_np
...
def test_interface(m1: float, m2: float):
    ...
    for l in range(2, 8):
        for m in range(1, l + 1):
            print(ret.contents.values[l][m].real + 1.0j * ret.contents.values[l][m].imag)
