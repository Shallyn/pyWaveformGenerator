...
from ..pyUtils import cTimeSeries, rTimeSeries
from .detector import Detector

__all__ = ['DetectorPSD', 'Detector']

LOC = Path(__file__).parent / "psd"
...
class DetectorPSD(object):
    ...
    def _sim_noise_seg(self, seg, psd_data, srate):
        ...
        stilde = sigma * (np.random.randn(len(sigma)) + 1.0j * np.random.randn(len(sigma)))
...
def PSD_DECIGO_fit(f, is_get_frange=False):
    ...
    return 6.53e-49 * (1 + fac_f2) + 4.45e-51 * fac_f_4 / (1 + 1 / fac_f2) + 4.49e-52 * fac_f_4
...
def PSD_ET_fit(f, is_get_frange=False):
    ...
    ret = 1.449e-52 * (p1 + a1p2 + 232.56 * (1 + 31.18 * x - 46.72 * x2 + 52.24 * x3 - 42.16 * x4 + 10.17 * x5 + 11.53 * x6) / (1 + 13.58 * x - 36.46 * x2 + 18.56 * x3 + 27.43 * x4))
...
def PSD_advLIGO_fit(f, is_get_frange=False):
    ...
    fac2 = np.power(fac, 2)
    fac4 = np.power(fac, 4)
    ret = 1e-49 * (np.power(fac, -4.14) - 5 / fac2 + 111 * (1 - fac2 + fac4 / 2) / (1 + fac2 / 2))
...
def get_PSD_Space_fit(name='LISA', flow=0, fhigh=None):
    ...
    for case in switch(name_low):
        if case('lisa'):
            ...
            fP_acc = lambda f: np.power(3.0e-15, 2) * (1 + np.power(4.0e-4 / f, 2))
            L = 2.5e9
            break
        if case('taiji'):
            ...
            break
        if case('tianqin'):
            P_OMS = np.power(1.0e-12, 2)
            fP_acc = lambda f: np.power(1.0e-15, 2) * (1 + np.power(1.0e-4 / f, 2))
            L = np.sqrt(3) * 1e8
            break
    ...
    def func_PSD(f, is_get_frange=False):
        ...
        ret = (10.0 / (3.0 * L * L)) * (P_OMS + 2 * (1 + np.power(np.cos(f / fstar), 2)) * (fP_acc(f) / np.power(2 * np.pi * f, 4))) * (1 + 6.0 * np.power(f / fstar, 2) / 10.0)
        if hasattr(f, '__len__'):
            f = np.asarray(f)
...
DetectorDict = {'CE1': Detector_CE1, 'CE2': Detector_CE2, 'ET1': Detector_ET1, 'ET2': Detector_ET2, 'ET3': Detector_ET3, 'H1': Detector_H1, 'L1': Detector_L1, 'Virgo': Detector_Virgo, 'KAGRA': Detector_KAGRA}

c_SI = 299792458  # Speed Of Light [m s^-1]


class AntennaPatternFCalculator(object):
    def __init__(self, psi, ra, dec, latitude, longitude, gamma, zeta=np.pi / 2.0):
        self.__psi = psi
        self.__alpha = ra
        self.__delta = dec
        self.__lbd = latitude
        ...
        self.__sin_2psi = np.sin(2.0 * self.__psi)
        self.__cos_2psi = np.cos(2.0 * self.__psi)
        self.__sin_2alphi = np.sin(2.0 * self.__alphi)
        self.__cos_2alphi = np.cos(2.0 * self.__alphi)
        self.__sin_alphi = np.sin(self.__alphi)
        self.__cos_alphi = np.cos(self.__alphi)
        self.__sin_2gamma = np.sin(2.0 * self.__gamma)
        self.__cos_2gamma = np.cos(2.0 * self.__gamma)
        self.__sin_2delta = np.sin(2.0 * self.__delta)
        self.__cos_2delta = np.cos(2.0 * self.__delta)
        self.__sin_delta = np.sin(self.__delta)
        self.__cos_delta = np.cos(self.__delta)
        self.__sin_2lambda = np.sin(2.0 * self.__lbd)
        self.__cos_2lambda = np.cos(2.0 * self.__lbd)
        self.__sin_lambda = np.sin(self.__lbd)
        self.__cos_lambda = np.cos(self.__lbd)
        self.__3MinusCos2Lambda = 3.0 - self.__cos_2lambda
        self.__3MinusCos2Delta = 3.0 - self.__cos_2delta

        self.__Gplus = (
            (1.0 / 16.0) * self.__sin_2gamma * self.__3MinusCos2Lambda * self.__3MinusCos2Delta * self.__cos_2alphi
        ...
        self.__Gcross = (
            self.__cos_2gamma * self.__sin_lambda * self.__sin_delta * self.__cos_2alphi
        ...
        self.__OmegaEarth = 2.0 * np.pi / (24.0 * 60.0 * 60.0)
    ...
    def Gplus_t(self, t_SI):
        ...
        cos_2alphi_t = np.cos(2.0 * alphi_t)
        sin_2alphi_t = np.sin(2.0 * alphi_t)
        cos_alphi_t = np.cos(alphi_t)
        sin_alphi_t = np.sin(alphi_t)
        return (
            (1.0 / 16.0) * self.__sin_2gamma * self.__3MinusCos2Lambda * self.__3MinusCos2Delta * cos_2alphi_t
    ...
    def Gcross_t(self, t_SI):
        ...
        sin_alphi_t = np.sin(alphi_t)
        return (
            self.__cos_2gamma * self.__sin_lambda * self.__sin_delta * cos_2alphi_t
