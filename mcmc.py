...
def simulate_waveform_from_detector_list(params, 
                                         f_min = 40., 
                                         gps0 = 1356566418, 
                                         detectors = None, 
                                         is_only22 = False, log_level = 1, retall = False, **kwargs) -> rTimeSeries:
    if 'debug' in kwargs:
        if kwargs['debug']:
    ...
    ge.set_params(m1 = m1, m2 = m2, 
                  ...
                  ecc = e0, distance = dL,
                  inc_rad = iota, beta_rad = Phi_c, f_min = f_min,
                  log_level=log_level, is_only22=is_only22, **kwargs)
    waveform, _ = ge.run()
    if waveform is None:
...
def simulate_waveform_from_detector_list_pycbc(params, f_min = 40., gps0 = 1356566418, detectors = None, approx=None, srate = 16384, **kwargs):
    if approx is None:
        approx = 'SEOBNRv4PHM'
    q, Mchirp, th1, ph1, chi1, th2, ph2, chi2, e0, dL, theta, phi, iota, psi, t_c, Phi_c = params
    s1x, s1y, s1z = calculate_chiVec_from_thphchi(th1, ph1, chi1)
    ...
    hp, hc = get_td_waveform(approximant=approx,
                            ...
                            distance = dL, inclination = iota*180/np.pi, coa_phase = Phi_c*180/np.pi,
                            delta_t=1.0/srate,
                            f_lower=f_min)
...
def simulate_mcmc(argv):
    ...
    parser.add_option('--distance', type = 'float', default = 100., help = 'input distance')
    ...
    parser.add_option('--t-extra', type = 'float', default = 5., help = 'extra time for fake data')
    ...
    theta = np.pi - args.dec*np.pi/180. # pi/2 - declination at the time gps0+t_c, in rad
    phi = args.ra*np.pi/180. # right ascension at the time gps0+t_c, in rad
    psi = args.polarization*np.pi/180. # polarization angle, in rad
    iota = args.inclination*np.pi/180. # inclination angle, in rad
    Phi_c = args.phi_ref*np.pi/180. # reference orbital phase, in rad
    ...
    params0 = (q, Mchirp, \
            th1, ph1, chi1, \
            th2, ph2, chi2, \
            ecc, dL, theta, phi, iota, psi, 0.0, Phi_c)
    ...
    def log_likelihood(params):
        ...
        hmodel_rts_list = simulate_waveform_from_detector_list((q, Mchirp, th1, ph1, chi1, th2, ph2, chi2, ecc, dL, theta, phi, iota, psi, t_c, Phi_c),
                    gps0 = gps0, f_min = f_min,
                    detectors = detector_list, is_only22 = False, prec_flag = 3, code_version = args.code_version, debug = args.code_debug)
    ...
    def log_likelihood_full(params):
        q, Mchirp, th1, ph1, chi1, th2, ph2, chi2, e0, dL, theta, phi, iota, psi, t_c, Phi_c = params
        hmodel_rts_list = simulate_waveform_from_detector_list((q, Mchirp, th1, ph1, chi1, th2, ph2, chi2, e0, dL, theta, phi, iota, psi, t_c, Phi_c),
                    gps0 = gps0, f_min = f_min,
                    detectors = detector_list, is_only22 = False, prec_flag = 3, code_version = args.code_version, debug = args.code_debug)
        if hmodel_rts_list is None:
            return -np.inf
    ...
    def log_prior_full(params):
        ...
        if  1<=q<=20 and (10. <= Mchirp <= 200.) and 0<= chi1 <= 0.95 and 0<= chi2 <= 0.95 and \
            ...
            0.0 <= psi <= np.pi and \
            0.0 <= iota <= np.pi and -2.*t_extra < t_c < 2.*t_extra and 0.0 <= Phi_c <= 2.*np.pi:
            return 0.0
        return -np.inf
    ...
    def log_likelihood_PrecCirc(params):
        ...
        hmodel_rts_list = simulate_waveform_from_detector_list((q, Mchirp, th1, ph1, chi1, th2, ph2, chi2, 0.0, dL, theta, phi, iota, psi, t_c, Phi_c),
                    gps0 = gps0, f_min = f_min,
                    detectors = detector_list, is_only22 = args.is_only22, code_version = 0, debug = args.code_debug)
        if hmodel_rts_list is None:
            return -np.inf
        yres_rts_list = [rTimeSeries_add(signal_rts_list[i], -hmodel_rts_list[i], srate = sample_rate, method = 1) for i in range(len(detector_list))]
        lnlike = np.sum([-0.5 * residuals_inner_product(yres_rts_list[i], detector_list[i], fmin = None, fmax = None) for i in range(len(detector_list))])
    ...
    def log_likelihood_SA(params):
        ...
        hmodel_rts_list = simulate_waveform_from_detector_list((q, Mchirp, 0, 0, chi1, 0, 0, chi2, e0, dL, theta, phi, iota, psi, t_c, Phi_c),
                    gps0 = gps0, f_min = f_min, egw_flag = 1,
                    detectors = detector_list, is_only22 = args.is_only22, debug = args.code_debug)
        if hmodel_rts_list is None :
            return -np.inf
        yres_rts_list = [rTimeSeries_add(signal_rts_list[i], -hmodel_rts_list[i], srate = sample_rate, method = 1) for i in range(len(detector_list))]
        lnlike = np.sum([-0.5 * residuals_inner_product(yres_rts_list[i], detector_list[i], fmin = None, fmax = None) for i in range(len(detector_list))])
    ...
    if is_pycbc_loaded:
        ...
        def log_likelihood_pycbc(params):
            ...
            hmodel_rts_list = simulate_waveform_from_detector_list_pycbc((q, Mchirp, th1, ph1, chi1, th2, ph2, chi2, 0.0, dL, theta, phi, iota, psi, t_c, Phi_c),
                        gps0 = gps0, f_min = f_min, approx = args.model,
                        detectors = detector_list, sample_rate = sample_rate)
            if hmodel_rts_list is None:
                return -np.inf
            yres_rts_list = [rTimeSeries_add(signal_rts_list[i], -hmodel_rts_list[i], srate = sample_rate, method = 1) for i in range(len(detector_list))]
            lnlike = np.sum([-0.5 * residuals_inner_product(yres_rts_list[i], detector_list[i], fmin = None, fmax = None) for i in range(len(detector_list))])
    ...
    if model_use == 'full':
        ...
        p0 = np.array([params0[0], params0[1],  # q, Mchirp
                       params0[2], params0[3], params0[4], # th1, ph1, chi1
                       params0[5], params0[6], params0[7], # th2, ph2, chi2
                       params0[9], params0[10], params0[11], # dL, theta, phi
                       params0[12], params0[13], params0[14], params0[15]]) # iota, psi, t_c, Phi_c
        sys.stderr.write(f'mcmc p0 = {p0}\n')
        if args.debug:
            testprob = log_likelihood(params0)
            sys.stderr.write(f'test run: prob0 = {testprob}\n')
        ...
        p0 = np.array([params0[0], params0[1], params0[4], params0[7], params0[8], # q, Mchirp, chi1, chi2, e0
                       params0[9], params0[10], params0[11], # dL, theta, phi
                       params0[12], params0[13], params0[14], params0[15]]) # iota, psi, t_c, Phi_c
        sys.stderr.write(f'mcmc p0 = {p0}\n')
        if args.debug:
