...
static int EOBHighSRStopConditionEcc_withFMax(double t,
                                  const double values[],
                                  double dvalues[],
                                  void *funcParams)
{
...
static int
XLALEOBSpinPrecStopConditionBasedOnPR_withFMax(double t, 
                                      const double values[],
                                      double dvalues[],
                                      void *funcParams) 
{
...
static int
XLALEOBSpinPrecStopConditionBasedOnPR_inverse(double t, 
                                      const double values[],
                                      double dvalues[],
                                      void *funcParams) 
{
                          ...
                          INT flagConstantSampling)
{
    ...
    if (seobParams->alignedSpins)
    {
        ...
        */
        ...
        integrator = XLALAdaptiveRungeKutta4Init(
            nb_Hamiltonian_variables_spinsaligned, XLALSpinAlignedHcapDerivative_inverse,
    ...
    else
    {
        ...
        integrator = XLALAdaptiveRungeKutta4Init(
            nb_Hamiltonian_variables, XLALSpinPrecHcapNumericalDerivative_inverse,
    ...
    if (seobParams->alignedSpins)
    {
        ...
        if (!flagConstantSampling)
            retLen = XLALAdaptiveRungeKutta4NoInterpolate(
                ...
                deltaT, deltaT_min, &dynamics_spinaligned_inverse);
        else
    ...
    else 
    {
        ...
        if (!flagConstantSampling)
            ...
            retLen = XLALAdaptiveRungeKutta4(integrator, seobParams, values->data, 0.,
                                            tend - tstart, deltaT, &dynamics_inverse);
        if (retLen < 0)
        {
            PRINT_LOG_INFO(LOG_CRITICAL, "Integration Failed!!!");
            failed = 1;
        ...
        REAL8 *ret_prT, REAL8 *ret_prDot, REAL8 *ret_r, REAL8 *ret_rDot, REAL8 *ret_L)
{
    ...
    // prDot3 = -0.25*(e*(-((1 + 3*cz2)*e) + 6*cz*(1 + e2))*(chi1*(2 + 2*dm - eta) - chi2*(-2 + 2*dm + eta)))/pf7;
    // prDot4 = (e*(e*(3*cz3*e*(-2*eta*(2 + chi12sq*eta) + chi22*(-1 + dm + 4*eta - 2*dm*eta) + 
    ...
    //     3*(-1 + 4*chi12 + 8*chi1*chi2 + 4*chi22)*eta2 + e4*(55 - 17*eta + 3*eta2) + 
    ...
    // pr0 = e2/pf2;
    // pr2 = (2*(-cz + e)*e3)/pf4;
    // pr3 = -(((-cz + e)*e3*(chi1*(2 + 2*dm - eta) - chi2*(-2 + 2*dm + eta)))/pf5);
    // pr4 = (e2*(-18 - 3*chi12 - 3*chi22 - 3*chi12*dm + 3*chi22*dm + 24*e2 + 2*chi12*e2 + 
    ...
    pr4 = (e*(22 + 3*chi22*(-1 + dm) - 3*chi12*(1 + dm) + 8*e4 + 
        ...
        2*(5 - 2*chi1*chi2 - chi22*(-3 + dm) + chi12*(3 + dm))*eta + 
        2*chi12sq*eta2) + 
        e*(2*cz*(6 + 3*chi22*(-1 + dm) - 3*chi12*(1 + dm) + 4*e2 + 
        ...
        REAL8 *ret_r, REAL8 *ret_L)
{
    ...
    // pr0 = e/L;
    // pr2 = ((-cz + e)*e2)/L3;
    // pr3 = (e*(2*c1l*dm*pe - 2*c2l*dm*pe - 2*c1e*dm*pl + 2*c2e*dm*pl - (-2 + eta)*((c1l + c2l)*pe - (c1e + c2e)*pl)))/(2.*L3);
    // pr4 = (e*(2*c1n2*dm + 6*c2c2*dm - 2*c2n2*dm - 3*c1n2*dm*e2 - 4*c2c2*dm*e2 + 3*c2n2*dm*e2 + 
    //     e2*(-3*c1n2 + 4*c2c2 - 3*c2n2 + 16*e2) - 4*c1n2*dm*eta - 12*c2c2*dm*eta + 
    //     4*c2n2*dm*eta - 6*c1n2*dm*e2*eta + 4*c2c2*dm*e2*eta + 6*c2n2*dm*e2*eta + 
    //     4*c1c2*(1 + dm)*(-3*eta + e2*(1 + eta)) + 
    ...
    //     2*eta2*(6*c2c2 - 2*c1nPc2nsq - (2*c2c2 + 3*c1nPc2nsq)*e2 + 
    //     2*pow((c1e + c2e)*L*pe + (c1l + c2l)*L*pl,2)) - 2*c1l2*dm*L2*pl2 + 
    //     2*c2l2*dm*L2*pl2 + 4*c1l2*dm*eta*L2*pl2 - 4*c2l2*dm*eta*L2*pl2 + 
    //     2*(-18 + c1n2 - 3*c2c2 + c2n2 + 23*e2 - 
    //     L2*((c1e2 + c2e2)*pe2 + 2*(c1e*c1l + c2e*c2l)*pe*pl + (c1l2 + c2l2)*pl2)) + 
    //     4*eta*(-2*c2n2 - 3*c2c2*(-2 + e2) - 2*e2 - 2*c1n*(c1n + 3*c2n*e2) + 
    ...
    pr3 = (e*(2*c1l*dm*pe - 2*c2l*dm*pe - 2*c1e*dm*pl + 2*c2e*dm*pl - 
       (-2 + eta)*((c1l + c2l)*pe - (c1e + c2e)*pl)))/(2.*L3);
    pr4 = (e*(2*c1n2*dm + 6*c2c2*dm - 2*c2n2*dm - 3*c1n2*dm*e2 - 4*c2c2*dm*e2 + 3*c2n2*dm*e2 + 
        e2*(-3*c1n2 + 4*c2c2 - 3*c2n2 + 16*e2) - 4*c1n2*dm*eta - 12*c2c2*dm*eta + 
        4*c2n2*dm*eta - 6*c1n2*dm*e2*eta + 4*c2c2*dm*e2*eta + 6*c2n2*dm*e2*eta + 
        4*c1c2*(1 + dm)*(-3*eta + e2*(1 + eta)) + 
        2*c1c1*(-3*(1 + dm) + 6*(2 + dm - eta)*eta + 
        2*e2*(1 + dm - (3 + dm)*eta + eta2)) + 
        e*(cz2*e*(6 + c2n2 + 2*c2c2*dm - c2n2*dm - 4*(3 + c1c2)*eta - 4*c1c2*dm*eta - 
        4*c2c2*dm*eta + 2*c2n2*dm*eta + 2*c2n2*(-2 + eta)*eta + 
        c1c1*(-2*(1 + dm) + 4*(2 + dm - eta)*eta) - 
        2*c2c2*(1 + 2*(-2 + eta)*eta) + c1n2*(1 + dm + 2*eta*(-2 - dm + eta)) + 
        4*c1n*c2n*eta2) + 4*cz*
        ...
        c1c1*(-3*(1 + dm) + (11 + 5*dm - 5*eta)*eta) + 
        (-5*c2c2 + c1nPc2nsq)*eta2)) - 2*c1e2*dm*L2*pe2 + 
        2*c2e2*dm*L2*pe2 + 4*c1e2*dm*eta*L2*pe2 - 4*c2e2*dm*eta*L2*pe2 - 
        4*c1e*c1l*dm*L2*pe*pl + 4*c2e*c2l*dm*L2*pe*pl + 8*c1e*c1l*dm*eta*L2*pe*pl - 
        8*c2e*c2l*dm*eta*L2*pe*pl - 
        2*eta2*(6*c2c2 - 2*c1nPc2nsq - (2*c2c2 + 3*c1nPc2nsq)*e2 + 
        ...
        L2*((c1e2 + c2e2)*pe2 + 2*(c1e*c1l + c2e*c2l)*pe*pl + (c1l2 + c2l2)*pl2)) + 
        4*eta*(-2*c2n2 - 3*c2c2*(-2 + e2) - 5*e2 - 2*c1n*(c1n + 3*c2n*e2) + 
        2*L2*((c1e2 + c2e2)*pe2 + 2*(c1e*c1l + c2e*c2l)*pe*pl + (c1l2 + c2l2)*pl2)))
        )/(8.*L5);
    // calculate prDot
    REAL8 prDot0, prDot2, prDot3, prDot4;
    ...
    rDot4 = (e*(-2*c1n2 - 2*c2c2 - 2*c2n2 - 2*c1n2*dm + 2*c2c2*dm + 2*c2n2*dm + 28*e2 - 
    ...
    r_0 = L2;
    r_2 = -3-e2;
    r_3 = -2*c1l*dm*pe + 2*c2l*dm*pe + (c1l + c2l)*(-2 + eta)*pe + 2*c1e*dm*pl - 2*c2e*dm*pl - 
        (c1e + c2e)*(-2 + eta)*pl;
    r_4 = (3*c1n2 - 2*c2c2 + 3*c2n2 + 3*c1n2*dm + 2*c2c2*dm - 3*c2n2*dm - 12*e2 + c1n2*dm*e2 + 
        2*c2c2*dm*e2 - c2n2*dm*e2 + (c1n2 - 2*c2c2 + c2n2 - 2*e2)*e2 + 6*(-3 + eta) + 
        3*c1n2*dm*eta - c2c2*dm*eta - 3*c2n2*dm*eta + c1n2*dm*e2*eta - 
        3*c2c2*dm*e2*eta - c2n2*dm*e2*eta - c1c2*(1 + dm)*(3 + eta + e2*(1 + 3*eta)) + 
        c1c1*(-2*(1 + dm) + (5 + dm - eta)*eta + 
        e2*(-2*(1 + dm) + (7 + 3*dm - 3*eta)*eta)) + c1e2*dm*L2*pe2 - 
        c2e2*dm*L2*pe2 - 2*c1e2*dm*eta*L2*pe2 + 2*c2e2*dm*eta*L2*pe2 + 
        2*c1e*c1l*dm*L2*pe*pl - 2*c2e*c2l*dm*L2*pe*pl - 4*c1e*c1l*dm*eta*L2*pe*pl + 
        4*c2e*c2l*dm*eta*L2*pe*pl - eta2*
        (c2c2 + 3*c1nPc2nsq + (3*c2c2 + c1nPc2nsq)*e2 - 
        2*pow((c1e + c2e)*L*pe + (c1l + c2l)*L*pl,2)) + c1l2*dm*L2*pl2 - 
        c2l2*dm*L2*pl2 - 2*c1l2*dm*eta*L2*pl2 + 2*c2l2*dm*eta*L2*pl2 + 
        L2*((c1e2 + c2e2)*pe2 + 2*(c1e*c1l + c2e*c2l)*pe*pl + (c1l2 + c2l2)*pl2) - 
        eta*(-5*c2c2 + 3*(c1n2 - 6*c1n*c2n + c2n2) - 2*e2 + 
        (c1n2 - 7*c2c2 - 6*c1n*c2n + c2n2)*e2 + 
        4*L2*((c1e2 + c2e2)*pe2 + 2*(c1e*c1l + c2e*c2l)*pe*pl + (c1l2 + c2l2)*pl2)))/
        (2.*L2);
    ...
    L3_3 = cbL3_0*((3*(2*c1l*dm*pe - 2*c2l*dm*pe - (c1l + c2l)*(-2 + eta)*pe - 2*c1e*dm*pl + 
        2*c2e*dm*pl + (c1e + c2e)*(-2 + eta)*pl))/2.);
    L3_4 = (cbL3_0*(36*c1c1*dm + 72*c1c2*dm - 72*c1n2*dm - 36*c2c2*dm + 72*c2n2*dm - 
        9*(15 + 8*c1c1 + 8*c2c2 - 8*(c1n2 - 6*c1n*c2n + c2n2))*eta - 72*c1n2*dm*eta + 
        72*c2n2*dm*eta + 5*e4*(9 + (-3 + eta)*eta) + 
        (5 + 72*c1nPc2nsq)*eta2 + 
        2*e2*(143 - 12*c1n2 + 21*c2c2 - 12*c2n2 - 12*c1n2*dm - 21*c2c2*dm + 
        12*c2n2*dm + 21*c1c1*(1 + dm) - 21*eta + 
        12*(c1n2 - 6*c2c2 - 6*c1n*c2n + c2n2)*eta - 12*c1n2*dm*eta + 
        30*c2c2*dm*eta + 12*c2n2*dm*eta + 6*c1c2*(1 + dm)*(2 + 5*eta) + 
        6*c1c1*eta*(-12 - 5*dm + 5*eta) + 
        (-5 + 30*c2c2 + 12*c1nPc2nsq)*eta2))*Omg - 
        8*cz*e*(3*c1c1*cbL3_0*dm*Omg - 3*c2c2*cbL3_0*dm*Omg + 
        3*cbL3_0*(7 + c1c1 + c2c2 + e2)*Omg - cbL3_0*eta*Omg - 12*c1c1*cbL3_0*eta*Omg + 
        6*c1c2*cbL3_0*eta*Omg - 12*c2c2*cbL3_0*eta*Omg - 6*c1c1*cbL3_0*dm*eta*Omg + 
        6*c1c2*cbL3_0*dm*eta*Omg + 6*c2c2*cbL3_0*dm*eta*Omg + cbL3_0*e2*eta*Omg + 
        6*c1c1*cbL3_0*eta2*Omg + 6*c2c2*cbL3_0*eta2*Omg + 
        3*c1e2*(1 + dm + 2*eta*(-2 - dm + eta))*pe2 + 
        3*c2e2*(1 - dm + 2*eta*(-2 + dm + eta))*pe2 + 
        6*c2e*(c2l - c2l*dm + 2*c2l*eta*(-2 + dm + eta) + 2*c1l*eta2)*pe*pl + 
        6*c1e*pe*(c1l*(1 + dm + 2*eta*(-2 - dm + eta))*pl + 
        2*eta2*(c2e*pe + c2l*pl)) + 3*(c1l2 + c2l2)*pl2 + 3*c1l2*dm*pl2 - 
        3*c2l2*dm*pl2 - 12*(c1l2 + c2l2)*eta*pl2 - 6*c1l2*dm*eta*pl2 + 
        6*c2l2*dm*eta*pl2 + 6*pow(c1l + c2l,2)*eta2*pl2) + 
        ...
        4*c2e*(c2l*(2 - 2*dm + 4*eta*(-2 + dm + eta) + dm*e2*(-1 + 2*eta)) + 
        4*c1l*eta2)*pe*pl - 4*c1e*pe*
        (c1l*(2 + 2*dm + dm*e2*(1 - 2*eta) + 4*eta*(-2 - dm + eta))*pl + 
        4*eta2*(c2e*pe + c2l*pl)) + 
        2*(c2l2*(-2 + 2*dm + dm*e2*(1 - 2*eta) - 4*eta*(-2 + dm + eta)) + 
        c1l2*(-2*(1 + dm) + 4*(2 + dm - eta)*eta + dm*e2*(-1 + 2*eta)) - 
        8*c1l*c2l*eta2)*pl2) - 12*(cz*cz)*e2*
        ...
        + 2*cz2*e2*(-((16 + 3*c1c1 + 3*c2c2)*cbL3_0*Omg) + 3*c1e2*dm*(-1 + 2*eta)*pe2 + 
        ...
        2*c2l2*dm*eta*pl2)))/(24.*cbL3_0*cbL3_0*Omg);
    *ret_prT = sz*(pr0 + pr2 + pr3 + pr4);
    *ret_prDot = eczp2*(prDot0 + prDot2 + prDot3 + prDot4);
    *ret_rDot = sz * (rDot0 + rDot2 + rDot3 + rDot4);
